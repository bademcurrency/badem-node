...
namespace
{
class callback_visitor : public badem::message_visitor
{
public:
	void keepalive (badem::keepalive const & message_a) override
	{
		result = badem::stat::detail::keepalive;
	}
	void publish (badem::publish const & message_a) override
	{
		result = badem::stat::detail::publish;
	}
	void confirm_req (badem::confirm_req const & message_a) override
	{
		result = badem::stat::detail::confirm_req;
	}
	void confirm_ack (badem::confirm_ack const & message_a) override
	{
		result = badem::stat::detail::confirm_ack;
	}
	void bulk_pull (badem::bulk_pull const & message_a) override
	{
		result = badem::stat::detail::bulk_pull;
	}
	void bulk_pull_account (badem::bulk_pull_account const & message_a) override
	{
		result = badem::stat::detail::bulk_pull_account;
	}
	void bulk_push (badem::bulk_push const & message_a) override
	{
		result = badem::stat::detail::bulk_push;
	}
	void frontier_req (badem::frontier_req const & message_a) override
	{
		result = badem::stat::detail::frontier_req;
	}
	void node_id_handshake (badem::node_id_handshake const & message_a) override
	{
		result = badem::stat::detail::node_id_handshake;
	}
	badem::stat::detail result;
};
}

badem::endpoint badem::transport::map_endpoint_to_v6 (badem::endpoint const & endpoint_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
		endpoint_l = badem::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint_l.address ().to_v4 ()), endpoint_l.port ());
	}
	return endpoint_l;
}

badem::endpoint badem::transport::map_tcp_to_endpoint (badem::tcp_endpoint const & endpoint_a)
{
	return badem::endpoint (endpoint_a.address (), endpoint_a.port ());
}

badem::tcp_endpoint badem::transport::map_endpoint_to_tcp (badem::endpoint const & endpoint_a)
{
	return badem::tcp_endpoint (endpoint_a.address (), endpoint_a.port ());
}

badem::transport::channel::channel (badem::node & node_a) :
...
void badem::transport::channel::send (badem::message const & message_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, bool const is_droppable_a)
{
	...
	if (!is_droppable_a || !limiter.should_drop (buffer.size ()))
	{
		...
		node.stats.inc (badem::stat::type::message, detail, badem::stat::dir::out);
	}
	else
	{
		node.stats.inc (badem::stat::type::drop, detail, badem::stat::dir::out);
...
bool badem::transport::reserved_address (badem::endpoint const & endpoint_a, bool allow_local_peers)
{
...
badem::bandwidth_limiter::bandwidth_limiter (const size_t limit_a) :
...
bool badem::bandwidth_limiter::should_drop (const size_t & message_size)
{
	bool result (false);
...
size_t badem::bandwidth_limiter::get_rate ()
{
	badem::lock_guard<std::mutex> lock (mutex);
	return trended_rate;
}
