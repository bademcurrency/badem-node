...
namespace transport
{
	...
	public:
		channel_tcp (badem::node &, std::shared_ptr<badem::socket>);
		~channel_tcp ();
		...
		std::function<void(boost::system::error_code const &, size_t)> tcp_callback (std::shared_ptr<std::vector<uint8_t>>, badem::stat::detail, badem::tcp_endpoint const &, std::function<void(boost::system::error_code const &, size_t)> const & = nullptr) const;
		...
		badem::endpoint get_endpoint () const override
		{
			...
			if (socket)
			{
				return badem::transport::map_tcp_to_endpoint (socket->remote_endpoint ());
			}
			else
			{
	...
	public:
		...
		bool insert (std::shared_ptr<badem::transport::channel_tcp>);
		...
		void process_keepalive (badem::keepalive const &, badem::tcp_endpoint const &, bool);
