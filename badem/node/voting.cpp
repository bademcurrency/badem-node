...
void badem::vote_generator::send (std::unique_lock<std::mutex> & lock_a)
{
	...
	lock_a.unlock ();
	{
		...
		node.wallets.foreach_representative (transaction, [this, &hashes_l, &transaction](badem::public_key const & pub_a, badem::raw_key const & prv_a) {
