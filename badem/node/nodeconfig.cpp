...
// once badem::network is factored out of node.{c|h}pp
...
const char * default_live_peer_network = "node.badem.io";
}

badem::node_config::node_config () :
node_config (0, badem::logging ())
{
}

badem::node_config::node_config (uint16_t peering_port_a, badem::logging const & logging_a) :
...
logging (logging_a)
{
	...
	max_work_generate_difficulty = badem::difficulty::from_multiplier (max_work_generate_multiplier, network_params.network.publish_threshold);
	switch (network_params.network.network ())
	{
		case badem::badem_networks::badem_test_network:
			enable_voting = true;
			preconfigured_representatives.push_back (network_params.ledger.genesis_account);
			break;
		case badem::badem_networks::badem_beta_network:
			...
			break;
		case badem::badem_networks::badem_live_network:
...
badem::error badem::node_config::serialize_toml (badem::tomlconfig & toml) const
{
	...
	badem::tomlconfig experimental_l;
...
badem::error badem::node_config::deserialize_toml (badem::tomlconfig & toml)
{
	try
	{
		...
		badem::network_constants network;
		toml.get<double> ("max_work_generate_multiplier", max_work_generate_multiplier);
		max_work_generate_difficulty = badem::difficulty::from_multiplier (max_work_generate_multiplier, network.publish_threshold);
		...
		if (frontiers_confirmation == badem::frontiers_confirmation_mode::invalid)
		{
...
badem::error badem::node_config::serialize_json (badem::jsonconfig & json) const
{
...
bool badem::node_config::upgrade_json (unsigned version_a, badem::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
		...
		case 2:
		{
			json.put ("inactive_supply", badem::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			json.put ("receive_minimum", badem::bdm_ratio.convert_to<std::string> ());
		case 4:
			json.erase ("receive_minimum");
			json.put ("receive_minimum", badem::bdm_ratio.convert_to<std::string> ());
		case 5:
		...
		case 9:
			json.put ("state_block_parse_canary", badem::block_hash (0).to_string ());
			json.put ("state_block_generate_canary", badem::block_hash (0).to_string ());
		case 10:
			json.put ("online_weight_minimum", online_weight_minimum.to_string_dec ());
...
badem::error badem::node_config::deserialize_json (bool & upgraded_a, badem::jsonconfig & json)
{
	try
	{
...
std::string badem::node_config::serialize_frontiers_confirmation (badem::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case badem::frontiers_confirmation_mode::always:
			return "always";
		case badem::frontiers_confirmation_mode::automatic:
			return "auto";
		case badem::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
badem::frontiers_confirmation_mode badem::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return badem::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return badem::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return badem::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return badem::frontiers_confirmation_mode::invalid;
	}
}

void badem::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
badem::account badem::node_config::random_representative () const
{
	assert (!preconfigured_representatives.empty ());
	size_t index (badem::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
