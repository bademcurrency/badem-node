...
badem::bootstrap_client::bootstrap_client (std::shared_ptr<badem::node> node_a, std::shared_ptr<badem::bootstrap_attempt> attempt_a, std::shared_ptr<badem::transport::channel_tcp> channel_a) :
node (node_a),
attempt (attempt_a),
channel (channel_a),
...
{
	auto this_l (shared_from_this ());
	connection->channel->socket->async_read (connection->receive_buffer, badem::frontier_req_client::size_frontier, [this_l](boost::system::error_code const & ec, size_t size_a) {
...
void badem::bulk_pull_client::request ()
{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_request_failure, badem::stat::dir::in);
		}
	},
	false); // is bootstrap traffic is_droppable false
}
...
void badem::bulk_pull_client::receive_block ()
{
	...
	connection->channel->socket->async_read (connection->receive_buffer, 1, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_receive_block_failure, badem::stat::dir::in);
		}
	});
}
...
void badem::bulk_pull_client::received_type ()
{
	...
	switch (type)
	{
		...
		{
			connection->channel->socket->async_read (connection->receive_buffer, badem::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			connection->channel->socket->async_read (connection->receive_buffer, badem::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->channel->socket->async_read (connection->receive_buffer, badem::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->channel->socket->async_read (connection->receive_buffer, badem::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->channel->socket->async_read (connection->receive_buffer, badem::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void badem::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, badem::block_type type_a)
{
	if (!ec)
	{
		...
		else
		{
			...
			connection->node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_deserialize_receive_block, badem::stat::dir::in);
		}
	}
	else
	{
		...
		connection->node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_receive_block_failure, badem::stat::dir::in);
	}
}
...
void badem::bulk_pull_account_client::request ()
{
	...
	req, [this_l](boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_error_starting_request, badem::stat::dir::in);
		}
	},
	false); // is bootstrap traffic is_droppable false
}
...
void badem::bootstrap_attempt::populate_connections ()
{
	...
	if (connections < target)
	{
		...
		for (auto i = 0u; i < delta; i++)
		{
			...
			if (endpoint != badem::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && endpoints.find (endpoint) == endpoints.end ())
			{
...
void badem::bootstrap_attempt::connect_client (badem::tcp_endpoint const & endpoint_a)
{
	...
	[this_l, socket, endpoint_a](boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<badem::bootstrap_client> (this_l->node, this_l, std::make_shared<badem::transport::channel_tcp> (*this_l->node, socket)));
...
void badem::bootstrap_attempt::lazy_pull_flush ()
{
	...
	for (auto & pull_start : lazy_pulls)
	{
		...
		if (lazy_blocks.find (pull_start) == lazy_blocks.end () && !node->store.block_exists (transaction, pull_start))
		{
			...
			pulls.push_back (badem::pull_info (pull_start, pull_start, badem::block_hash (0), static_cast<badem::pull_info::count_t> (node->network_params.bootstrap.lazy_max_pull_blocks)));
		}
	}
	lazy_pulls.clear ();
}
...
bool badem::bootstrap_attempt::process_block (std::shared_ptr<badem::block> block_a, badem::account const & known_account_a, uint64_t pull_blocks, bool block_expected)
{
...
void badem::bootstrap_initiator::bootstrap (badem::endpoint const & endpoint_a, bool add_to_peers)
{
	if (add_to_peers)
	{
		node.network.udp_channels.insert (badem::transport::map_endpoint_to_v6 (endpoint_a), badem::protocol_version);
	}
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
...
void badem::bootstrap_server::receive_bulk_pull_account_action (boost::system::error_code const & ec, size_t size_a, badem::message_header const & header_a)
{
	if (!ec)
	{
		...
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
				node->logger.try_log (boost::str (boost::format ("Received bulk pull account for %1% with a minimum amount of %2%") % request->account.to_account () % badem::amount (request->minimum_amount).format_balance (badem::BDM_ratio, 10, true)));
			}
			if (is_bootstrap_connection ())
			{
				add_request (std::unique_ptr<badem::message> (request.release ()));
...
void badem::bootstrap_server::receive_keepalive_action (boost::system::error_code const & ec, size_t size_a, badem::message_header const & header_a)
{
	if (!ec)
	{
		...
		if (!error)
		{
			if (type == badem::bootstrap_server_type::realtime || type == badem::bootstrap_server_type::realtime_response_server)
			{
				add_request (std::unique_ptr<badem::message> (request.release ()));
			}
			receive ();
	...
	void node_id_handshake (badem::node_id_handshake const & message_a) override
	{
		...
		if (message_a.query)
		{
			...
			connection->socket->async_write (bytes, [ bytes, connection = connection ](boost::system::error_code const & ec, size_t size_a) {
				...
				{
					connection->node->stats.inc (badem::stat::type::message, badem::stat::detail::node_id_handshake, badem::stat::dir::out);
...
void badem::pulls_cache::add (badem::pull_info const & pull_a)
{
	if (pull_a.processed > 500)
	{
		...
		badem::uint512_union head_512 (pull_a.account, pull_a.head_original);
