...
{
	switch (static_cast<badem::error_cli> (ev))
	{
		case badem::error_cli::generic:
			return "Unknown error";
		case badem::error_cli::parse_error:
			return "Coud not parse command line";
		case badem::error_cli::invalid_arguments:
...
void badem::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
std::error_code badem::handle_node_options (boost::program_options::variables_map & vm)
{
	...
	else if (vm.count ("diagnostics"))
	{
		...
		badem::raw_key key;
		key.data.clear ();
		badem::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		badem::raw_key junk1;
		junk1.data.clear ();
		badem::uint256_union junk2 (0);
		badem::kdf kdf;
		kdf.phs (junk1, "", junk2);
	...
	{
		ec = badem::error_cli::unknown_command;
	}
