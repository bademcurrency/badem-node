...
constexpr double badem::bootstrap_limits::bootstrap_connection_scale_target_blocks;
constexpr double badem::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy;
constexpr double badem::bootstrap_limits::bootstrap_minimum_blocks_per_sec;
constexpr double badem::bootstrap_limits::bootstrap_minimum_termination_time_sec;
constexpr unsigned badem::bootstrap_limits::bootstrap_max_new_connections;
constexpr size_t badem::bootstrap_limits::bootstrap_max_confirm_frontiers;
constexpr double badem::bootstrap_limits::required_frontier_confirmation_ratio;
constexpr unsigned badem::bootstrap_limits::frontier_confirmation_blocks_limit;
constexpr unsigned badem::bootstrap_limits::requeued_pulls_limit;
constexpr unsigned badem::bootstrap_limits::requeued_pulls_limit_test;
constexpr std::chrono::seconds badem::bootstrap_limits::lazy_flush_delay_sec;
constexpr unsigned badem::bootstrap_limits::lazy_destinations_request_limit;
constexpr uint64_t badem::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit;
constexpr double badem::bootstrap_limits::lazy_batch_pull_count_resize_ratio;
constexpr size_t badem::bootstrap_limits::lazy_blocks_restart_limit;
constexpr std::chrono::hours badem::bootstrap_excluded_peers::exclude_time_hours;
constexpr std::chrono::hours badem::bootstrap_excluded_peers::exclude_remove_hours;

badem::bootstrap_client::bootstrap_client (std::shared_ptr<badem::node> node_a, std::shared_ptr<badem::bootstrap_attempt> attempt_a, std::shared_ptr<badem::transport::channel_tcp> channel_a, std::shared_ptr<badem::socket> socket_a) :
node (node_a),
attempt (attempt_a),
channel (channel_a),
socket (socket_a),
...
{
	auto elapsed = std::max (elapsed_seconds (), badem::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
...
badem::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<badem::node> node_a, badem::bootstrap_mode mode_a) :
...
{
	badem::lock_guard<std::mutex> guard (next_log_mutex);
	auto result (false);
...
bool badem::bootstrap_attempt::request_frontier (badem::unique_lock<std::mutex> & lock_a, bool first_attempt)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		if (node->config.logging.network_logging ())
		{
			...
			{
				node->stats.inc (badem::stat::type::error, badem::stat::detail::frontier_req, badem::stat::dir::out);
			}
		}
	}
	return result;
...
void badem::bootstrap_attempt::request_pull (badem::unique_lock<std::mutex> & lock_a)
{
	...
	if (connection_l)
	{
		...
		if (recent_pulls_head.size () > badem::bootstrap_limits::bootstrap_max_confirm_frontiers)
		{
...
void badem::bootstrap_attempt::request_push (badem::unique_lock<std::mutex> & lock_a)
{
	bool error (false);
...
void badem::bootstrap_attempt::run_start (badem::unique_lock<std::mutex> & lock_a)
{
...
std::shared_ptr<badem::bootstrap_client> badem::bootstrap_attempt::connection (badem::unique_lock<std::mutex> & lock_a, bool use_front_connection)
{
	// clang-format off
...
unsigned badem::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
	...
	double target_blocks = (mode == badem::bootstrap_mode::lazy) ? badem::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy : badem::bootstrap_limits::bootstrap_connection_scale_target_blocks;
	double step_scale = std::min (1.0, std::max (0.0, (double)pulls_remaining / target_blocks));
	double lazy_term = (mode == badem::bootstrap_mode::lazy) ? (double)node->config.bootstrap_connections : 0.0;
...
void badem::bootstrap_attempt::populate_connections ()
{
		...
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				if (auto socket_l = client->channel->socket.lock ())
				{
					...
					if (client->elapsed_seconds () > badem::bootstrap_limits::bootstrap_connection_warmup_time_sec && client->block_count > 0)
					{
					...
					if (elapsed_sec > badem::bootstrap_limits::bootstrap_minimum_termination_time_sec && blocks_per_sec < badem::bootstrap_limits::bootstrap_minimum_blocks_per_sec)
					{
						if (node->config.logging.bulk_pull_logging ())
						{
							node->logger.try_log (boost::str (boost::format ("Stopping slow peer %1% (elapsed sec %2%s > %3%s and %4% blocks per second < %5%)") % client->channel->to_string () % elapsed_sec % badem::bootstrap_limits::bootstrap_minimum_termination_time_sec % blocks_per_sec % badem::bootstrap_limits::bootstrap_minimum_blocks_per_sec));
						}
	...
	{
		auto delta = std::min ((target - connections) * 2, badem::bootstrap_limits::bootstrap_max_new_connections);
		// TODO - tune this better
		// Not many peers respond, need to try to make more connections than we need.
		for (auto i = 0u; i < delta; i++)
		{
			auto endpoint (node->network.bootstrap_peer (mode == badem::bootstrap_mode::lazy));
			if (endpoint != badem::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && endpoints.find (endpoint) == endpoints.end () && !node->bootstrap_initiator.excluded_peers.check (endpoint))
			{
...
void badem::bootstrap_attempt::start_populate_connections ()
{
...
{
	connect_client (badem::tcp_endpoint (endpoint_a.address (), endpoint_a.port ()));
}

void badem::bootstrap_attempt::connect_client (badem::tcp_endpoint const & endpoint_a)
{
	++connections;
	auto socket (std::make_shared<badem::socket> (node));
	...
	[this_l, socket, endpoint_a](boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<badem::bootstrap_client> (this_l->node, this_l, std::make_shared<badem::transport::channel_tcp> (*this_l->node, socket), socket));
...
void badem::bootstrap_attempt::add_pull (badem::pull_info const & pull_a)
{
	badem::pull_info pull (pull_a);
...
void badem::bootstrap_attempt::requeue_pull (badem::pull_info const & pull_a, bool network_error)
{
	auto pull (pull_a);
	if (!network_error)
	{
	...
	++requeued_pulls;
	if (mode != badem::bootstrap_mode::lazy && pull.attempts < pull.retry_limit + (pull.processed / 10000))
	{
		badem::lock_guard<std::mutex> lock (mutex);
		pulls.push_front (pull);
		condition.notify_all ();
	}
	else if (mode == badem::bootstrap_mode::lazy && (pull.retry_limit == std::numeric_limits<unsigned>::max () || pull.attempts <= pull.retry_limit + (pull.processed / node->network_params.bootstrap.lazy_max_pull_blocks)))
	{
	...
	else
	{
		...
		node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_failed_account, badem::stat::dir::in);

		node->bootstrap_initiator.cache.add (pull);
		if (mode == badem::bootstrap_mode::lazy && pull.processed > 0)
		{
			assert (pull.account_or_head == pull.head);
			badem::lock_guard<std::mutex> lazy_lock (lazy_mutex);
...
void badem::bootstrap_attempt::attempt_restart_check (badem::unique_lock<std::mutex> & lock_a)
{
	...
	if (!frontiers_confirmed && (requeued_pulls > (!node->network_params.network.is_test_network () ? badem::bootstrap_limits::requeued_pulls_limit : badem::bootstrap_limits::requeued_pulls_limit_test) || total_blocks > badem::bootstrap_limits::frontier_confirmation_blocks_limit))
	{
		auto confirmed (confirm_frontiers (lock_a));
		assert (lock_a.owns_lock ());
		if (!confirmed)
		{
			node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::frontier_confirmation_failed, badem::stat::dir::in);
			auto score (node->bootstrap_initiator.excluded_peers.add (endpoint_frontier_request, node->network.size ()));
			if (score >= badem::bootstrap_excluded_peers::score_limit)
			{
		...
		{
			node->stats.inc (badem::stat::type::bootstrap, badem::stat::detail::frontier_confirmation_successful, badem::stat::dir::in);
		}
		frontiers_confirmed = confirmed;
	}
}

bool badem::bootstrap_attempt::confirm_frontiers (badem::unique_lock<std::mutex> & lock_a)
{
	...
	// clang-format on
	std::vector<badem::block_hash> frontiers;
	for (auto i (pulls.begin ()), end (pulls.end ()); i != end && frontiers.size () != badem::bootstrap_limits::bootstrap_max_confirm_frontiers; ++i)
	{
	...
	for (auto i (recent_pulls_head.begin ()), end (recent_pulls_head.end ()); i != end && frontiers.size () != badem::bootstrap_limits::bootstrap_max_confirm_frontiers; ++i)
	{
	...
	if (frontiers_count > 0)
	{
		...
		{
			auto k = badem::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (representatives_copy.size () - 1));
		...
		{
			std::unordered_map<std::shared_ptr<badem::transport::channel>, std::deque<std::pair<badem::block_hash, badem::root>>> batched_confirm_req_bundle;
			std::deque<std::pair<badem::block_hash, badem::root>> request;
			...
			for (auto ii (frontiers.begin ()); ii != frontiers.end ();)
			{
				...
				{
					badem::lock_guard<std::mutex> active_lock (node->active.mutex);
					auto existing (node->active.find_inactive_votes_cache (*ii));
					badem::uint128_t tally;
					...
					else
					{
						for (auto const & rep : representatives)
						{
							if (std::find (existing.voters.begin (), existing.voters.end (), rep.account) == existing.voters.end ())
							{
								...
								{
									std::deque<std::pair<badem::block_hash, badem::root>> insert_root_hash = { std::make_pair (*ii, *ii) };
			...
			if (confirmed_count >= frontiers_count * badem::bootstrap_limits::required_frontier_confirmation_ratio) // 80% of frontiers confirmed
			{
				confirmed = true;
			}
...
void badem::bootstrap_attempt::lazy_start (badem::hash_or_account const & hash_or_account_a, bool confirmed)
{
	badem::lock_guard<std::mutex> lazy_lock (lazy_mutex);
...
void badem::bootstrap_attempt::lazy_add (badem::hash_or_account const & hash_or_account_a, unsigned retry_limit)
{
...
void badem::bootstrap_attempt::lazy_requeue (badem::block_hash const & hash_a, badem::block_hash const & previous_a, bool confirmed_a)
{
	badem::unique_lock<std::mutex> lazy_lock (lazy_mutex);
	...
	if (existing != lazy_blocks.end ())
	{
		...
		lazy_lock.unlock ();
		requeue_pull (badem::pull_info (hash_a, hash_a, previous_a, static_cast<badem::pull_info::count_t> (1), confirmed_a ? std::numeric_limits<unsigned>::max () : node->network_params.bootstrap.lazy_destinations_retry_limit));
	}
}
...
{
	assert (!mutex.try_lock ());
	static size_t const max_pulls (badem::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy * 3);
	if (pulls.size () < max_pulls)
	{
		last_lazy_flush = std::chrono::steady_clock::now ();
		badem::lock_guard<std::mutex> lazy_lock (lazy_mutex);
		assert (node->network_params.bootstrap.lazy_max_pull_blocks <= std::numeric_limits<badem::pull_info::count_t>::max ());
		badem::pull_info::count_t batch_count (node->network_params.bootstrap.lazy_max_pull_blocks);
		if (total_blocks > badem::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit && !lazy_blocks.empty ())
		{
			double lazy_blocks_ratio (total_blocks / lazy_blocks.size ());
			if (lazy_blocks_ratio > badem::bootstrap_limits::lazy_batch_pull_count_resize_ratio)
			{
				// Increasing blocks ratio weight as more important (^3). Small batch count should lower blocks ratio below target
				double lazy_blocks_factor (std::pow (lazy_blocks_ratio / badem::bootstrap_limits::lazy_batch_pull_count_resize_ratio, 3.0));
				// Decreasing total block count weight as less important (sqrt)
				double total_blocks_factor (std::sqrt (total_blocks / badem::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit));
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			{
				pulls.emplace_back (pull_start.first, pull_start.first, badem::block_hash (0), batch_count, pull_start.second);
				++count;
			}
...
bool badem::bootstrap_attempt::lazy_has_expired () const
{
	bool result (false);
	...
	else if (!node->flags.disable_legacy_bootstrap && lazy_blocks_count > badem::bootstrap_limits::lazy_blocks_restart_limit)
	{
		result = true;
	}
	return result;
...
void badem::bootstrap_attempt::lazy_run ()
{
	...
	while ((still_pulling () || !lazy_finished ()) && !lazy_has_expired ())
	{
		...
		while (still_pulling () && !lazy_has_expired ())
		{
			...
			++iterations;
			// Flushing lazy pulls
			if (iterations % 100 == 0 || last_lazy_flush + badem::bootstrap_limits::lazy_flush_delay_sec < std::chrono::steady_clock::now ())
			{
				lazy_pull_flush ();
			}
...
bool badem::bootstrap_attempt::process_block (std::shared_ptr<badem::block> block_a, badem::account const & known_account_a, uint64_t pull_blocks, badem::bulk_pull::count_t max_blocks, bool block_expected, unsigned retry_limit)
{
...
bool badem::bootstrap_attempt::process_block_lazy (std::shared_ptr<badem::block> block_a, badem::account const & known_account_a, uint64_t pull_blocks, badem::bulk_pull::count_t max_blocks, unsigned retry_limit)
{
	bool stop_pull (false);
	...
	if (lazy_blocks.find (hash) == lazy_blocks.end ())
	{
		...
		if (pull_blocks == 0 && (block_a->type () == badem::block_type::state || block_a->type () == badem::block_type::send))
		{
		...
		lazy_lock.unlock ();
		badem::unchecked_info info (block_a, known_account_a, 0, badem::signature_verification::unknown, retry_limit == std::numeric_limits<unsigned>::max ());
		node->block_processor.add (info);
	}
...
void badem::bootstrap_attempt::lazy_block_state (std::shared_ptr<badem::block> block_a, unsigned retry_limit)
{
	std::shared_ptr<badem::state_block> block_l (std::static_pointer_cast<badem::state_block> (block_a));
	if (block_l != nullptr)
	{
		auto transaction (node->store.tx_begin_read ());
		badem::uint128_t balance (block_l->hashables.balance.number ());
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && lazy_blocks.find (link) == lazy_blocks.end () && !node->store.block_exists (transaction, link))
		{
			...
			{
				lazy_state_backlog.emplace (previous, badem::lazy_state_backlog_item{ link, balance, retry_limit });
			}
		}
	}
}

void badem::bootstrap_attempt::lazy_block_state_backlog_check (std::shared_ptr<badem::block> block_a, badem::block_hash const & hash_a)
{
	...
	if (find_state != lazy_state_backlog.end ())
	{
		...
		if (block_a->type () == badem::block_type::state || block_a->type () == badem::block_type::send)
		{
...
void badem::bootstrap_attempt::lazy_backlog_cleanup ()
{
	auto transaction (node->store.tx_begin_read ());
	badem::lock_guard<std::mutex> lazy_lock (lazy_mutex);
...
void badem::bootstrap_attempt::lazy_destinations_increment (badem::account const & destination_a)
{
	...
	if (node->flags.disable_legacy_bootstrap)
	{
		...
		{
			lazy_destinations.get<account_tag> ().modify (existing, [](badem::lazy_destinations_item & item_a) {
				++item_a.count;
			});
		}
		else
		{
			lazy_destinations.insert (badem::lazy_destinations_item{ destination_a, 1 });
		}
	}
}

void badem::bootstrap_attempt::lazy_destinations_flush ()
{
	lazy_destinations_flushed = true;
	size_t count (0);
	badem::lock_guard<std::mutex> lazy_lock (lazy_mutex);
	for (auto it (lazy_destinations.get<count_tag> ().begin ()), end (lazy_destinations.get<count_tag> ().end ()); it != end && count < badem::bootstrap_limits::lazy_destinations_request_limit && !stopped;)
	{
...
bool badem::bootstrap_attempt::lazy_processed_or_exists (badem::block_hash const & hash_a)
{
	bool result (false);
	badem::unique_lock<std::mutex> lazy_lock (lazy_mutex);
...
void badem::bootstrap_attempt::request_pending (badem::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
...
void badem::bootstrap_initiator::bootstrap (bool force)
{
	badem::unique_lock<std::mutex> lock (mutex);
...
void badem::bootstrap_initiator::bootstrap (badem::endpoint const & endpoint_a, bool add_to_peers, bool frontiers_confirmed)
{
	if (add_to_peers)
	{
		node.network.udp_channels.insert (badem::transport::map_endpoint_to_v6 (endpoint_a), node.network_params.protocol.protocol_version);
	}
	badem::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		{
			excluded_peers.remove (badem::transport::map_endpoint_to_tcp (endpoint_a));
		}
		if (!excluded_peers.check (badem::transport::map_endpoint_to_tcp (endpoint_a)))
		{
...
void badem::bootstrap_initiator::bootstrap_lazy (badem::hash_or_account const & hash_or_account_a, bool force, bool confirmed)
{
	{
		badem::unique_lock<std::mutex> lock (mutex);
		if (force && attempt != nullptr)
		...
		if (attempt == nullptr)
		{
			attempt = std::make_shared<badem::bootstrap_attempt> (node.shared (), badem::bootstrap_mode::lazy);
		}
...
{
	{
		...
		if (attempt == nullptr)
		{
			attempt = std::make_shared<badem::bootstrap_attempt> (node.shared (), badem::bootstrap_mode::wallet_lazy);
		}
...
{
	badem::lock_guard<std::mutex> lock (observers_mutex);
...
std::unique_ptr<seq_con_info_component> collect_seq_con_info (bootstrap_initiator & bootstrap_initiator, const std::string & name)
{
	...
	{
		badem::lock_guard<std::mutex> guard (bootstrap_initiator.observers_mutex);
		count = bootstrap_initiator.observers.size ();
	}
	{
		badem::lock_guard<std::mutex> guard (bootstrap_initiator.cache.pulls_cache_mutex);
		cache_count = bootstrap_initiator.cache.cache.size ();
	}
	{
		badem::lock_guard<std::mutex> guard (bootstrap_initiator.excluded_peers.excluded_peers_mutex);
...
void badem::pulls_cache::add (badem::pull_info const & pull_a)
{
	if (pull_a.processed > 500)
	{
		badem::lock_guard<std::mutex> guard (pulls_cache_mutex);
		// Clean old pull
		...
		badem::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
		auto existing (cache.get<account_head_tag> ().find (head_512));
		if (existing == cache.get<account_head_tag> ().end ())
		{
			// Insert new pull
			auto inserted (cache.insert (badem::cached_pulls{ std::chrono::steady_clock::now (), head_512, pull_a.head }));
			(void)inserted;
		...
		else
		{
			...
			cache.get<account_head_tag> ().modify (existing, [pull_a](badem::cached_pulls & cache_a) {
...
void badem::pulls_cache::update_pull (badem::pull_info & pull_a)
{
	badem::lock_guard<std::mutex> guard (pulls_cache_mutex);
	badem::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
	auto existing (cache.get<account_head_tag> ().find (head_512));
...
void badem::pulls_cache::remove (badem::pull_info const & pull_a)
{
	badem::lock_guard<std::mutex> guard (pulls_cache_mutex);
	badem::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
...
uint64_t badem::bootstrap_excluded_peers::add (badem::tcp_endpoint const & endpoint_a, size_t network_peers_count)
{
	uint64_t result (0);
	badem::lock_guard<std::mutex> guard (excluded_peers_mutex);
	...
	{
		// Insert new endpoint
		auto inserted (peers.insert (badem::excluded_peers_item{ std::chrono::steady_clock::steady_clock::now () + exclude_time_hours, endpoint_a, 1 }));
		(void)inserted;
		assert (inserted.second);
		result = 1;
	}
	else
	{
		...
		peers.get<endpoint_tag> ().modify (existing, [&result](badem::excluded_peers_item & item_a) {
			++item_a.score;
			result = item_a.score;
			if (item_a.score == badem::bootstrap_excluded_peers::score_limit)
			{
				item_a.exclude_until = std::chrono::steady_clock::now () + badem::bootstrap_excluded_peers::exclude_time_hours;
			}
			else if (item_a.score > badem::bootstrap_excluded_peers::score_limit)
			{
				item_a.exclude_until = std::chrono::steady_clock::now () + badem::bootstrap_excluded_peers::exclude_time_hours * item_a.score * 2;
			}
		});
	}
	return result;
...
bool badem::bootstrap_excluded_peers::check (badem::tcp_endpoint const & endpoint_a)
{
	bool excluded (false);
	badem::lock_guard<std::mutex> guard (excluded_peers_mutex);
	auto existing (peers.get<endpoint_tag> ().find (endpoint_a));
...
void badem::bootstrap_excluded_peers::remove (badem::tcp_endpoint const & endpoint_a)
{
	badem::lock_guard<std::mutex> guard (excluded_peers_mutex);
