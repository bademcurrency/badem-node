...
public:
	...
	~bootstrap_attempt ();
	void run ();
	std::shared_ptr<badem::bootstrap_client> connection (badem::unique_lock<std::mutex> &, bool = false);
	bool consume_future (std::future<bool> &);
	void populate_connections ();
	void start_populate_connections ();
	bool request_frontier (badem::unique_lock<std::mutex> &, bool = false);
	void request_pull (badem::unique_lock<std::mutex> &);
	void request_push (badem::unique_lock<std::mutex> &);
	...
	void stop ();
	void requeue_pull (badem::pull_info const &, bool = false);
	void add_pull (badem::pull_info const &);
	bool still_pulling ();
	void run_start (badem::unique_lock<std::mutex> &);
	unsigned target_connections (size_t pulls_remaining);
	bool should_log ();
	void add_bulk_push_target (badem::block_hash const &, badem::block_hash const &);
	void attempt_restart_check (badem::unique_lock<std::mutex> &);
	bool confirm_frontiers (badem::unique_lock<std::mutex> &);
	bool process_block (std::shared_ptr<badem::block>, badem::account const &, uint64_t, badem::bulk_pull::count_t, bool, unsigned);
	/** Lazy bootstrap */
	void lazy_run ();
	void lazy_start (badem::hash_or_account const &, bool confirmed = true);
	void lazy_add (badem::hash_or_account const &, unsigned = std::numeric_limits<unsigned>::max ());
	void lazy_requeue (badem::block_hash const &, badem::block_hash const &, bool);
	bool lazy_finished ();
	bool lazy_has_expired () const;
	void lazy_pull_flush ();
	void lazy_clear ();
	bool process_block_lazy (std::shared_ptr<badem::block>, badem::account const &, uint64_t, badem::bulk_pull::count_t, unsigned);
	void lazy_block_state (std::shared_ptr<badem::block>, unsigned);
	void lazy_block_state_backlog_check (std::shared_ptr<badem::block>, badem::block_hash const &);
	void lazy_backlog_cleanup ();
	void lazy_destinations_increment (badem::account const &);
	void lazy_destinations_flush ();
	bool lazy_processed_or_exists (badem::block_hash const &);
	/** Lazy bootstrap */
	/** Wallet bootstrap */
	void request_pending (badem::unique_lock<std::mutex> &);
	...
	std::unordered_map<badem::block_hash, badem::lazy_state_backlog_item> lazy_state_backlog;
	std::unordered_set<badem::block_hash> lazy_undefined_links;
	std::unordered_map<badem::block_hash, badem::uint128_t> lazy_balances;
	std::unordered_set<badem::block_hash> lazy_keys;
	std::deque<std::pair<badem::hash_or_account, unsigned>> lazy_pulls;
	...
	boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>, boost::multi_index::member<lazy_destinations_item, badem::account, &lazy_destinations_item::account>>>>
	lazy_destinations;
...
{
public:
	bootstrap_client (std::shared_ptr<badem::node>, std::shared_ptr<badem::bootstrap_attempt>, std::shared_ptr<badem::transport::channel_tcp>, std::shared_ptr<badem::socket>);
	~bootstrap_client ();
...
{
public:
	uint64_t add (badem::tcp_endpoint const &, size_t);
	bool check (badem::tcp_endpoint const &);
	...
	boost::multi_index_container<
	badem::excluded_peers_item,
	boost::multi_index::indexed_by<
	boost::multi_index::ordered_non_unique<boost::multi_index::member<badem::excluded_peers_item, std::chrono::steady_clock::time_point, &badem::excluded_peers_item::exclude_until>>,
	boost::multi_index::hashed_unique<boost::multi_index::tag<endpoint_tag>, boost::multi_index::member<badem::excluded_peers_item, badem::tcp_endpoint, &badem::excluded_peers_item::endpoint>>>>
	peers;
...
public:
	...
	~bootstrap_initiator ();
	void bootstrap (badem::endpoint const &, bool add_to_peers = true, bool frontiers_confirmed = false);
	void bootstrap (bool force = false);
	void bootstrap_lazy (badem::hash_or_account const &, bool force = false, bool confirmed = true);
	...
	badem::bootstrap_excluded_peers excluded_peers;
	void stop ();
