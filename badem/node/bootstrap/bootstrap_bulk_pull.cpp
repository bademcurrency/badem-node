...
badem::pull_info::pull_info (badem::hash_or_account const & account_or_head_a, badem::block_hash const & head_a, badem::block_hash const & end_a, count_t count_a, unsigned retry_limit_a) :
...
badem::bulk_pull_client::~bulk_pull_client ()
{
	...
	{
		badem::lock_guard<std::mutex> mutex (connection->attempt->mutex);
...
void badem::bulk_pull_client::request ()
{
	...
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		badem::unique_lock<std::mutex> lock (connection->attempt->mutex);
...
void badem::bulk_pull_client::throttled_receive_block ()
{
...
void badem::bulk_pull_client::received_type ()
{
	...
	if (auto socket_l = connection->channel->socket.lock ())
	{
		switch (type)
		{
			...
			{
				socket_l->async_read (connection->receive_buffer, badem::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
			...
			{
				socket_l->async_read (connection->receive_buffer, badem::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, badem::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, badem::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			...
			{
				socket_l->async_read (connection->receive_buffer, badem::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
...
void badem::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, badem::block_type type_a)
{
	if (!ec)
	{
		...
		if (block != nullptr && !badem::work_validate (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see badem::bootstrap_attempt::lazy_destinations_increment (...)
...
{
	badem::shared_const_buffer send_buffer (static_cast<uint8_t> (badem::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
