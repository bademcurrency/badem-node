...
BOOST_LOG_ATTRIBUTE_KEYWORD (severity, "Severity", badem::severity_level)

boost::shared_ptr<boost::log::sinks::synchronous_sink<boost::log::sinks::text_file_backend>> badem::logging::file_sink;
std::atomic_flag badem::logging::logging_already_added ATOMIC_FLAG_INIT;

void badem::logging::init (boost::filesystem::path const & application_path_a)
{
	if (!logging_already_added.test_and_set ())
	{
		...
		{
#ifdef BOOST_WINDOWS
			if (badem::event_log_reg_entry_exists () || badem::is_windows_elevated ())
			{
				...
				boost::log::sinks::event_log::custom_event_type_mapping<badem::severity_level> mapping ("Severity");
				mapping[badem::severity_level::error] = boost::log::sinks::event_log::error;
				event_sink->locked_backend ()->set_event_type_mapper (mapping);

				// Only allow messages or error or greater severity to the event log
				event_sink->set_filter (severity >= badem::severity_level::error);
			...
			// Currently only mapping sys log errors
			boost::log::sinks::syslog::custom_severity_mapping<badem::severity_level> mapping ("Severity");
			mapping[badem::severity_level::error] = boost::log::sinks::syslog::error;
			sys_sink->locked_backend ()->set_severity_mapper (mapping);

			// Only allow messages or error or greater severity to the sys log
			sys_sink->set_filter (severity >= badem::severity_level::error);
...
void badem::logging::release_file_sink ()
{
	if (logging_already_added.test_and_set ())
	{
		boost::log::core::get ()->remove_sink (badem::logging::file_sink);
		badem::logging::file_sink.reset ();
	}
}

badem::error badem::logging::serialize_toml (badem::tomlconfig & toml) const
{
...
badem::error badem::logging::deserialize_toml (badem::tomlconfig & toml)
{
...
badem::error badem::logging::serialize_json (badem::jsonconfig & json) const
{
...
bool badem::logging::upgrade_json (unsigned version_a, badem::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
...
badem::error badem::logging::deserialize_json (bool & upgraded_a, badem::jsonconfig & json)
{
	int version_l{ 1 };
...
bool badem::logging::ledger_logging () const
{
	return ledger_logging_value;
}

bool badem::logging::ledger_duplicate_logging () const
{
	return ledger_logging () && ledger_duplicate_logging_value;
}

bool badem::logging::vote_logging () const
{
	return vote_logging_value;
}

bool badem::logging::network_logging () const
{
	return network_logging_value;
}

bool badem::logging::network_timeout_logging () const
{
	return network_logging () && network_timeout_logging_value;
}

bool badem::logging::network_message_logging () const
{
	return network_logging () && network_message_logging_value;
}

bool badem::logging::network_publish_logging () const
{
	return network_logging () && network_publish_logging_value;
}

bool badem::logging::network_packet_logging () const
{
	return network_logging () && network_packet_logging_value;
}

bool badem::logging::network_keepalive_logging () const
{
	return network_logging () && network_keepalive_logging_value;
}

bool badem::logging::network_node_id_handshake_logging () const
{
	return network_logging () && network_node_id_handshake_logging_value;
}

bool badem::logging::node_lifetime_tracing () const
{
	return node_lifetime_tracing_value;
}

bool badem::logging::insufficient_work_logging () const
{
	return network_logging () && insufficient_work_logging_value;
}

bool badem::logging::log_ipc () const
{
	return network_logging () && log_ipc_value;
}

bool badem::logging::bulk_pull_logging () const
{
	return network_logging () && bulk_pull_logging_value;
}

bool badem::logging::callback_logging () const
{
	return network_logging ();
}

bool badem::logging::work_generation_time () const
{
	return work_generation_time_value;
}

bool badem::logging::upnp_details_logging () const
{
	return upnp_details_logging_value;
}

bool badem::logging::timing_logging () const
{
	return timing_logging_value;
}

bool badem::logging::active_update_logging () const
{
	return active_update_value;
}

bool badem::logging::log_to_cerr () const
{
	return log_to_cerr_value;
}

bool badem::logging::single_line_record () const
{
