...
namespace
{
void construct_json (badem::seq_con_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void(badem::json_handler *)>>;
...
bool block_confirmed (badem::node & node, badem::transaction & transaction, badem::block_hash const & hash, bool include_active, bool include_only_confirmed);
const char * epoch_as_string (badem::epoch);
}

badem::json_handler::json_handler (badem::node & node_a, badem::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void(std::string const &)> const & response_a, std::function<void()> stop_callback_a) :
body (body_a),
node (node_a),
response (response_a),
...
void badem::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kbadem_from_raw" || action == "kbdm_from_raw")
			{
				mbadem_from_raw (badem::kbdm_ratio);
			}
			else if (action == "kbadem_to_raw" || action == "kbdm_to_raw")
			{
				mbadem_to_raw (badem::kbdm_ratio);
			}
			else if (action == "badem_from_raw" || action == "bdm_from_raw")
			{
				mbadem_from_raw (badem::bdm_ratio);
			}
			else if (action == "badem_to_raw" || action == "bdm_to_raw")
			{
				mbadem_to_raw (badem::bdm_ratio);
			}
			else if (action == "mbadem_from_raw" || action == "mbdm_from_raw")
			{
				mbadem_from_raw ();
			}
			else if (action == "mbadem_to_raw" || action == "mbdm_to_raw")
			{
				mbadem_to_raw ();
			}
...
void badem::json_handler::response_errors ()
{
...
std::shared_ptr<badem::wallet> badem::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = badem::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = badem::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool badem::json_handler::wallet_locked_impl (badem::transaction const & transaction_a, std::shared_ptr<badem::wallet> wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = badem::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool badem::json_handler::wallet_account_impl (badem::transaction const & transaction_a, std::shared_ptr<badem::wallet> wallet_a, badem::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = badem::error_common::account_not_found_wallet;
		}
	}
	return result;
}

badem::account badem::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
...
badem::account_info badem::json_handler::account_info_impl (badem::transaction const & transaction_a, badem::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = badem::error_common::account_not_found;
...
badem::amount badem::json_handler::amount_impl ()
{
	badem::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = badem::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<badem::block> badem::json_handler::block_impl (bool signature_work_required)
{
	std::shared_ptr<badem::block> result{ nullptr };
	if (!ec)
	{
		...
		{
			ec = badem::error_blocks::invalid_block;
		}
		if (!ec)
		{
			...
			result = badem::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = badem::error_blocks::invalid_block;
			}
...
std::shared_ptr<badem::block> badem::json_handler::block_json_impl (bool signature_work_required)
{
...
badem::block_hash badem::json_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = badem::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

badem::amount badem::json_handler::threshold_optional_impl ()
{
	badem::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = badem::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t badem::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (badem::from_string_hex (work_text.get (), result))
		{
			ec = badem::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t badem::json_handler::difficulty_optional_impl ()
{
	...
	{
		if (badem::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = badem::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

double badem::json_handler::multiplier_optional_impl (uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = badem::difficulty::from_multiplier (multiplier, node.network_params.network.publish_threshold);
		}
		else
		{
			ec = badem::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}
...
uint64_t badem::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = badem::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t badem::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t badem::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = badem::error_rpc::invalid_offset;
		}
	}
	return result;
}

void badem::json_handler::account_balance ()
{
...
void badem::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::account_create ()
{
	...
	node.worker.push_task ([rpc_l]() {
		...
		if (!rpc_l->ec)
		{
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = badem::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = badem::error_common::wallet_locked;
				}
			}
		}
...
void badem::json_handler::account_get ()
{
	...
	{
		ec = badem::error_common::bad_public_key;
	}
	response_errors ();
}

void badem::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			std::string balance;
			badem::uint128_union (info.balance).encode_dec (balance);
...
void badem::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", badem::account (i->first).to_account ());
...
void badem::json_handler::account_move ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<badem::public_key> accounts;
				...
				{
					rpc_l->ec = badem::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = badem::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	});
...
void badem::json_handler::account_remove ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
...
void badem::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::account_representative_set ()
{
	auto rpc_l (shared_from_this ());
	// clang-format off
	node.worker.push_task ([ rpc_l, work_generation_enabled = node.work_generation_enabled () ]() {
		...
		auto representative (rpc_l->account_impl (representative_text, badem::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						if (badem::work_validate (info.head, work))
						{
							rpc_l->ec = badem::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = badem::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				// clang-format off
				wallet->change_async(account, representative, [response_a, response_data](std::shared_ptr<badem::block> block) {
					if (block != nullptr)
					{
...
void badem::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::accounts_balances ()
{
...
void badem::json_handler::accounts_create ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				badem::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void badem::json_handler::accounts_frontiers ()
{
...
void badem::json_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, badem::pending_key (account, 0))); badem::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<badem::uint128_t> ("amount") > child2.second.template get<badem::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<badem::uint128_t> ("") > child2.second.template get<badem::uint128_t> ("");
					});
				}
			}
...
void badem::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	response_l.put ("network_minimum", badem::to_string_hex (node.network_params.network.publish_threshold));
	auto difficulty_active = node.active.active_difficulty ();
	response_l.put ("network_current", badem::to_string_hex (difficulty_active));
	auto multiplier = badem::difficulty::to_multiplier (difficulty_active, node.network_params.network.publish_threshold);
	response_l.put ("multiplier", badem::to_string (multiplier));
	if (include_trend)
	{
		...
		{
			boost::property_tree::ptree entry;
			entry.put ("", badem::to_string (multiplier_l));
...
void badem::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (badem::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (badem::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (badem::account (0))).second); // Burning 0 account
...
void state_subtype (badem::transaction const & transaction_a, badem::node & node_a, std::shared_ptr<badem::block> block_a, badem::uint128_t const & balance_a, boost::property_tree::ptree & tree_a)
{
	// Subtype check
...
void badem::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			badem::account account (block->account ().is_zero () ? sideband.account : block->account ());
		...
		{
			ec = badem::error_blocks::not_found;
		}
	}
	response_errors ();
}

void badem::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				badem::election_status status{ block_l, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, badem::election_status_type::active_confirmation_height };
				{
					badem::lock_guard<std::mutex> lock (node.active.mutex);
				...
				if (auto state = dynamic_cast<badem::state_block *> (block_l.get ()))
				{
...
void badem::json_handler::blocks ()
{
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = badem::error_blocks::bad_hash_number;
			}
		}
	}
...
void badem::json_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	const bool json_block_l = request.get<bool> ("json_block", false);
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, badem::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						badem::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void badem::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void badem::json_handler::block_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
void badem::json_handler::block_count_type ()
{
	auto transaction (node.store.tx_begin_read ());
	badem::block_counts count (node.store.block_count (transaction));
...
void badem::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	badem::wallet_id wallet (0);
	...
	{
		representative = account_impl (representative_text.get (), badem::error_rpc::bad_representative_number);
	}
	...
	{
		destination = account_impl (destination_text.get (), badem::error_rpc::bad_destination);
	}
	badem::block_hash source (0);
	...
	{
		ec = badem::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = badem::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.is_initialized ())
	{
		...
		{
			ec = badem::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = badem::error_rpc::invalid_balance;
		}
	}
	badem::link link (0);
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this](badem::block const & block_a) {
		...
		auto get_callback_l = [rpc_l, this, block_response_put_l](std::shared_ptr<badem::block> block_a) {
			...
			return [block_a, rpc_l, this, block_response_put_l](boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = badem::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = badem::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv.data != 0)
		{
			badem::account pub (badem::pub_key (prv.as_private_key ()));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = badem::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = badem::error_rpc::block_create_public_key_mismatch;
				}
			}
			badem::block_builder builder_l;
			std::shared_ptr<badem::block> block_l{ nullptr };
			badem::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = badem::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = badem::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = badem::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = badem::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = badem::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = badem::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = badem::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == badem::error_common::missing_work))
			{
				if (work == 0)
				{
					node.work_generate (root_l, get_callback_l (block_l), badem::account (pub));
				}
				else
				{
		...
		{
			ec = badem::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void badem::json_handler::block_hash ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
...
void badem::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!badem::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				node.bootstrap_initiator.bootstrap (badem::endpoint (address, port), true, bypass_frontier_confirmation);
				response_l.put ("success", "");
			}
			else
			{
				ec = badem::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = badem::error_common::invalid_port;
		}
	}
	else
	{
		ec = badem::error_common::invalid_ip_address;
	}
	response_errors ();
}

void badem::json_handler::bootstrap_any ()
{
...
void badem::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	const bool force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = badem::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void badem::json_handler::bootstrap_status ()
{
	auto attempt (node.bootstrap_initiator.current_attempt ());
	if (attempt != nullptr)
	{
		badem::lock_guard<std::mutex> lock (attempt->mutex);
		badem::lock_guard<std::mutex> lazy_lock (attempt->lazy_mutex);
		...
		else if (attempt->mode == badem::bootstrap_mode::wallet_lazy)
		{
...
void badem::json_handler::chain (bool successors)
{
...
void badem::json_handler::confirmation_active ()
{
...
void badem::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = badem::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void badem::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void badem::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (conflict_info != node.active.roots.end ())
		{
			...
			for (auto i (tally_l.begin ()), n (tally_l.end ()); i != n; ++i)
			{
				...
				if (representatives)
				{
					std::multimap<badem::uint128_t, badem::account, std::greater<badem::uint128_t>> representatives;
					for (auto ii (election->last_votes.begin ()), nn (election->last_votes.end ()); ii != nn; ++ii)
					{
						if (i->second->hash () == ii->second.hash)
						{
							badem::account const & representative (ii->first);
		...
		{
			ec = badem::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = badem::error_rpc::invalid_root;
	}
	response_errors ();
}

void badem::json_handler::confirmation_quorum ()
{
...
void badem::json_handler::database_txn_tracker ()
{
	...
	{
		ec = badem::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void badem::json_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void badem::json_handler::deterministic_key ()
{
	...
	if (!seed.data.decode_hex (seed_text))
	{
		try
		{
			...
			badem::private_key prv = badem::deterministic_key (seed, index);
		...
		{
			ec = badem::error_common::invalid_index;
		}
	}
	else
	{
		ec = badem::error_common::bad_seed;
	}
	response_errors ();
}

void epoch_upgrader (std::shared_ptr<badem::node> node_a, badem::private_key const & prv_a, badem::epoch epoch_a, uint64_t count_limit)
{
	...
	raw_key.data = prv_a;
	auto signer (badem::pub_key (prv_a));
	...
	boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>, boost::multi_index::member<account_upgrade_item, badem::account, &account_upgrade_item::account>>>>
	accounts_list;
	...
	while (!finished_upgrade && !node_a->stopped)
	{
		...
		{
			{
				...
				for (auto i (node_a->store.latest_begin (transaction)), n (node_a->store.latest_end ()); i != n; ++i)
				{
					...
					{
						release_assert (badem::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && upgraded_accounts < upgrade_batch_size && upgraded_accounts < count_limit && !node_a->stopped; ++i)
			{
				...
				if (!node_a->store.account_get (transaction, i->account, info) && info.epoch () < epoch_a)
				{
					auto epoch = builder.state ()
					             ...
					             .build ();
					bool valid_signature (!badem::validate_message (signer, epoch->hash (), epoch->block_signature ()));
					bool valid_work (!badem::work_validate (*epoch.get ()));
					badem::process_result result (badem::process_result::old);
					if (valid_signature && valid_work)
					{
						result = node_a->process_local (std::move (epoch)).code;
					}
					if (result == badem::process_result::progress)
					{
						++upgraded_accounts;
					}
					else
					{
						bool fork (result == badem::process_result::fork);
		...
		while (!finished_pending && count_limit != 0 && !node_a->stopped)
		{
			...
			auto transaction (node_a->store.tx_begin_read ());
			for (auto i (node_a->store.pending_begin (transaction, badem::pending_key (1, 0))), n (node_a->store.pending_end ()); i != n && upgraded_pending < upgrade_batch_size && upgraded_pending < count_limit && !node_a->stopped;)
			{
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<badem::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = node_a->store.pending_begin (transaction, badem::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void badem::json_handler::epoch_upgrade ()
{
	badem::epoch epoch (badem::epoch::invalid);
	...
	switch (epoch_int)
	{
		case 1:
			...
			break;
		case 2:
			epoch = badem::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != badem::epoch::invalid)
	{
		...
		{
			if (badem::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = badem::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = badem::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void badem::json_handler::frontiers ()
{
...
void badem::json_handler::account_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
namespace
{
class history_visitor : public badem::block_visitor
{
public:
	history_visitor (badem::json_handler & handler_a, bool raw_a, badem::transaction & transaction_a, boost::property_tree::ptree & tree_a, badem::block_hash const & hash_a, std::vector<badem::public_key> const & accounts_filter_a) :
	handler (handler_a),
	raw (raw_a),
	...
	bool should_ignore_account (badem::public_key const & account)
	{
		bool ignore (false);
	...
	badem::json_handler & handler;
	bool raw;
	badem::transaction & transaction;
	boost::property_tree::ptree & tree;
	badem::block_hash const & hash;
	badem::network_params network_params;
	std::vector<badem::public_key> const & accounts_filter;
};
}

void badem::json_handler::account_history ()
{
	std::vector<badem::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", badem::to_string_hex (block->block_work ()));
...
void badem::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void badem::json_handler::key_create ()
{
	badem::keypair pair;
...
void badem::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	badem::private_key prv;
	if (!prv.decode_hex (key_text))
...
void badem::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		badem::account start (0);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = badem::error_rpc::invalid_timestamp;
			}
		}
		const bool sorting = request.get<bool> ("sorting", false);
		const bool representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<badem::uint128_union, badem::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				badem::account_info const & info (i->second);
				badem::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					badem::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (pending)
					{
						auto account_pending (node.ledger.account_pending (transaction, account));
...
void badem::json_handler::mbadem_from_raw (badem::uint128_t ratio)
{
...
void badem::json_handler::mbadem_to_raw (badem::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = badem::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void badem::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void badem::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void badem::json_handler::password_change ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
...
void badem::json_handler::password_enter ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
...
void badem::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::peers ()
{
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == badem::transport::transport_type::tcp ? "tcp" : "udp");
...
void badem::json_handler::pending ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
...
void badem::json_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	const bool include_only_confirmed = request.get<bool> ("include_only_confirmed", false);
...
void badem::json_handler::payment_begin ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		...
		if (!id.decode_hex (id_text))
		{
			...
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				std::shared_ptr<badem::wallet> wallet (existing->second);
				if (wallet->store.valid_password (transaction))
				{
					...
					{
						rpc_l->ec = badem::error_rpc::payment_unable_create_account;
					}
				}
				else
				{
			...
			{
				rpc_l->ec = badem::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = badem::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	});
}

void badem::json_handler::payment_init ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
...
void badem::json_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = badem::error_rpc::payment_account_balance;
			}
		}
	}
	response_errors ();
...
void badem::json_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<badem::json_payment_observer> (node, response, account, amount));
		...
		{
			ec = badem::error_rpc::bad_timeout;
		}
	}
	if (ec)
	{
...
void badem::json_handler::process ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		...
		if (!rpc_l->ec && block->type () == badem::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<badem::state_block> block_state (std::static_pointer_cast<badem::state_block> (block));
				...
				{
					rpc_l->ec = badem::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = badem::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = badem::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = badem::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = badem::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!badem::work_validate (*block))
			{
				...
				switch (result.code)
				{
					...
					case badem::process_result::fork:
					{
						...
						{
							rpc_l->ec = badem::error_process::fork;
						}
						break;
					}
					default:
					{
						rpc_l->ec = badem::error_process::other;
						break;
					}
				}
			}
			...
			{
				rpc_l->ec = badem::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	});
...
void badem::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (block != nullptr)
			{
				if (node.store.pending_exists (block_transaction, badem::pending_key (account, hash)))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						...
						if (badem::work_validate (head, work))
						{
							ec = badem::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						// clang-format off
						wallet->receive_async(std::move(block), account, node.network_params.ledger.genesis_amount, [response_a](std::shared_ptr<badem::block> block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = badem::error_process::unreceivable;
				}
			}
			else
			{
...
void badem::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void badem::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void badem::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<badem::account, badem::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<badem::uint128_t, std::string>> representation;
...
void badem::json_handler::representatives_online ()
{
...
void badem::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = badem::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = badem::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<badem::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					badem::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, badem::pending_key (destination, hash)))
						{
							badem::block_hash previous (node.ledger.latest (transaction, destination));
...
void badem::json_handler::search_pending ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::search_pending_all ()
{
	if (!ec)
	{
...
void badem::json_handler::send ()
{
	...
	auto source (account_impl (source_text, badem::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, badem::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		...
		if (!ec)
		{
			...
			// clang-format off
			wallet->send_async(source, destination, amount.number(), [balance, amount, response_a, response_data](std::shared_ptr<badem::block> block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec(badem::error_common::insufficient_balance);
...
void badem::json_handler::sign ()
{
	...
	{
		ec = badem::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		badem::raw_key prv;
		...
		if (prv.data != 0)
		{
			...
			badem::signature signature (badem::sign_message (prv, pub, hash));
...
void badem::json_handler::stats ()
{
	...
	{
		ec = badem::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void badem::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void badem::json_handler::unchecked ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
...
void badem::json_handler::unchecked_clear ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
...
void badem::json_handler::unchecked_get ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
...
void badem::json_handler::unchecked_keys ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	badem::block_hash key (0);
	...
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, badem::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			badem::unchecked_info const & info (i->second);
...
void badem::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	badem::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending_begin (transaction, badem::pending_key (start, 0)));
		auto end (node.store.pending_end ());
		badem::account current_account (start);
		badem::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			badem::pending_key key (iterator->first);
			badem::account account (key.account);
			badem::pending_info info (iterator->second);
			if (node.store.account_exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<badem::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending_begin (transaction, badem::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void badem::json_handler::wallet_add ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = badem::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	});
...
void badem::json_handler::wallet_add_watch ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = badem::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void badem::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			badem::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == badem::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == badem::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void badem::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			badem::account const & account (i->first);
			badem::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				badem::uint128_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void badem::json_handler::wallet_change_seed ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.data.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					badem::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = badem::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	});
...
void badem::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void badem::json_handler::wallet_create ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		badem::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = badem::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				badem::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void badem::json_handler::wallet_destroy ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
...
void badem::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account_get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<badem::public_key> no_filter;
...
void badem::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void badem::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			badem::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, badem::pending_key (account, 0))); badem::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				badem::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						badem::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void badem::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_representative_set ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					// clang-format off
					wallet->change_async(account, representative, [](std::shared_ptr<badem::block>) {}, 0, false);
					// clang-format on
				}
			}
		}
...
void badem::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void badem::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void badem::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), badem::to_string_hex (work));
		}
...
void badem::json_handler::work_generate ()
{
	boost::optional<badem::account> account;
	...
	if (!ec)
	{
		...
		{
			ec = badem::error_rpc::difficulty_limit;
		}
		if (!ec)
		{
			...
			auto callback = [rpc_l, hash, this](boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", badem::to_string_hex (work));
					std::stringstream ostream;
					uint64_t result_difficulty;
					badem::work_validate (hash, work, &result_difficulty);
					response_l.put ("difficulty", badem::to_string_hex (result_difficulty));
					auto result_multiplier = badem::difficulty::to_multiplier (result_difficulty, this->node.network_params.network.publish_threshold);
					response_l.put ("multiplier", badem::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				...
				{
					ec = badem::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void badem::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void badem::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void badem::json_handler::work_set ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l]() {
		auto wallet (rpc_l->wallet_impl ());
...
void badem::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	auto difficulty (difficulty_optional_impl ());
	...
	if (!ec)
	{
		...
		response_l.put ("difficulty", badem::to_string_hex (result_difficulty));
		auto result_multiplier = badem::difficulty::to_multiplier (result_difficulty, node.network_params.network.publish_threshold);
		response_l.put ("multiplier", badem::to_string (result_multiplier));
	}
	response_errors ();
}

void badem::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void badem::json_handler::work_peers ()
{
...
void badem::json_handler::work_peers_clear ()
{
...
void construct_json (badem::seq_con_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<badem::seq_con_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<badem::seq_con_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &badem::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &badem::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &badem::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &badem::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &badem::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &badem::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &badem::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &badem::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &badem::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &badem::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &badem::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &badem::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &badem::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &badem::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &badem::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_create", &badem::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &badem::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &badem::json_handler::accounts_pending);
	no_arg_funcs.emplace ("active_difficulty", &badem::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &badem::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &badem::json_handler::block_info);
	no_arg_funcs.emplace ("block", &badem::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &badem::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &badem::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &badem::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &badem::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &badem::json_handler::block_count);
	no_arg_funcs.emplace ("block_count_type", &badem::json_handler::block_count_type);
	no_arg_funcs.emplace ("block_create", &badem::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &badem::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &badem::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &badem::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &badem::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &badem::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &badem::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &badem::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &badem::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &badem::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &badem::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &badem::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &badem::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &badem::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &badem::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &badem::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &badem::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &badem::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &badem::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &badem::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &badem::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &badem::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &badem::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &badem::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &badem::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &badem::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &badem::json_handler::password_enter);
	no_arg_funcs.emplace ("payment_begin", &badem::json_handler::payment_begin);
	no_arg_funcs.emplace ("payment_init", &badem::json_handler::payment_init);
	no_arg_funcs.emplace ("payment_end", &badem::json_handler::payment_end);
	no_arg_funcs.emplace ("payment_wait", &badem::json_handler::payment_wait);
	no_arg_funcs.emplace ("peers", &badem::json_handler::peers);
	no_arg_funcs.emplace ("pending", &badem::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &badem::json_handler::pending_exists);
	no_arg_funcs.emplace ("process", &badem::json_handler::process);
	no_arg_funcs.emplace ("receive", &badem::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &badem::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &badem::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &badem::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &badem::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &badem::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &badem::json_handler::search_pending);
	no_arg_funcs.emplace ("search_pending_all", &badem::json_handler::search_pending_all);
	no_arg_funcs.emplace ("send", &badem::json_handler::send);
	no_arg_funcs.emplace ("sign", &badem::json_handler::sign);
	no_arg_funcs.emplace ("stats", &badem::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &badem::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &badem::json_handler::stop);
	no_arg_funcs.emplace ("unchecked", &badem::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &badem::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &badem::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &badem::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &badem::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &badem::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &badem::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &badem::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &badem::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &badem::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &badem::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &badem::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &badem::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &badem::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &badem::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &badem::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &badem::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &badem::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &badem::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &badem::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &badem::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &badem::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &badem::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &badem::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_representative", &badem::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &badem::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &badem::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &badem::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &badem::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &badem::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &badem::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &badem::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &badem::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &badem::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &badem::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &badem::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
const char * epoch_as_string (badem::epoch epoch)
{
	switch (epoch)
	{
		case badem::epoch::epoch_2:
			return "2";
