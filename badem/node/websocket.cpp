...
badem::websocket::confirmation_options::confirmation_options (badem::node & node_a) :
node (node_a)
{
}

badem::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, badem::node & node_a) :
node (node_a)
{
...
bool badem::websocket::confirmation_options::should_filter (badem::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			badem::account source_l (0), destination_l (0);
...
badem::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, badem::node & node_a) :
node (node_a)
{
...
bool badem::websocket::vote_options::should_filter (badem::websocket::message const & message_a) const
{
...
badem::websocket::session::session (badem::websocket::listener & listener_a, socket_type socket_a) :
...
void badem::websocket::session::handshake ()
{
...
void badem::websocket::session::close ()
{
...
void badem::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
badem::websocket::topic to_topic (std::string const & topic_a)
{
	badem::websocket::topic topic = badem::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = badem::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = badem::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = badem::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = badem::websocket::topic::ack;
	}
...
std::string from_topic (badem::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == badem::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == badem::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == badem::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == badem::websocket::topic::ack)
	{
		topic = "ack";
	}
...
void badem::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	...
	badem::websocket::message msg (badem::websocket::topic::ack);
...
void badem::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != badem::websocket::topic::invalid)
	{
		...
		{
			options_l = std::make_unique<badem::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_node ());
		}
		else if (options_text_l && topic_l == badem::websocket::topic::vote)
		{
			options_l = std::make_unique<badem::websocket::vote_options> (options_text_l.get (), ws_listener.get_node ());
		}
		else
		{
			options_l = std::make_unique<badem::websocket::options> ();
		}
...
void badem::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();
...
badem::websocket::listener::listener (badem::node & node_a, boost::asio::ip::tcp::endpoint endpoint_a) :
node (node_a),
...
void badem::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void badem::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void badem::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		auto session (std::make_shared<badem::websocket::session> (*this, std::move (socket)));
		sessions_mutex.lock ();
...
void badem::websocket::listener::broadcast_confirmation (std::shared_ptr<badem::block> block_a, badem::account const & account_a, badem::amount const & amount_a, std::string subtype, badem::election_status const & election_status_a)
{
	...
	for (auto & weak_session : sessions)
	{
		...
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (badem::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				badem::websocket::confirmation_options default_options (node);
				auto conf_options (dynamic_cast<badem::websocket::confirmation_options *> (subscription->second.get ()));
...
void badem::websocket::listener::increase_subscriber_count (badem::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void badem::websocket::listener::decrease_subscriber_count (badem::websocket::topic const & topic_a)
{
...
badem::websocket::message badem::websocket::message_builder::stopped_election (badem::block_hash const & hash_a)
{
	badem::websocket::message message_l (badem::websocket::topic::stopped_election);
...
badem::websocket::message badem::websocket::message_builder::block_confirmed (std::shared_ptr<badem::block> block_a, badem::account const & account_a, badem::amount const & amount_a, std::string subtype, bool include_block_a, badem::election_status const & election_status_a, badem::websocket::confirmation_options const & options_a)
{
	badem::websocket::message message_l (badem::websocket::topic::confirmation);
	set_common_fields (message_l);
...
badem::websocket::message badem::websocket::message_builder::vote_received (std::shared_ptr<badem::vote> vote_a)
{
	badem::websocket::message message_l (badem::websocket::topic::vote);
	set_common_fields (message_l);
