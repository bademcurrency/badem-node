...
badem::signature_checker::signature_checker (unsigned num_threads) :
...
badem::signature_checker::~signature_checker ()
{
	stop ();
}

void badem::signature_checker::verify (badem::signature_check_set & check_a)
{
	{
...
bool badem::signature_checker::verify_batch (const badem::signature_check_set & check_a, size_t start_index, size_t size)
{
	/* Returns false if there are at least 1 invalid signature */
	auto code (badem::validate_message_batch (check_a.messages + start_index, check_a.message_lengths + start_index, check_a.pub_keys + start_index, check_a.signatures + start_index, size, check_a.verifications + start_index));
	(void)code;
 ...
 */
void badem::signature_checker::verify_async (badem::signature_check_set & check_a, size_t num_batches, std::promise<void> & promise)
{
...
void badem::signature_checker::set_thread_names (unsigned num_threads)
{
	auto ready = false;
