...
{
public:
	explicit matches_txn (const badem::transaction_impl * transaction_impl_a) :
	transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (badem::mdb_txn_stats const & mdb_txn_stats)
	{
...
badem::read_mdb_txn::read_mdb_txn (badem::mdb_env const & environment_a, badem::mdb_txn_callbacks txn_callbacks_a) :
...
badem::read_mdb_txn::~read_mdb_txn ()
{
...
void badem::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void badem::read_mdb_txn::renew ()
{
...
void * badem::read_mdb_txn::get_handle () const
{
	return handle;
}

badem::write_mdb_txn::write_mdb_txn (badem::mdb_env const & environment_a, badem::mdb_txn_callbacks txn_callbacks_a) :
env (environment_a),
txn_callbacks (txn_callbacks_a)
{
	renew ();
...
badem::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void badem::write_mdb_txn::commit () const
{
	auto status (mdb_txn_commit (handle));
	release_assert (status == MDB_SUCCESS);
	txn_callbacks.txn_end (this);
...
void badem::write_mdb_txn::renew ()
{
...
void * badem::write_mdb_txn::get_handle () const
{
	return handle;
}
...
void badem::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			badem::jsonconfig mdb_lock_config;
			...
			badem::jsonconfig stack (stacktrace_config);
...
void badem::mdb_txn_tracker::output_finished (badem::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == badem::thread_role::get_string (badem::thread_role::name::block_processing);
...
badem::mdb_txn_stats::mdb_txn_stats (const badem::transaction_impl * transaction_impl) :
transaction_impl (transaction_impl),
thread_name (badem::thread_role::get_string ()),
stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool badem::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<const badem::write_transaction_impl *> (transaction_impl) != nullptr);
}
