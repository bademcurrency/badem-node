...
{
public:
	mdb_iterator (badem::transaction const & transaction_a, MDB_dbi db_a, badem::epoch epoch_a = badem::epoch::unspecified) :
	cursor (nullptr)
	{
	...
	mdb_iterator (std::nullptr_t, badem::epoch epoch_a = badem::epoch::unspecified) :
	cursor (nullptr)
	{
		current.first.epoch = epoch_a;
		current.second.epoch = epoch_a;
	...
	mdb_iterator (badem::transaction const & transaction_a, MDB_dbi db_a, MDB_val const & val_a, badem::epoch epoch_a = badem::epoch::unspecified) :
	cursor (nullptr)
	{
		current.first.epoch = epoch_a;
		current.second.epoch = epoch_a;
	...
	void clear ()
	{
		current.first = badem::db_val<MDB_val> (current.first.epoch);
		current.second = badem::db_val<MDB_val> (current.second.epoch);
