...
badem::error badem::ipc::ipc_config::serialize_toml (badem::tomlconfig & toml) const
{
	badem::tomlconfig tcp_l;
...
badem::error badem::ipc::ipc_config::deserialize_toml (badem::tomlconfig & toml)
{
...
badem::error badem::ipc::ipc_config::serialize_json (badem::jsonconfig & json) const
{
	badem::jsonconfig tcp_l;
	// Only write out experimental config values if they're previously set explicitly in the config file
	if (transport_tcp.io_threads >= 0)
	{
...
badem::error badem::ipc::ipc_config::deserialize_json (bool & upgraded_a, badem::jsonconfig & json)
{
