...
badem::rocksdb_store::rocksdb_store (badem::logger_mt & logger_a, boost::filesystem::path const & path_a, badem::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
logger (logger_a),
rocksdb_config (rocksdb_config_a)
{
	boost::system::error_code error_mkdir, error_chmod;
	boost::filesystem::create_directories (path_a, error_mkdir);
	badem::set_secure_perm_directory (path_a, error_chmod);
...
badem::rocksdb_store::~rocksdb_store ()
{
...
void badem::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
badem::write_transaction badem::rocksdb_store::tx_begin_write (std::vector<badem::tables> const & tables_requiring_locks_a, std::vector<badem::tables> const & tables_no_locks_a)
{
	std::unique_ptr<badem::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<badem::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<badem::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return badem::write_transaction{ std::move (txn) };
}

badem::read_transaction badem::rocksdb_store::tx_begin_read ()
{
	return badem::read_transaction{ std::make_unique<badem::read_rocksdb_txn> (db) };
}

rocksdb::ColumnFamilyHandle * badem::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool badem::rocksdb_store::exists (badem::transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a) const
{
...
int badem::rocksdb_store::del (badem::write_transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a)
{
...
bool badem::rocksdb_store::block_info_get (badem::transaction const &, badem::block_hash const &, badem::block_info &) const
{
...
void badem::rocksdb_store::version_put (badem::write_transaction const & transaction_a, int version_a)
{
	assert (transaction_a.contains (tables::meta));
	badem::uint256_union version_key (1);
	badem::uint256_union version_value (version_a);
	auto status (put (transaction_a, tables::meta, version_key, badem::rocksdb_val (version_value)));
	release_assert (success (status));
}

rocksdb::Transaction * badem::rocksdb_store::tx (badem::transaction const & transaction_a) const
{
	assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int badem::rocksdb_store::get (badem::transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a, badem::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
bool badem::rocksdb_store::is_caching_counts (badem::tables table_a) const
{
	switch (table_a)
	{
		case tables::accounts:
...
int badem::rocksdb_store::increment (badem::write_transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	uint64_t base;
	badem::rocksdb_val value;
	...
	return put (transaction_a, table_a, key_a, badem::rocksdb_val (base + amount_a));
}

int badem::rocksdb_store::decrement (badem::write_transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	badem::rocksdb_val value;
...
int badem::rocksdb_store::put (badem::write_transaction const & transaction_a, tables table_a, badem::rocksdb_val const & key_a, badem::rocksdb_val const & value_a)
{
	assert (transaction_a.contains (table_a));
...
bool badem::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool badem::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int badem::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t badem::rocksdb_store::count (badem::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle) const
{
	uint64_t count = 0;
	badem::rocksdb_val val;
	auto const & key = handle->GetName ();
	auto status = get (transaction_a, tables::cached_counts, badem::rocksdb_val (key.size (), (void *)key.data ()), val);
	if (success (status))
	{
...
size_t badem::rocksdb_store::count (badem::transaction const & transaction_a, tables table_a) const
{
	size_t sum = 0;
...
int badem::rocksdb_store::drop (badem::write_transaction const & transaction_a, tables table_a)
{
	assert (transaction_a.contains (table_a));
	...
	{
		// Reset counter to 0
		status = put (transaction_a, tables::cached_counts, badem::rocksdb_val (rocksdb::Slice (col->GetName ())), badem::rocksdb_val (uint64_t{ 0 }));
	}
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peers_begin (transaction_a), n = peers_end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, badem::rocksdb_val (i->first));
				release_assert (success (status));
			}
			return status;
		}
...
int badem::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
void badem::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options badem::rocksdb_store::get_db_options () const
{
...
rocksdb::BlockBasedTableOptions badem::rocksdb_store::get_table_options () const
{
...
rocksdb::ColumnFamilyOptions badem::rocksdb_store::get_cf_options () const
{
...
std::vector<badem::tables> badem::rocksdb_store::all_tables () const
{
	return std::vector<badem::tables>{ tables::accounts, tables::cached_counts, tables::change_blocks, tables::confirmation_height, tables::frontiers, tables::meta, tables::online_weight, tables::open_blocks, tables::peers, tables::pending, tables::receive_blocks, tables::representation, tables::send_blocks, tables::state_blocks, tables::unchecked, tables::vote };
}

bool badem::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		badem::rocksdb_store rocksdb_store (logger, destination_path.string (), rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

bool badem::rocksdb_store::init_error () const
{
	return error;
}