...
badem::json_payment_observer::json_payment_observer (badem::node & node_a, std::function<void(std::string const &)> const & response_a, badem::account const & account_a, badem::amount const & amount_a) :
node (node_a),
account (account_a),
amount (amount_a),
response (response_a)
...
void badem::json_payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
	node.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout), [this_l]() {
		this_l->complete (badem::payment_status::nothing);
	});
}

void badem::json_payment_observer::observe ()
{
	if (node.balance (account) >= amount.number ())
	{
		complete (badem::payment_status::success);
	}
}

void badem::json_payment_observer::complete (badem::payment_status status)
{
	...
	if (!already)
	{
		...
		switch (status)
		{
			case badem::payment_status::nothing:
			{
			...
			case badem::payment_status::success:
			{
				boost::property_tree::ptree response_l;
				response_l.put ("deprecated", "1");
