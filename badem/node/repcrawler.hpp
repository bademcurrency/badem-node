...
{
public:
	representative (badem::account account_a, badem::amount weight_a, std::shared_ptr<badem::transport::channel> channel_a) :
	account (account_a), weight (weight_a), channel (channel_a)
	{
	}
	std::reference_wrapper<badem::transport::channel const> channel_ref () const
	{
		return *channel;
	};
	badem::account account{ 0 };
	badem::amount weight{ 0 };
...
class rep_crawler
{
	...
	mi::indexed_by<
		mi::hashed_unique<mi::member<representative, badem::account, &representative::account>>,
		mi::random_access<>,
		...
		mi::ordered_non_unique<mi::tag<tag_weight>,
			mi::member<representative, badem::amount, &representative::weight>, std::greater<badem::amount>>,
		mi::hashed_non_unique<mi::tag<tag_channel_ref>,
			mi::const_mem_fun<representative, std::reference_wrapper<badem::transport::channel const>, &representative::channel_ref>>>>;
	// clang-format on

public:
	rep_crawler (badem::node & node_a);
	...
	void query (std::vector<std::shared_ptr<badem::transport::channel>> const & channels_a);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::shared_ptr<badem::transport::channel> channel_a);
	 ...
	 */
	bool response (std::shared_ptr<badem::transport::channel> channel_a, badem::account const & rep_account_a, badem::amount const & weight_a);

	/** Get total available weight from representatives */
	badem::uint128_t total_weight () const;
	...
	std::vector<std::shared_ptr<badem::transport::channel>> representative_endpoints (size_t count_a);

	/** Returns all representatives registered with weight in descending order */
	std::vector<badem::representative> representatives_by_weight ();
...
private:
	...
	std::vector<std::shared_ptr<badem::transport::channel>> get_crawl_targets (badem::uint128_t total_weight_a);

	/** When a rep request is made, this is called to update the last-request timestamp. */
	void on_rep_request (std::shared_ptr<badem::transport::channel> channel_a);
