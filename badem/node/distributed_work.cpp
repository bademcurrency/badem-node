...
std::shared_ptr<request_type> badem::work_peer_request::get_prepared_json_request (std::string const & request_string_a) const
{
...
badem::distributed_work::distributed_work (badem::node & node_a, badem::root const & root_a, std::vector<std::pair<std::string, uint16_t>> const & peers_a, unsigned int backoff_a, std::function<void(boost::optional<uint64_t>)> const & callback_a, uint64_t difficulty_a, boost::optional<badem::account> const & account_a) :
callback (callback_a),
backoff (backoff_a),
node (node_a),
root (root_a),
...
elapsed (badem::timer_state::started, "distributed work generation timer")
{
	assert (!completed);
}

badem::distributed_work::~distributed_work ()
{
	if (!node.stopped && node.websocket_server && node.websocket_server->any_subscriber (badem::websocket::topic::work))
	{
...
void badem::distributed_work::start ()
{
...
void badem::distributed_work::start_work ()
{
	auto this_l (shared_from_this ());
	...
	if (!outstanding.empty ())
	{
		...
		for (auto const & i : outstanding)
		{
			...
			auto connection (std::make_shared<badem::work_peer_request> (this_l->node.io_ctx, host, service));
...
void badem::distributed_work::cancel_connection (std::shared_ptr<badem::work_peer_request> connection_a)
{
	auto this_l (shared_from_this ());
	auto cancelling_l (std::make_shared<badem::work_peer_request> (node.io_ctx, connection_a->address, connection_a->port));
	cancelling_l->socket.async_connect (badem::tcp_endpoint (cancelling_l->address, cancelling_l->port), [this_l, cancelling_l](boost::system::error_code const & ec) {
		if (!ec)
		{
			std::string request_string;
			{
...
void badem::distributed_work::success (std::string const & body_a, boost::asio::ip::address const & address_a, uint16_t port_a)
{
	...
	try
	{
		...
		if (!badem::from_string_hex (work_text, work))
		{
			...
			{
				node.logger.try_log (boost::str (boost::format ("Incorrect work response from %1%:%2% for root %3% with diffuculty %4%: %5%") % address_a % port_a % root.to_string () % badem::to_string_hex (difficulty) % work_text));
...
void badem::distributed_work::stop_once (bool const local_stop_a)
{
...
void badem::distributed_work::set_once (uint64_t work_a, std::string const & source_a)
{
	if (!cancelled && !completed.exchange (true))
	{
		...
		if (node.config.logging.work_generation_time ())
		{
			...
			auto multiplier_text_l (badem::to_string (badem::difficulty::to_multiplier (difficulty, node.network_params.network.publish_threshold), 2));
			node.logger.try_log (boost::str (unformatted_l % root.to_string () % badem::to_string_hex (difficulty) % multiplier_text_l % elapsed.value ().count ()));
		}
	}
}

void badem::distributed_work::cancel_once ()
{
...
void badem::distributed_work::failure (boost::asio::ip::address const & address_a)
{
	auto last (remove (address_a));
	handle_failure (last);
}

void badem::distributed_work::handle_failure (bool const last_a)
{
...
bool badem::distributed_work::remove (boost::asio::ip::address const & address_a)
{
	badem::lock_guard<std::mutex> guard (mutex);
...
void badem::distributed_work::add_bad_peer (boost::asio::ip::address const & address_a, uint16_t port_a)
{
	badem::lock_guard<std::mutex> guard (mutex);
	bad_peers.emplace_back (boost::str (boost::format ("%1%:%2%") % address_a % port_a));
}

badem::distributed_work_factory::distributed_work_factory (badem::node & node_a) :
node (node_a)
{
}

badem::distributed_work_factory::~distributed_work_factory ()
{
	stop ();
}

bool badem::distributed_work_factory::make (badem::root const & root_a, std::vector<std::pair<std::string, uint16_t>> const & peers_a, std::function<void(boost::optional<uint64_t>)> const & callback_a, uint64_t difficulty_a, boost::optional<badem::account> const & account_a)
{
	return make (1, root_a, peers_a, callback_a, difficulty_a, account_a);
}

bool badem::distributed_work_factory::make (unsigned int backoff_a, badem::root const & root_a, std::vector<std::pair<std::string, uint16_t>> const & peers_a, std::function<void(boost::optional<uint64_t>)> const & callback_a, uint64_t difficulty_a, boost::optional<badem::account> const & account_a)
{
	bool error_l{ true };
	if (!stopped)
	{
		...
		{
			auto distributed (std::make_shared<badem::distributed_work> (node, root_a, peers_a, backoff_a, callback_a, difficulty_a, account_a));
			{
				badem::lock_guard<std::mutex> guard (mutex);
...
void badem::distributed_work_factory::cancel (badem::root const & root_a, bool const local_stop)
{
...
void badem::distributed_work_factory::cleanup_finished ()
{
	badem::lock_guard<std::mutex> guard (mutex);
...
void badem::distributed_work_factory::stop ()
{
	if (!stopped.exchange (true))
	{
		// Cancel any ongoing work
		std::unordered_set<badem::root> roots_l;
		badem::unique_lock<std::mutex> lock_l (mutex);
...
std::unique_ptr<seq_con_info_component> collect_seq_con_info (distributed_work_factory & distributed_work, const std::string & name)
{
	size_t item_count = 0;
	{
		badem::lock_guard<std::mutex> guard (distributed_work.mutex);
