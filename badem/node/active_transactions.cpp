...
void badem::active_transactions::confirm_frontiers (badem::transaction const & transaction_a)
{
...
{
	std::unordered_set<badem::qualified_root> inactive;
	...
	std::unordered_map<std::shared_ptr<badem::transport::channel>, std::vector<std::pair<badem::block_hash, badem::block_hash>>> requests_bundle;
	std::deque<std::shared_ptr<badem::block>> rebroadcast_bundle;
	std::deque<std::pair<std::shared_ptr<badem::block>, std::shared_ptr<std::vector<std::shared_ptr<badem::transport::channel>>>>> confirm_req_bundle;
	...
	for (auto i (roots.get<1> ().begin ()), n (roots.get<1> ().end ()); i != n; ++i)
	{
		...
		else
		{
			...
			auto rep_channels (std::make_shared<std::vector<std::shared_ptr<badem::transport::channel>>> ());
			...
			std::unordered_set<std::shared_ptr<badem::transport::channel>> channels;
			for (auto & rep : reps)
			{
			...
			{
				auto single_confirm_req_channels (std::make_shared<std::vector<std::shared_ptr<badem::transport::channel>>> ());
				for (auto & rep : *rep_channels)
				{
					if (rep->get_network_version () >= badem::tcp_realtime_protocol_version_min)
					{
...
bool badem::active_transactions::add (std::shared_ptr<badem::block> block_a, std::function<void(std::shared_ptr<badem::block>)> const & confirmation_action_a)
{
	...
	if (!stopped)
	{
		...
		if (existing == roots.end () && confirmed_set.get<1> ().find (root) == confirmed_set.get<1> ().end ())
		{
			...
			auto election (badem::make_shared<badem::election> (node, block_a, confirmation_action_a));
...
void badem::active_transactions::update_active_difficulty (std::unique_lock<std::mutex> & lock_a)
{
...
void badem::active_transactions::add_confirmed (badem::election_status const & status_a, badem::qualified_root const & root_a)
{
	...
	auto inserted (confirmed_set.insert (badem::confirmed_set_info{ std::chrono::steady_clock::now (), root_a }));
...
void badem::active_transactions::flush_lowest ()
{
	size_t count (0);
	assert (!roots.empty ());
...
void badem::active_transactions::confirm_block (badem::transaction const & transaction_a, std::shared_ptr<badem::block> block_a, badem::block_sideband const & sideband_a)
{
	auto hash (block_a->hash ());
	std::unique_lock<std::mutex> lock (mutex);
	auto existing (blocks.find (hash));
	...
	else
	{
		...
		node.observers.blocks.notify (badem::election_status{ block_a, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), badem::election_status_type::inactive_confirmation_height }, account, amount, is_state_send);
	}
}
