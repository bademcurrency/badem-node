...
std::shared_ptr<badem::node> badem::system::add_node (badem::node_config const & node_config_a, badem::node_flags node_flags_a, badem::transport::transport_type type_a)
{
	...
	if (nodes.size () > 1)
	{
		...
		for (auto i (begin), j (begin + 1), n (nodes.end ()); j != n; ++i, ++j)
		{
			...
			if (type_a == badem::transport::transport_type::tcp)
			{
			...
			if (type_a == badem::transport::transport_type::tcp && node_config_a.tcp_incoming_connections_max != 0 && !node_flags_a.disable_tcp_realtime)
			{
		...
		auto iterations1 (0);
		while (std::any_of (begin, nodes.end (), [](std::shared_ptr<badem::node> const & node_a) { return node_a->bootstrap_initiator.in_progress (); }))
		{
			poll ();
			++iterations1;
...
badem::system::system (uint16_t port_a, uint16_t count_a, badem::transport::transport_type type_a) :
system ()
{
	...
	for (uint16_t i (0); i < count_a; ++i)
	{
		...
		add_node (config, badem::node_flags (), type_a);
	}
}
...
{
public:
	traffic_generator (uint32_t count_a, uint32_t wait_a, std::shared_ptr<badem::node> const & node_a, badem::system & system_a) :
	count (count_a),
	wait (wait_a),
	node (node_a),
	system (system_a)
...
badem::uint128_t badem::system::get_random_amount (badem::transaction const & transaction_a, badem::node & node_a, badem::account const & account_a)
{
	...
	return (((badem::uint256_t{ random_amount.number () } * balance) / badem::uint256_t{ std::numeric_limits<badem::uint128_t>::max () }).convert_to<badem::uint128_t> ());
}
...
void cleanup_test_directories_on_exit ()
{
	...
	badem::logging::release_file_sink ();
	// Clean up tmp directories created by the tests. Since it's sometimes useful to
	// see log files after test failures, an environment variable is supported to
	// retain the files.
	if (std::getenv ("TEST_KEEP_TMPDIRS") == nullptr)
