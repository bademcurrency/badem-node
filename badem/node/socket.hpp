...
class socket : public std::enable_shared_from_this<badem::socket>
{
...
public:
	 ...
	 */
	explicit socket (std::shared_ptr<badem::node> node, boost::optional<std::chrono::seconds> io_timeout = boost::none, concurrency = concurrency::single_writer);
	virtual ~socket ();
	...
	void async_write (badem::shared_const_buffer const &, std::function<void(boost::system::error_code const &, size_t)> = nullptr);
...
protected:
	...
	{
	public:
		badem::shared_const_buffer buffer;
		std::function<void(boost::system::error_code const &, size_t)> callback;
	};

	boost::asio::strand<boost::asio::io_context::executor_type> strand;
	boost::asio::ip::tcp::socket tcp_socket;
	std::weak_ptr<badem::node> node;
...
public:
	 ...
	 */
	explicit server_socket (std::shared_ptr<badem::node> node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a, concurrency concurrency_a = concurrency::single_writer);
	...
	void on_connection (std::function<bool(std::shared_ptr<badem::socket> new_connection, boost::system::error_code const &)>);
...
private:
	std::vector<std::weak_ptr<badem::socket>> connections;
