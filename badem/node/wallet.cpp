...
std::shared_ptr<badem::block> badem::wallet::send_action (badem::account const & source_a, badem::account const & account_a, badem::uint128_t const & amount_a, uint64_t work_a, bool generate_work_a, boost::optional<std::string> id_a)
{
	...
	std::tuple<std::shared_ptr<badem::block>, bool, bool> result;
	{
		if (id_mdb_val)
		{
...
{
	auto this_l (shared_from_this ());
	wallets.node.wallets.queue_wallet_action (badem::wallets::high_priority, this_l, [this_l, source_a, representative_a, action_a, work_a, generate_work_a](badem::wallet & wallet_a) {
...
{
	auto this_l (shared_from_this ());
	wallets.node.wallets.queue_wallet_action (amount_a, this_l, [this_l, block_a, representative_a, amount_a, action_a, work_a, generate_work_a](badem::wallet & wallet_a) {
...
{
	auto this_l (shared_from_this ());
	wallets.node.wallets.queue_wallet_action (badem::wallets::high_priority, this_l, [this_l, source_a, account_a, amount_a, action_a, work_a, generate_work_a, id_a](badem::wallet & wallet_a) {
...
badem::read_transaction badem::wallets::tx_begin_read ()
{
...
void badem::wallets::split_if_needed (badem::transaction & transaction_destination, badem::block_store & store_a)
{
	...
	if (store_l != nullptr)
	{
		if (items.empty ())
		{
			...
			// clang-format off
			auto get_store_it = [&handle = handle](badem::transaction const & transaction_source, std::string const & hash) {
				return badem::store_iterator<std::array<char, 64>, badem::no_value> (std::make_unique<badem::mdb_iterator<std::array<char, 64>, badem::no_value>> (transaction_source, handle, badem::mdb_val (hash.size (), const_cast<char *> (hash.c_str ()))));
			};
			// clang-format on
