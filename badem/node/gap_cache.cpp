...
void badem::gap_cache::add (badem::block_hash const & hash_a, std::chrono::steady_clock::time_point time_point_a)
{
	...
	for (auto hash : *vote_a)
	{
		...
		if (existing != blocks.get<1> ().end () && !existing->confirmed)
		{
			...
			if (is_new)
			{
				...
				{
					blocks.get<1> ().modify (existing, [](badem::gap_information & info) {
						info.confirmed = true;
					});
				}
			}
...
bool badem::gap_cache::bootstrap_check (std::vector<badem::account> const & voters_a, badem::block_hash const & hash_a)
{
	uint128_t tally;
...
badem::uint128_t badem::gap_cache::bootstrap_threshold ()
{
