...
private:
	struct Task final
	{
		Task (badem::signature_check_set & check, size_t pending) :
		...
		badem::signature_check_set & check;
		std::atomic<size_t> pending;
	};

	bool verify_batch (const badem::signature_check_set & check_a, size_t index, size_t size);
	void verify_async (badem::signature_check_set & check_a, size_t num_batches, std::promise<void> & promise);
