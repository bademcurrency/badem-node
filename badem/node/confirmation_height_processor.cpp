...
badem::confirmation_height_processor::confirmation_height_processor (badem::pending_confirmation_height & pending_confirmation_height_a, badem::block_store & store_a, badem::stat & stats_a, badem::active_transactions & active_a, badem::block_hash const & epoch_link_a, badem::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, badem::logger_mt & logger_a) :
...
void badem::confirmation_height_processor::add_confirmation_height (badem::block_hash const & hash_a)
{
	...
	do
	{
		...
		badem::account account (store.block_account (read_transaction, current));
		...
		while (!all_pending_a.empty ())
		{
			...
			if (pending.height > confirmation_height)
			{
				...
				if (!block)
				{
					...
					stats.inc (badem::stat::type::confirmation_height, badem::stat::detail::invalid_block);
				...
				stats.add (badem::stat::type::confirmation_height, badem::stat::detail::blocks_confirmed, badem::stat::dir::in, pending.height - confirmation_height);
...
void badem::confirmation_height_processor::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, badem::block_hash const & hash_a, badem::account const & account_a, badem::read_transaction const & transaction_a)
{
	auto hash (hash_a);
...
namespace badem
{
confirmation_height_processor::conf_height_details::conf_height_details (badem::account const & account_a, badem::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a) :
account (account_a),
hash (hash_a),
height (height_a),
