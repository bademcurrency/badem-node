...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a)
{
	...
	if (!config.read_and_update (config_a, config_path_a))
	{
		if (account != config_a.account || wallet != config_a.wallet)
		{
			...
			badem::set_secure_perm_file (config_path_a, error_chmod);
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		badem::work_pool work (config.node.work_threads, config.node.pow_sleep_interval, opencl ? [&opencl](badem::uint256_union const & root_a, uint64_t difficulty_a) {
		...
		if (!init.error ())
		{
			...
			application.postEvent (&processor, new badem_qt::eventloop_event ([&]() {
				gui = std::make_shared<badem_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
