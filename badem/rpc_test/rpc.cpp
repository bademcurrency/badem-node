...
public:
	...
	{
		sock.async_connect (badem::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port), [this](boost::system::error_code const & ec) {
			if (!ec)
			{
...
void enable_ipc_transport_tcp (badem::ipc::ipc_config_tcp_socket & transport_tcp, uint16_t ipc_port)
{
	transport_tcp.enabled = true;
	transport_tcp.port = ipc_port;
}

void enable_ipc_transport_tcp (badem::ipc::ipc_config_tcp_socket & transport_tcp)
{
...
void reset_confirmation_height (badem::block_store & store, badem::account const & account)
{
...
void check_block_response_count (badem::system & system, badem::rpc & rpc, boost::property_tree::ptree & request, uint64_t size_count)
{
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (badem::error_common::invalid_index).message (), response2.json.get<std::string> ("error"));
}
...
TEST (rpc, send)
{
	...
	ASSERT_EQ (node->latest (badem::test_genesis_key.pub), block);
	thread2.join ();
}
...
TEST (rpc, send_fail)
{
	...
	ASSERT_EQ (std::error_code (badem::error_common::account_not_found_wallet).message (), response.json.get<std::string> ("error"));
	thread2.join ();
}
...
TEST (rpc, send_idempotent)
{
	...
	ASSERT_EQ (std::error_code (badem::error_common::insufficient_balance).message (), response3.json.get<std::string> ("error"));
}
...
TEST (rpc, pending)
{
	...
	request.put ("count", "100");
	{
		...
		badem::block_hash hash (blocks_node.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		badem::block_hash hash (blocks_node.begin ()->first);
		ASSERT_EQ (block1->hash (), hash);
...
TEST (rpc, version)
{
	...
	auto genesis_open (node1->latest (badem::test_genesis_key.pub));
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		ASSERT_EQ (badem::difficulty::to_multiplier (result_difficulty, node->network_params.network.publish_threshold), multiplier);
	...
	}
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (badem::error_rpc::difficulty_limit);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", badem::to_string_hex (0xff00000000000000));
	...
	}
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (badem::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
	{
...
TEST (rpc, work_peer_many)
{
	...
	badem::ipc_rpc_processor ipc_rpc_processor2 (system2.io_ctx, config2);
	badem::rpc rpc2 (system2.io_ctx, config2, ipc_rpc_processor2);
	rpc2.start ();
	...
	badem::ipc::ipc_server ipc_server3 (node3, node_rpc_config);
	badem::ipc_rpc_processor ipc_rpc_processor3 (system3.io_ctx, config3);
	badem::rpc rpc3 (system3.io_ctx, config3, ipc_rpc_processor3);
	rpc3.start ();
	...
	badem::ipc::ipc_server ipc_server4 (node4, node_rpc_config);
	badem::ipc_rpc_processor ipc_rpc_processor4 (system4.io_ctx, config4);
	badem::rpc rpc4 (system2.io_ctx, config4, ipc_rpc_processor4);
	rpc4.start ();
...
TEST (rpc, block_count)
{
	...
	// Should not be able to get the cemented count when enable_control is false.
	{
		...
		badem::node_rpc_config node_rpc_config;
		badem::ipc::ipc_server ipc_server (node1, node_rpc_config);
		badem::rpc_config rpc_config (false);
		badem::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
		badem::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
		rpc.start ();
		boost::property_tree::ptree request1;
...
TEST (rpc, badem_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "badem_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
...
TEST (rpc, bootstrap)
{
	...
	badem::ipc::ipc_server ipc_server (*node, node_rpc_config);
	badem::rpc_config rpc_config (true);
	badem::ipc_rpc_processor ipc_rpc_processor (system0.io_ctx, rpc_config);
	badem::rpc rpc (system0.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, work_validate)
{
	...
	request.put ("work", badem::to_string_hex (work1));
	{
		...
		uint64_t difficulty;
		ASSERT_FALSE (badem::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, params.network.publish_threshold);
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, badem::difficulty::to_multiplier (difficulty, params.network.publish_threshold), 1e-6);
	}
	uint64_t work2 (0);
	...
	uint64_t result_difficulty;
	ASSERT_FALSE (badem::work_validate (hash, work1, &result_difficulty));
	ASSERT_GE (result_difficulty, params.network.publish_threshold);
	request.put ("work", badem::to_string_hex (work1));
	request.put ("difficulty", badem::to_string_hex (result_difficulty));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
	...
	request.put ("work", badem::to_string_hex (work1));
	request.put ("difficulty", badem::to_string_hex (difficulty4));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
...
TEST (rpc, accounts_pending)
{
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			std::string account_text (blocks.first);
			ASSERT_EQ (key1.pub.to_account (), account_text);
			badem::block_hash hash1 (blocks.second.begin ()->first);
			ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, account_info)
{
	...
	request.put ("account", badem::account ().to_account ());
...
TEST (rpc, json_block_input)
{
	...
	badem::state_block block (json_error, response.json.get_child ("block"));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	badem::send_block send_from_json (json_error, response.json.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	std::string random_hash = badem::block_hash ().to_string ();
	...
	request.add_child ("hashes", hashes);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (badem::error_blocks::not_found).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, wallet_create_fail)
{
	...
	ASSERT_EQ (std::error_code (badem::error_common::wallet_lmdb_max_dbs).message (), response.json.get<std::string> ("error"));
}
...
TEST (rpc, online_reps)
{
	...
	badem::node_rpc_config node_rpc_config;
	badem::ipc::ipc_server ipc_server (*system.nodes[1], node_rpc_config);
	badem::rpc_config rpc_config (true);
	badem::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	badem::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (badem::test_genesis_key.pub);
	{
	...
	// Make the same request, it should now return an error
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (badem::error_rpc::confirmation_height_not_processing);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, block_confirm_confirmed)
{
	...
	// Check callback
	system.deadline_set (5s);
	while (node->stats.count (badem::stat::type::error, badem::stat::detail::http_callback, badem::stat::dir::out) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	// Callback result is error because callback target port isn't listening
	ASSERT_EQ (1, node->stats.count (badem::stat::type::error, badem::stat::detail::http_callback, badem::stat::dir::out));
	node->stop ();
}
...
TEST (rpc, unopened)
{
	badem::system system (24000, 1);
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::account account1 (1), account2 (account1.number () + 1);
	auto genesis (system.nodes[0]->latest (badem::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (badem::test_genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", badem::account (account2.number () + 1).to_account ());
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		{
...
TEST (rpc, block_confirmed)
{
	...
	ASSERT_EQ (200, response.status);
	ASSERT_EQ (std::error_code (badem::error_blocks::invalid_block_hash).message (), response.json.get<std::string> ("error"));
...
TEST (rpc, database_txn_tracker)
{
	// First try when database tracking is disabled
	{
		...
		request.put ("action", "database_txn_tracker");
		{
			...
			ASSERT_EQ (200, response.status);
			std::error_code ec (badem::error_common::tracking_not_enabled);
			ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &request, &rpc_port = rpc.config.port]() {
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (badem::error_common::invalid_amount);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	};
	// clang-format on
	...
	auto thread_name = badem::thread_role::get_string (badem::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	...
	auto expected_multiplier{ (1.5 + 4.2 + (trend_size - 2) * 1) / trend_size };
	{
		...
		ASSERT_FALSE (badem::from_string_hex (network_minimum_text, network_minimum));
		...
		ASSERT_FALSE (badem::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (badem::difficulty::from_multiplier (expected_multiplier, node->network_params.network.publish_threshold), network_current);
...
TEST (rpc, in_process)
{
	...
	badem::rpc_config rpc_config (true);
	badem::node_rpc_config node_rpc_config;
	badem::inprocess_rpc_handler inprocess_rpc_handler (*node, node_rpc_config);
	badem::rpc rpc (system.io_ctx, rpc_config, inprocess_rpc_handler);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", badem::test_genesis_key.pub.to_account ());
...
TEST (rpc_config, migrate)
{
	...
	bool updated = false;
	auto data_path = badem::unique_path ();
	boost::filesystem::create_directory (data_path);
	badem::node_rpc_config badem_rpc_config;
	badem_rpc_config.deserialize_json (updated, rpc, data_path);
	ASSERT_TRUE (updated);

	// Check that the rpc config file is created
	auto rpc_path = badem::get_rpc_config_path (data_path);
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (badem::test_genesis_key.pub.to_account ());
	account_text[4] = '-';
