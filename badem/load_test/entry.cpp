...
{
	badem::daemon_config daemon_config (data_path);
	...
	toml.write (badem::get_node_toml_config_path (data_path));
	...
	// Write rpc config
	badem::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (badem::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the badem_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the badem_rpc to test");
	// clang-format on
	...
	wallet_add_rpc (ioc, primary_node_results, wallet, badem::test_genesis_key.prv.data.to_string ());
	...
	std::thread t ([send_count, &destination_accounts, &ioc, &primary_node_results, &wallet, &resolver, &node_count]() {
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			std::make_shared<send_session> (ioc, send_calls_remaining, wallet, badem::genesis_account.to_account (), destination_account->as_string, primary_node_results)->run ();
		}
	...
	badem::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
