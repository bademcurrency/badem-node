...
#ifdef BADEM_SECURE_RPC
#include <badem/rpc/rpc_secure.hpp>
#endif

badem::rpc::rpc (boost::asio::io_context & io_ctx_a, badem::rpc_config const & config_a, badem::rpc_handler_interface & rpc_handler_interface_a) :
config (config_a),
acceptor (io_ctx_a),
...
void badem::rpc::accept ()
{
	auto connection (std::make_shared<badem::rpc_connection> (config, io_ctx, logger, rpc_handler_interface));
...
std::unique_ptr<badem::rpc> badem::get_rpc (boost::asio::io_context & io_ctx_a, badem::rpc_config const & config_a, badem::rpc_handler_interface & rpc_handler_interface_a)
{
