...
badem::rpc_handler::rpc_handler (badem::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void(std::string const &)> const & response_a, badem::rpc_handler_interface & rpc_handler_interface_a, badem::logger_mt & logger) :
body (body_a),
...
void badem::rpc_handler::process_request ()
{
	try
	{
		...
		else
		{
			...
			std::error_code rpc_control_disabled_ec = badem::error_rpc::rpc_control_disabled;
