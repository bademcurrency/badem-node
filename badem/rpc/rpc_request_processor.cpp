...
badem::rpc_request_processor::rpc_request_processor (boost::asio::io_context & io_ctx, badem::rpc_config & rpc_config) :
ipc_address (rpc_config.rpc_process.ipc_address.to_string ()),
ipc_port (rpc_config.rpc_process.ipc_port),
thread ([this]() {
	badem::thread_role::set (badem::thread_role::name::rpc_request_processor);
	this->run ();
})
{
	...
	{
		connections.push_back (std::make_shared<badem::ipc_connection> (badem::ipc::ipc_client (io_ctx), false));
...
void badem::rpc_request_processor::read_payload (std::shared_ptr<badem::ipc_connection> connection, std::shared_ptr<std::vector<uint8_t>> res, std::shared_ptr<badem::rpc_request> rpc_request)
{
	uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
	res->resize (payload_size_l);
	// Read JSON payload
	connection->client.async_read (res, payload_size_l, [this, connection, res, rpc_request](badem::error err_read_a, size_t size_read_a) {
...
void badem::rpc_request_processor::try_reconnect_and_execute_request (std::shared_ptr<badem::ipc_connection> connection, std::shared_ptr<std::vector<uint8_t>> req, std::shared_ptr<std::vector<uint8_t>> res, std::shared_ptr<badem::rpc_request> rpc_request)
{
	connection->client.async_connect (ipc_address, ipc_port, [this, connection, req, res, rpc_request](badem::error err) {
		if (!err)
		{
			connection->client.async_write (req, [this, connection, res, rpc_request](badem::error err_a, size_t size_a) {
				if (size_a != 0 && !err_a)
				{
					// Read length
					connection->client.async_read (res, sizeof (uint32_t), [this, connection, res, rpc_request](badem::error err_read_a, size_t size_read_a) {
	...
	while (!stopped)
	{
		if (!requests.empty ())
		{
			...
			if (it != connections.cend ())
			{
				...
				auto req (badem::ipc::prepare_request (badem::ipc::payload_encoding::json_legacy, rpc_request->body));
				auto res (std::make_shared<std::vector<uint8_t>> ());

				// Have we tried to connect yet?
				connection->client.async_write (req, [this, connection, req, res, rpc_request](badem::error err_a, size_t size_a) {
					if (!err_a)
					{
						connection->client.async_read (res, sizeof (uint32_t), [this, connection, req, res, rpc_request](badem::error err_read_a, size_t size_read_a) {
							if (size_read_a != 0 && !err_read_a)
							{
								this->read_payload (connection, res, rpc_request);
							}
