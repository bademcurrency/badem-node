...
TEST (wallet, status)
{
	...
	wallet->start ();
	auto wallet_has = [wallet](badem_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (badem::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (badem_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (badem_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (badem_qt::status_types::disconnected));
}
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (badem::keypair ().prv);
	{
...
TEST (wallet, send)
{
	...
	badem::public_key key1 (system.wallet (1)->insert_adhoc (badem::keypair ().prv));
	auto account (badem::test_genesis_key.pub);
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, badem::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	badem::uint128_t amount (wallet->node.balance (key1));
...
TEST (wallet, process_block)
{
	...
	badem::system system (24000, 1);
	badem::account account;
	badem::block_hash latest (system.nodes[0]->latest (badem::genesis_account));
	system.wallet (0)->insert_adhoc (badem::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, badem::test_genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	badem::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (badem::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (badem::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	badem::system system (24000, 1);
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	system.wallet (0)->send_action (badem::test_genesis_key.pub, key.pub, 100);
	badem::block_hash latest1 (system.nodes[0]->latest (badem::test_genesis_key.pub));
	system.wallet (0)->send_action (badem::test_genesis_key.pub, key.pub, 100);
	badem::block_hash latest2 (system.nodes[0]->latest (badem::test_genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, badem::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	badem::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (badem::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (badem::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	badem::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (badem::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (badem::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	badem::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (badem::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (badem::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, badem::test_genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, badem::test_genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	ASSERT_FALSE (system.nodes[1]->balance (badem::test_genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (badem::test_genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	badem::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, badem::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	badem::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_FALSE (badem::work_validate (system.nodes[0]->ledger.latest_root (transaction, pub), work));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
	...
	while (wallet->active_status.active.count (badem_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (badem_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
