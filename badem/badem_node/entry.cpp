...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for badem_test_network)")
		("debug_profile_votes", "Profile votes processing (only for badem_test_network)")
	...
	boost::filesystem::path data_path ((data_path_it != vm.end ()) ? data_path_it->second.as<std::string> () : badem::working_path ());
	...
	if (ec == badem::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				{
					badem::keypair genesis (key.to_string ());
					...
					for (auto i (0); i != 8; ++i)
					{
						badem::uint128_t yearly_distribution (badem::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node.node->store.tx_begin_read ());
			std::unordered_set<badem::block_hash> frontier_hashes;
		...
		else if (vm.count ("debug_profile_generate"))
		{
			...
			badem::work_pool work (std::numeric_limits<unsigned>::max (), pow_rate_limiter);
		...
		else if (vm.count ("debug_opencl"))
		{
			...
			if (!error)
			{
				...
				{
					if (badem::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
		...
		else if (vm.count ("debug_verify_profile"))
		{
			...
			{
				badem::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			badem::uint256_union message;
			badem::uint512_union signature (badem::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			badem::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				badem::keypair key;
				badem::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					badem::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			badem::network_params test_params;
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			badem::uint128_t genesis_balance (std::numeric_limits<badem::uint128_t>::max ());
			// Generating keys
			std::vector<badem::keypair> keys (num_representatives);
			badem::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<badem::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<badem::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<badem::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./badem_node --debug_random_feed | dieharder -a -g 200
			 */
			badem::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			badem::inactive_node inactive_node_l (data_path);
			badem::node_rpc_config config;
			badem::ipc::ipc_server server (*inactive_node_l.node, config);
			badem::json_handler handler_l (*inactive_node_l.node, config, command_l.str (), response_handler_l);
		...
		else if (vm.count ("debug_validate_blocks"))
		{
			...
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				...
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (badem::work_validate (*block.get ()))
					{
						std::cerr << boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % badem::to_string_hex (block->block_work ()));
					}
			...
			for (auto i (node.node->store.pending_begin (transaction)), n (node.node->store.pending_end ()); i != n; ++i)
			{
				...
				else
				{
					...
					if (auto state = dynamic_cast<badem::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<badem::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			size_t count (0);
			{
				badem::inactive_node node (data_path, 24000);
				auto transaction (node.node->store.tx_begin_read ());
				...
				for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							badem::unchecked_info unchecked_info (block, account, 0, badem::signature_verification::unknown);
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % badem::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		...
		else if (vm.count ("debug_sys_logging"))
		{
			...
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Badem\\Badem registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			badem::inactive_node node (data_path);
		...
		{
			std::cout << "Version " << BADEM_VERSION_STRING << "\n"
