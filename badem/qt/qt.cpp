...
bool badem_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<badem_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<badem_qt::eventloop_event *> (event_a)->action ();
	return true;
}

badem_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
...
your_account_label (new QLabel ("Your Badem account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % BADEM_VERSION_STRING % network).c_str ());
...
void badem_qt::accounts::refresh_wallet_balance ()
{
	...
	{
		badem::public_key const & key (i->first);
...
badem_qt::import::import (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
badem_qt::block_viewer::block_viewer (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void badem_qt::block_viewer::rebroadcast_action (badem::block_hash const & hash_a)
{
	auto done (true);
...
badem_qt::account_viewer::account_viewer (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void badem_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
badem_qt::status::status (badem_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (badem_qt::status_types::nominal);
	set_text ();
}

void badem_qt::status::erase (badem_qt::status_types status_a)
{
	assert (status_a != badem_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void badem_qt::status::insert (badem_qt::status_types status_a)
{
	assert (status_a != badem_qt::status_types::nominal);
...
std::string badem_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case badem_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case badem_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case badem_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case badem_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case badem_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case badem_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case badem_qt::status_types::nominal:
...
std::string badem_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
rendering_ratio (badem::Mbdm_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void badem_qt::wallet::start ()
{
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				badem::uint128_t actual (amount.number ());
	...
	node.observers.blocks.add ([this_w](badem::election_status const & status_a, badem::account const & account_a, badem::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (badem_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (badem_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](std::shared_ptr<badem::transport::channel>) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (badem_qt::status_types::synchronizing);
					}
					else
					{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (badem_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (badem_qt::status_types::working);
					}
				}
			}));
		}
...
void badem_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (badem_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (badem_qt::status_types::disconnected);
	}
}
...
{
	auto balance_str = badem::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("BADEM");
	if (rendering_ratio == badem::kbdm_ratio)
	{
		unit = std::string ("kbadem");
	}
	else if (rendering_ratio == badem::bdm_ratio)
	{
		unit = std::string ("badem");
	}
	else if (rendering_ratio == badem::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void badem_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void badem_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

badem_qt::settings::settings (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
void badem_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void badem_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (badem_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (badem_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (badem_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (badem_qt::status_types::vulnerable);
	}
}

badem_qt::advanced_actions::advanced_actions (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (mbadem_unit, &QRadioButton::toggled, [this]() {
		if (mbadem_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (badem::Mbdm_ratio);
		}
	});
	QObject::connect (kbadem_unit, &QRadioButton::toggled, [this]() {
		if (kbadem_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (badem::kbdm_ratio);
		}
	});
...
void badem_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

badem_qt::block_entry::block_entry (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
badem_qt::block_creation::block_creation (badem_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void badem_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void badem_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void badem_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void badem_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void badem_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						assert (!error);
						badem::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
			...
			if (!destination.is_zero ())
			{
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							badem::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
