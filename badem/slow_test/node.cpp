	...
	std::vector<boost::thread> threads;
	{
		...
		for (auto i (0); i < 20; ++i)
		{
			threads.push_back (boost::thread ([wallet_l, &key]() {
				...
				{
					wallet_l->send_async (badem::test_genesis_key.pub, key.pub, 1000, [](std::shared_ptr<badem::block> block_a) {
...
TEST (confirmation_height, many_accounts_many_confirmations)
{
	...
	std::vector<std::shared_ptr<badem::open_block>> open_blocks;
	{
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
	...
	system.deadline_set (60s);
	while (node->stats.count (badem::stat::type::confirmation_height, badem::stat::detail::blocks_confirmed, badem::stat::dir::in) != (num_accounts - 1) * 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
