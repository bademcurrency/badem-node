...
TEST (bootstrap_processor, process_one)
{
	...
	auto node1 (std::make_shared<badem::node> (system.io_ctx, badem::unique_path (), system.alarm, node_config, system.work, node_flags));
	badem::block_hash hash1 (node0->latest (badem::test_genesis_key.pub));
...
TEST (bootstrap_processor, process_new)
{
	...
	auto node1 (std::make_shared<badem::node> (system.io_ctx, 24002, badem::unique_path (), system.alarm, system.logging, system.work));
	ASSERT_FALSE (node1->init_error ());
	node1->bootstrap_initiator.bootstrap (system.nodes[0]->network.endpoint ());
	system.deadline_set (10s);
...
TEST (bootstrap_processor, pull_requeue_network_error)
{
	...
	badem::genesis genesis;
	badem::keypair key1;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	...
	{
		badem::unique_lock<std::mutex> lock (attempt->mutex);
		ASSERT_FALSE (attempt->stopped);
		attempt->pulls.push_back (badem::pull_info (badem::test_genesis_key.pub, send1->hash (), genesis.hash ()));
	...
	ASSERT_EQ (0, node1->stats.count (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_failed_account, badem::stat::dir::in)); // Requeue is not increasing failed attempts
}
...
TEST (bootstrap_processor, frontiers_unconfirmed)
{
	...
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (badem::process_result::progress, node1->process (*send2).code);
	auto open1 (std::make_shared<badem::state_block> (key1.pub, 0, key1.pub, badem::Gbdm_ratio, send1->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (badem::process_result::progress, node1->process (*open1).code);
	auto open2 (std::make_shared<badem::state_block> (key2.pub, 0, key2.pub, badem::Gbdm_ratio, send2->hash (), key2.prv, key2.pub, *system.work.generate (key2.pub)));
	...
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::bdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	ASSERT_EQ (badem::process_result::progress, node2->process (*send3).code);
	auto open3 (std::make_shared<badem::state_block> (key1.pub, 0, key1.pub, badem::bdm_ratio, send3->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (badem::process_result::progress, node2->process (*open3).code);
	...
	node3->bootstrap_initiator.excluded_peers.add (badem::transport::map_endpoint_to_tcp (node1->network.endpoint ()), 0);
	ASSERT_FALSE (node3->bootstrap_initiator.excluded_peers.check (badem::transport::map_endpoint_to_tcp (node1->network.endpoint ())));
	...
	ASSERT_EQ (1, node3->stats.count (badem::stat::type::bootstrap, badem::stat::detail::frontier_confirmation_failed, badem::stat::dir::in)); // failed request from node1
	ASSERT_TRUE (node3->bootstrap_initiator.excluded_peers.check (badem::transport::map_endpoint_to_tcp (node1->network.endpoint ())));
}
...
TEST (bootstrap_processor, frontiers_confirmed)
{
	...
	ASSERT_EQ (1, node2->stats.count (badem::stat::type::bootstrap, badem::stat::detail::frontier_confirmation_successful, badem::stat::dir::in)); // Successful request from node1
	ASSERT_EQ (0, node2->stats.count (badem::stat::type::bootstrap, badem::stat::detail::frontier_confirmation_failed, badem::stat::dir::in));
}
...
TEST (bootstrap_processor, push_one)
{
	...
	badem::keypair key1;
	auto node1 (std::make_shared<badem::node> (system.io_ctx, 24001, badem::unique_path (), system.alarm, system.logging, system.work));
	auto wallet (node1->wallets.create (badem::random_wallet_id ()));
...
TEST (bootstrap_processor, lazy_hash)
{
	...
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (genesis.hash ())));
	auto receive1 (std::make_shared<badem::state_block> (key1.pub, 0, key1.pub, badem::Gbdm_ratio, send1->hash (), key1.prv, key1.pub, *system.nodes[0]->work_generate_blocking (key1.pub)));
	...
	auto receive2 (std::make_shared<badem::state_block> (key2.pub, 0, key2.pub, badem::Gbdm_ratio, send2->hash (), key2.prv, key2.pub, *system.nodes[0]->work_generate_blocking (key2.pub)));
...
TEST (bootstrap_processor, lazy_max_pull_count)
{
	...
	auto send2 (std::make_shared<badem::state_block> (key1.pub, receive1->hash (), key1.pub, 0, key2.pub, key1.prv, key1.pub, *system.nodes[0]->work_generate_blocking (receive1->hash ())));
	auto receive2 (std::make_shared<badem::state_block> (key2.pub, 0, key2.pub, badem::Gbdm_ratio, send2->hash (), key2.prv, key2.pub, *system.nodes[0]->work_generate_blocking (key2.pub)));
	auto change1 (std::make_shared<badem::state_block> (key2.pub, receive2->hash (), key1.pub, badem::Gbdm_ratio, 0, key2.prv, key2.pub, *system.nodes[0]->work_generate_blocking (receive2->hash ())));
	auto change2 (std::make_shared<badem::state_block> (key2.pub, change1->hash (), badem::test_genesis_key.pub, badem::Gbdm_ratio, 0, key2.prv, key2.pub, *system.nodes[0]->work_generate_blocking (change1->hash ())));
	auto change3 (std::make_shared<badem::state_block> (key2.pub, change2->hash (), key2.pub, badem::Gbdm_ratio, 0, key2.prv, key2.pub, *system.nodes[0]->work_generate_blocking (change2->hash ())));
	// Processing test chain
	system.nodes[0]->block_processor.add (send1);
	system.nodes[0]->block_processor.add (receive1);
	system.nodes[0]->block_processor.add (send2);
...
TEST (bootstrap_processor, lazy_unclear_state_link)
{
	...
	ASSERT_EQ (badem::process_result::progress, node1->process (*send1).code);
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (badem::process_result::progress, node1->process (*send2).code);
	auto open (std::make_shared<badem::open_block> (send1->hash (), key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (badem::process_result::progress, node1->process (*open).code);
	auto receive (std::make_shared<badem::state_block> (key.pub, open->hash (), key.pub, 2 * badem::Gbdm_ratio, send2->hash (), key.prv, key.pub, *system.work.generate (open->hash ()))); // It is not possible to define this block send/receive status based on previous block (legacy open)
	ASSERT_EQ (badem::process_result::progress, node1->process (*receive).code);
	// Start lazy bootstrap with last block in chain known
	...
	ASSERT_EQ (0, node2->stats.count (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_failed_account, badem::stat::dir::in));
}
...
TEST (bootstrap_processor, lazy_unclear_state_link_not_existing)
{
	...
	auto open (std::make_shared<badem::open_block> (send1->hash (), key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (badem::process_result::progress, node1->process (*open).code);
	auto send3 (std::make_shared<badem::state_block> (key.pub, open->hash (), key.pub, 0, key2.pub, key.prv, key.pub, *system.work.generate (open->hash ()))); // It is not possible to define this block send/receive status based on previous block (legacy open)
	ASSERT_EQ (badem::process_result::progress, node1->process (*send3).code);
	// Start lazy bootstrap with last block in chain known
	auto node2 = system.add_node (badem::node_config (24001, system.logging), node_flags);
	node2->network.udp_channels.insert (node1->network.endpoint (), node1->network_params.protocol.protocol_version);
	...
	ASSERT_EQ (1, node2->stats.count (badem::stat::type::bootstrap, badem::stat::detail::bulk_pull_failed_account, badem::stat::dir::in));
}
...
TEST (bootstrap_processor, lazy_destinations)
{
	...
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (badem::process_result::progress, node1->process (*send2).code);
	auto open (std::make_shared<badem::open_block> (send1->hash (), key1.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (badem::process_result::progress, node1->process (*open).code);
	auto state_open (std::make_shared<badem::state_block> (key2.pub, 0, key2.pub, badem::Gbdm_ratio, send2->hash (), key2.prv, key2.pub, *system.work.generate (key2.pub)));
	ASSERT_EQ (badem::process_result::progress, node1->process (*state_open).code);
...
TEST (frontier_req, count)
{
	...
	badem::state_block send1 (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
	node1.work_generate_blocking (send1);
	ASSERT_EQ (badem::process_result::progress, node1.process (send1).code);
	badem::state_block receive1 (key1.pub, 0, badem::test_genesis_key.pub, badem::Gbdm_ratio, send1.hash (), key1.prv, key1.pub, 0);
