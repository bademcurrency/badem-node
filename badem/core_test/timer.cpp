...
TEST (timer, states)
{
	badem::timer<std::chrono::milliseconds> t1;
	ASSERT_EQ (t1.current_state (), badem::timer_state::stopped);
	t1.start ();
	ASSERT_EQ (t1.current_state (), badem::timer_state::started);
	t1.restart ();
	ASSERT_EQ (t1.current_state (), badem::timer_state::started);
	t1.pause ();
	ASSERT_EQ (t1.current_state (), badem::timer_state::stopped);
	...
	badem::timer<std::chrono::milliseconds> t2 (badem::timer_state::started);
	ASSERT_EQ (t2.current_state (), badem::timer_state::started);
	t2.stop ();
	ASSERT_EQ (t2.current_state (), badem::timer_state::stopped);
}

TEST (timer, measure_and_compare)
{
	using namespace std::chrono_literals;
	badem::timer<std::chrono::milliseconds> t1 (badem::timer_state::started);
	ASSERT_EQ (t1.current_state (), badem::timer_state::started);
