...
TEST (socket, concurrent_writes)
{
	...
	badem::util::counted_completion read_count_completion (total_message_count);
	std::function<void(std::shared_ptr<badem::socket>)> reader = [&read_count_completion, &total_message_count, &reader](std::shared_ptr<badem::socket> socket_a) {
...
#pragma GCC diagnostic pop
	...
	auto server_socket (std::make_shared<badem::server_socket> (node, endpoint, max_connections, badem::socket::concurrency::multi_writer));
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	std::vector<std::shared_ptr<badem::socket>> connections;

	// On every new connection, start reading data
	server_socket->on_connection ([&connections, &reader](std::shared_ptr<badem::socket> new_connection, boost::system::error_code const & ec_a) {
		if (ec_a)
		{
	...
	badem::util::counted_completion connection_count_completion (client_count);
	std::vector<std::shared_ptr<badem::socket>> clients;
	for (unsigned i = 0; i < client_count; i++)
	{
		auto client (std::make_shared<badem::socket> (node, boost::none, badem::socket::concurrency::multi_writer));
...
#pragma GCC diagnostic ignored "-Wunused-lambda-capture"
		...
		client_threads.emplace_back ([&client, &message_count]() {
			for (int i = 0; i < message_count; i++)
			{
				...
				buff.push_back ('A' + i);
				client->async_write (badem::shared_const_buffer (std::move (buff)));
			}
		});
#ifndef _WIN32
#pragma GCC diagnostic pop
	...
	ASSERT_EQ (node->stats.count (badem::stat::type::tcp, badem::stat::detail::tcp_accept_success, badem::stat::dir::in), client_count);
	// We may exhaust max connections and have some tcp accept failures, but no more than the client count
	ASSERT_LT (node->stats.count (badem::stat::type::tcp, badem::stat::detail::tcp_accept_failure, badem::stat::dir::in), client_count);
