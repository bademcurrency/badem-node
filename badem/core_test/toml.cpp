...
TEST (toml, diff)
{
	badem::tomlconfig defaults, other;
...
TEST (toml, daemon_config_update_array)
{
	...
	badem::daemon_config c (data_path);
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	badem::tomlconfig t;
	t.read (ss);
	badem::rpc_config conf;
	badem::rpc_config defaults;
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	badem::tomlconfig t;
	t.read (ss);
	badem::daemon_config conf;
	badem::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == badem::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == badem::error_config::invalid_value);
}

TEST (toml, put)
{
	badem::tomlconfig config;
	badem::tomlconfig config_node;
	// Overwrite value and add to child node
	config_node.put ("port", "2223");
	config_node.put ("port", "2224");
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["badem_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
	...
	badem::tomlconfig toml2;
	toml2.read (ss_frontiers_confirmation);
	badem::daemon_config conf2;
	...
	ASSERT_EQ (conf2.node.frontiers_confirmation, badem::frontiers_confirmation_mode::invalid);
}
