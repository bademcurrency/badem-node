...
namespace
{
void add_required_children_node_config_tree (badem::jsonconfig & tree);
}
...
TEST (node, block_store_path_failure)
{
	...
	badem::alarm alarm (*service);
	auto path (badem::unique_path ());
	badem::logging logging;
	logging.init (path);
	badem::work_pool work (std::numeric_limits<unsigned>::max ());
	auto node (std::make_shared<badem::node> (init, *service, 24000, path, alarm, logging, work));
...
TEST (node, password_fanout)
{
	...
	auto node (std::make_shared<badem::node> (init, *service, path, alarm, config, work));
...
TEST (node, balance)
{
	...
	ASSERT_EQ (std::numeric_limits<badem::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, badem::test_genesis_key.pub));
}
...
TEST (node, send_unkeyed)
{
	...
	badem::keypair key2;
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	system.wallet (0)->store.password.value_set (badem::keypair ().prv);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
	...
	ASSERT_EQ (std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (badem::test_genesis_key.pub));
}
...
TEST (node, send_single_observing_peer)
{
	...
	ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<badem::node> const & node_a) { return node_a->balance (key2.pub).is_zero (); }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, send_out_of_order)
{
	...
	badem::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ()));
	badem::send_block send2 (send1.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send1.hash ()));
	badem::send_block send3 (send2.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 3, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send2.hash ()));
	system.nodes[0]->process_active (std::make_shared<badem::send_block> (send3));
	system.nodes[0]->process_active (std::make_shared<badem::send_block> (send2));
	system.nodes[0]->process_active (std::make_shared<badem::send_block> (send1));
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<badem::node> const & node_a) { return node_a->balance (badem::test_genesis_key.pub) != badem::genesis_amount - system.nodes[0]->config.receive_minimum.number () * 3; }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, quick_confirm)
{
	badem::system system (24000, 1);
	badem::keypair key;
	badem::block_hash previous (system.nodes[0]->latest (badem::test_genesis_key.pub));
	auto genesis_start_balance (system.nodes[0]->balance (badem::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	auto send (std::make_shared<badem::send_block> (previous, key.pub, system.nodes[0]->config.online_weight_minimum.number () + 1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (previous)));
	...
	ASSERT_EQ (system.nodes[0]->balance (badem::test_genesis_key.pub), system.nodes[0]->config.online_weight_minimum.number () + 1);
...
TEST (node, node_receive_quorum)
{
	...
	badem::block_hash previous (system.nodes[0]->latest (badem::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (std::make_shared<badem::send_block> (previous, key.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (previous)));
	system.nodes[0]->process_active (send);
	system.deadline_set (10s);
	...
	{
		{
			...
			auto info (system.nodes[0]->active.roots.find (badem::qualified_root (previous, previous)));
			ASSERT_NE (system.nodes[0]->active.roots.end (), info);
			done = info->election->confirmation_request_count > badem::active_transactions::minimum_confirmation_request_count;
		}
		ASSERT_NO_ERROR (system.poll ());
	}
	badem::system system2 (24001, 1);
	system2.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	ASSERT_TRUE (system.nodes[0]->balance (key.pub).is_zero ());
	auto channel (std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system2.nodes[0]->network.endpoint ()));
...
TEST (node, auto_bootstrap)
{
	...
	while (node1->stats.count (badem::stat::type::observer, badem::stat::detail::observer_confirmation_active_quorum, badem::stat::dir::out) + node1->stats.count (badem::stat::type::observer, badem::stat::detail::observer_confirmation_active_conf_height, badem::stat::dir::out) < 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (node, auto_bootstrap_reverse)
{
	...
	ASSERT_FALSE (init1.error ());
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	auto channel (std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, node1->network.endpoint ()));
	system.nodes[0]->network.send_keepalive (channel);
	node1->start ();
	system.nodes.push_back (node1);
	system.deadline_set (10s);
...
TEST (node, receive_gap)
{
	...
	auto block (std::make_shared<badem::send_block> (5, 1, 2, badem::keypair ().prv, 4, 0));
...
{
	badem::system system (24000, 1);
	std::array<badem::endpoint, 8> endpoints;
	endpoints.fill (badem::endpoint (boost::asio::ip::address_v6::loopback (), 24000));
	endpoints[0] = badem::endpoint (boost::asio::ip::address_v6::loopback (), 24001);
...
TEST (node, search_pending_multiple)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key3.pub, system.nodes[0]->config.receive_minimum.number ()));
	system.deadline_set (10s);
...
TEST (node, search_pending_confirmed)
{
	...
	badem::keypair key2;
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	auto send1 (system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, node->config.receive_minimum.number ()));
	...
	}
	{
		...
		system.wallet (0)->store.erase (transaction, badem::test_genesis_key.pub);
	}
	system.wallet (0)->insert_adhoc (key2.prv);
	ASSERT_FALSE (system.wallet (0)->search_pending ());
	{
...
TEST (node, unlock_search)
{
	badem::system system (24000, 1);
	badem::keypair key2;
	badem::uint128_t balance (system.nodes[0]->balance (badem::test_genesis_key.pub));
	{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	system.deadline_set (10s);
	while (system.nodes[0]->balance (badem::test_genesis_key.pub) == balance)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (node, connect_after_junk)
{
	...
	auto channel1 (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, system.nodes[0]->network.endpoint ()));
	channel1->send_buffer (junk_buffer, badem::stat::detail::bulk_pull, [](boost::system::error_code const &, size_t) {});
	system.deadline_set (10s);
	while (system.nodes[0]->stats.count (badem::stat::type::error) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	node1->start ();
	system.nodes.push_back (node1);
	auto channel2 (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, system.nodes[0]->network.endpoint ()));
...
{
	badem::system system (24000, 1);
	auto price1 (system.nodes[0]->price (badem::kBDM_ratio, 1));
	ASSERT_EQ (badem::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (badem::kBDM_ratio * int(badem::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (badem::kBDM_ratio * int(badem::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (badem::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (badem::kBDM_ratio * int(badem::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node, confirm_locked)
{
	...
	auto block (std::make_shared<badem::send_block> (0, 0, 0, badem::keypair ().prv, 0, 0));
...
{
	auto path (badem::unique_path ());
	badem::logging logging1;
	logging1.init (path);
	badem::node_config config1 (100, logging1);
...
TEST (node_config, v2_v3_upgrade)
{
	...
	config1.deserialize_json (upgraded, tree);
	//ASSERT_EQ (badem::uint128_union (0).to_string_dec (), tree.get<std::string> ("inactive_supply"));
...
TEST (node_config, v15_v16_upgrade)
{
	auto test_upgrade = [](auto old_preconfigured_peers_url, auto new_preconfigured_peers_url) {
		...
		badem::jsonconfig preconfigured_peers_json;
	...
	test_upgrade ("node.badem.io", "node.badem.io");
	test_upgrade ("node-beta.badem.io", "peering-beta.badem.io");
}
...
TEST (node_config, v16_values)
{
	...
	tree.put ("vote_minimum", badem::kBDM_ratio.convert_to<std::string> ());
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_FALSE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 1);
	ASSERT_EQ (config.vote_minimum.number (), badem::kBDM_ratio);

	// Check config is correct with other values
	tree.put ("allow_local_peers", true);
	tree.put ("signature_checker_threads", 4);
	tree.put ("vote_minimum", (std::numeric_limits<badem::uint128_t>::max () - 100).convert_to<std::string> ());
	upgraded = false;
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_TRUE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 4);
	ASSERT_EQ (config.vote_minimum.number (), std::numeric_limits<badem::uint128_t>::max () - 100);
}
...
TEST (node_config, v17_values)
{
	...
	// Check config is correct
	{
		...
		badem::jsonconfig diagnostics_l;
...
public:
	...
	badem::error serialize_json (badem::jsonconfig & json)
	{
		json.put ("thing", text);
...
{
public:
	badem::error deserialize_json (bool & upgraded, badem::jsonconfig & json)
	{
		if (!json.empty ())
		{
			...
			if (text_l == "error")
			{
				json.get_error () = badem::error_common::generic;
			}
			text = text_l;
		}
		else
...
TEST (node_flags, disable_tcp_realtime)
{
	...
	ASSERT_EQ (badem::transport::transport_type::udp, list1[0]->get_type ());
	ASSERT_EQ (1, node2->network.size ());
	auto list2 (node2->network.list (2));
	ASSERT_EQ (node1->network.endpoint (), list2[0]->get_endpoint ());
	ASSERT_EQ (badem::transport::transport_type::udp, list2[0]->get_type ());
}
...
TEST (node_flags, disable_udp)
{
	...
	badem::node_init init;
	auto node2 (std::make_shared<badem::node> (init, system.io_ctx, badem::unique_path (), system.alarm, badem::node_config (24001, system.logging), system.work, node_flags));
	system.nodes.push_back (node2);
	node2->start ();
	// Send UDP message
	auto channel (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, node2->network.endpoint ()));
	node1->network.send_keepalive (channel);
...
TEST (node, fork_publish)
{
	std::weak_ptr<badem::node> node0;
	{
		...
		badem::keypair key2;
		auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
		...
		node1.block_processor.flush ();
		lock.lock ();
		auto existing1 (election->last_votes.find (badem::test_genesis_key.pub));
...
TEST (node, fork_keep)
{
	...
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key1.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	node1.block_processor.flush ();
	...
	auto conflict (node2.active.roots.find (badem::qualified_root (genesis.hash (), genesis.hash ())));
...
TEST (node, fork_flip)
{
	...
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key1.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	badem::publish publish1 (send1);
	badem::keypair key2;
	auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	badem::publish publish2 (send2);
...
TEST (node, fork_multi_flip)
{
	...
	auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	badem::publish publish2 (send2);
	auto send3 (std::make_shared<badem::send_block> (publish2.block->hash (), key2.pub, badem::genesis_amount - 100, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (publish2.block->hash ())));
	badem::publish publish3 (send3);
...
TEST (node, fork_bootstrap_flip)
{
	...
	badem::block_hash latest (system0.nodes[0]->latest (badem::test_genesis_key.pub));
	badem::keypair key1;
	auto send1 (std::make_shared<badem::send_block> (latest, key1.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system0.work.generate (latest)));
	badem::keypair key2;
	auto send2 (std::make_shared<badem::send_block> (latest, key2.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system0.work.generate (latest)));
	// Insert but don't rebroadcast, simulating settled blocks
	node1.block_processor.add (send1, badem::seconds_since_epoch ());
	node1.block_processor.flush ();
	node2.block_processor.add (send2, badem::seconds_since_epoch ());
	node2.block_processor.flush ();
	{
...
TEST (node, fork_open)
{
	...
	badem::keypair key1;
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key1.pub, 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	badem::publish publish1 (send1);
	auto channel1 (node1.network.udp_channels.create (node1.network.endpoint ()));
	node1.network.process_message (publish1, channel1);
	node1.block_processor.flush ();
	auto open1 (std::make_shared<badem::open_block> (publish1.block->hash (), 1, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	badem::publish publish2 (open1);
	node1.network.process_message (publish2, channel1);
	node1.block_processor.flush ();
	auto open2 (std::make_shared<badem::open_block> (publish1.block->hash (), 2, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	badem::publish publish3 (open2);
...
TEST (node, fork_open_flip)
{
	...
	badem::keypair rep1;
	badem::keypair rep2;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key1.pub, badem::genesis_amount - 1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	node2.process_active (send1);
	// We should be keeping this block
	auto open1 (std::make_shared<badem::open_block> (send1->hash (), rep1.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	// This block should be evicted
	auto open2 (std::make_shared<badem::open_block> (send1->hash (), rep2.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
	...
	ASSERT_EQ (badem::genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
	badem::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1](badem::election_status const & status_a, badem::account const &, badem::uint128_t const &, bool) {
		auto transaction (node1.store.tx_begin_read ());
		ASSERT_TRUE (node1.store.block_exists (transaction, status_a.winner->hash ()));
	});
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::keypair key;
	system.wallet (0)->send_action (badem::test_genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
	...
	system.wallet (0)->send_action (badem::test_genesis_key.pub, key4, badem::genesis_amount / 4);
	...
	auto block (system.wallet (0)->send_action (badem::test_genesis_key.pub, key1, node1.config.receive_minimum.number ()));
	...
	badem::state_block send1 (badem::test_genesis_key.pub, block->hash (), badem::test_genesis_key.pub, (badem::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), key1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (block->hash ()));
	ASSERT_EQ (badem::process_result::progress, node1.process (send1).code);
	ASSERT_EQ (badem::process_result::progress, node2.process (send1).code);
	ASSERT_EQ (badem::process_result::progress, node3.process (send1).code);
	auto key2 (system.wallet (2)->deterministic_insert ());
	auto send2 (std::make_shared<badem::send_block> (block->hash (), key2, (badem::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (block->hash ())));
	badem::raw_key key3;
	auto transaction (system.wallet (1)->wallets.tx_begin_read ());
	ASSERT_FALSE (system.wallet (1)->store.fetch (transaction, key1, key3));
	auto vote (std::make_shared<badem::vote> (key1, key3, 0, send2));
	...
	badem::transport::channel_udp channel (node2.network.udp_channels, node3.network.endpoint ());
	channel.send_buffer (bytes, badem::stat::detail::confirm_ack);
	while (node3.stats.count (badem::stat::type::message, badem::stat::detail::confirm_ack, badem::stat::dir::in) < 3)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_TRUE (node1.latest (badem::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node2.latest (badem::test_genesis_key.pub) == send1.hash ());
	ASSERT_TRUE (node3.latest (badem::test_genesis_key.pub) == send1.hash ());
}
...
TEST (node, DISABLED_fork_pre_confirm)
{
	...
	system.deadline_set (30s);
	auto block0 (system.wallet (0)->send_action (badem::test_genesis_key.pub, key1.pub, badem::genesis_amount / 3));
	ASSERT_NE (nullptr, block0);
	while (node0.balance (key1.pub) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto block1 (system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, badem::genesis_amount / 3));
	...
	badem::keypair key3;
	badem::keypair key4;
	auto block2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, node0.latest (badem::test_genesis_key.pub), key3.pub, node0.balance (badem::test_genesis_key.pub), 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	auto block3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, node0.latest (badem::test_genesis_key.pub), key4.pub, node0.balance (badem::test_genesis_key.pub), 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	while (!done)
	{
		done |= node0.latest (badem::test_genesis_key.pub) == block2->hash () && node1.latest (badem::test_genesis_key.pub) == block2->hash () && node2.latest (badem::test_genesis_key.pub) == block2->hash ();
		done |= node0.latest (badem::test_genesis_key.pub) == block3->hash () && node1.latest (badem::test_genesis_key.pub) == block3->hash () && node2.latest (badem::test_genesis_key.pub) == block3->hash ();
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, DISABLED_fork_stale)
{
	...
	system1.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::system system2 (24001, 1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	auto channel (std::make_shared<badem::transport::channel_udp> (node2.network.udp_channels, node1.network.endpoint ()));
	node2.rep_crawler.response (channel, badem::test_genesis_key.pub, badem::genesis_amount);
	badem::genesis genesis;
	badem::keypair key1;
	badem::keypair key2;
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::BDM_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::BDM_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::BDM_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	{
		auto transaction1 (node1.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node1.ledger.process (transaction1, *send1).code);
		auto transaction2 (node2.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node2.ledger.process (transaction2, *send2).code);
	}
	node1.process_active (send1);
	node1.process_active (send2);
	node2.process_active (send1);
...
TEST (node, broadcast_elected)
{
	...
	for (auto & type : types)
	{
		badem::system system (24000, 3, type);
		auto node0 (system.nodes[0]);
		auto node1 (system.nodes[1]);
		auto node2 (system.nodes[2]);
		badem::keypair rep_big;
		badem::keypair rep_small;
		badem::keypair rep_other;
		...
		//std::cerr << "Other: " << rep_other.pub.to_account () << std::endl;
		{
			...
			auto transaction1 (node1->store.tx_begin_write ());
			auto transaction2 (node2->store.tx_begin_write ());
			badem::send_block fund_big (node0->ledger.latest (transaction0, badem::test_genesis_key.pub), rep_big.pub, badem::kBDM_ratio * 5, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
			badem::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
			badem::send_block fund_small (fund_big.hash (), rep_small.pub, badem::kBDM_ratio * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
			badem::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
			badem::send_block fund_other (fund_small.hash (), rep_other.pub, badem::kBDM_ratio * 1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
			badem::open_block open_other (fund_other.hash (), rep_other.pub, rep_other.pub, rep_other.prv, rep_other.pub, 0);
			...
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, open_big).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, open_big).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, open_big).code);
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, open_small).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, open_small).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, open_small).code);
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
			ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, open_other).code);
			ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, open_other).code);
			ASSERT_EQ (badem::process_result::progress, node2->ledger.process (transaction2, open_other).code);
		}
		...
		auto fork0 (std::make_shared<badem::send_block> (node2->latest (badem::test_genesis_key.pub), rep_small.pub, 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
		node0->work_generate_blocking (*fork0);
		node0->process_active (fork0);
		node1->process_active (fork0);
		auto fork1 (std::make_shared<badem::send_block> (node2->latest (badem::test_genesis_key.pub), rep_big.pub, 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
		...
		system.deadline_set (5s);
		while (node1->stats.count (badem::stat::type::observer, badem::stat::detail::observer_confirmation_inactive, badem::stat::dir::out) == 0)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	}
...
TEST (node, rep_self_vote)
{
	...
	{
		auto transaction0 (node0->store.tx_begin_write ());
		badem::send_block fund_big (node0->ledger.latest (transaction0, badem::test_genesis_key.pub), rep_big.pub, badem::uint128_t ("0xb0000000000000000000000000000000"), badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
		badem::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
		node0->work_generate_blocking (fund_big);
		node0->work_generate_blocking (open_big);
		ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
	...
	auto block0 (std::make_shared<badem::send_block> (node0->latest (badem::test_genesis_key.pub), rep_big.pub, badem::uint128_t ("0x60000000000000000000000000000000"), badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node0->work_generate_blocking (*block0);
	ASSERT_EQ (badem::process_result::progress, node0->process (*block0).code);
	...
	ASSERT_NE (rep_votes.end (), rep_votes.find (badem::test_genesis_key.pub));
...
TEST (node, DISABLED_bootstrap_no_publish)
{
	...
	badem::send_block send0 (system0.nodes[0]->latest (badem::test_genesis_key.pub), key0.pub, 500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, system0.nodes[0]->ledger.process (transaction, send0).code);
	}
...
{
	badem::system system0;
	badem::node_flags node_flags;
	node_flags.delay_frontier_confirmation_height_updating = true;
	auto node0 = system0.add_node (badem::node_config (24000, system0.logging), node_flags);
	auto node1 = system0.add_node (badem::node_config (24001, system0.logging), node_flags);
	system0.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::keypair key0;
	badem::send_block send0 (system0.nodes[0]->latest (badem::test_genesis_key.pub), key0.pub, badem::genesis_amount - 500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
	badem::open_block open0 (send0.hash (), 1, key0.pub, key0.prv, key0.pub, 0);
	badem::open_block open1 (send0.hash (), 2, key0.pub, key0.prv, key0.pub, 0);
	node0->work_generate_blocking (send0);
	...
	node0->work_generate_blocking (open1);
	{
		...
		ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, send0).code);
		ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, send0).code);
		// They disagree about open0/open1
		ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction0, open0).code);
		ASSERT_EQ (badem::process_result::progress, node1->ledger.process (transaction1, open1).code);
	}
	ASSERT_FALSE (node1->bootstrap_initiator.in_progress ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint ());
	ASSERT_TRUE (node1->active.empty ());
...
TEST (node, bootstrap_confirm_frontiers)
{
	...
	badem::keypair key0;
	// node0 knows about send0 but node1 doesn't.
	badem::send_block send0 (node0->latest (badem::test_genesis_key.pub), key0.pub, badem::genesis_amount - 500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
	node0->work_generate_blocking (send0);
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node0->ledger.process (transaction, send0).code);
	}
	ASSERT_FALSE (node0->bootstrap_initiator.in_progress ());
	ASSERT_FALSE (node1->bootstrap_initiator.in_progress ());
	ASSERT_TRUE (node1->active.empty ());
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	wallet0->insert_adhoc (badem::test_genesis_key.prv);
	auto send1 (wallet0->send_action (badem::genesis_account, key0.pub, 2 * badem::BDM_ratio));
	system.deadline_set (10s);
	while (node1.balance (key0.pub) != 2 * badem::BDM_ratio || node1.bootstrap_initiator.in_progress ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto latest (node1.latest (key0.pub));
	badem::state_block send2 (key0.pub, latest, badem::genesis_account, badem::BDM_ratio, badem::genesis_account, key0.prv, key0.pub, node0.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node1.ledger.process (transaction, send2).code);
	}
	auto send3 (wallet1->send_action (key0.pub, badem::genesis_account, badem::BDM_ratio));
	system.deadline_set (10s);
	while (node0.balance (badem::genesis_account) != badem::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (badem::test_genesis_key.pub, key1.pub, badem::BDM_ratio);
...
TEST (node, rep_weight)
{
	...
	node.network.udp_channels.insert (badem::endpoint (boost::asio::ip::address_v6::loopback (), 24001), 0);
	...
	auto channel0 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint0));
	auto channel1 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint1));
	auto channel2 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint2));
	badem::amount amount100 (100);
	badem::amount amount50 (50);
	node.network.udp_channels.insert (endpoint2, badem::protocol_version);
	node.network.udp_channels.insert (endpoint0, badem::protocol_version);
	node.network.udp_channels.insert (endpoint1, badem::protocol_version);
	badem::keypair keypair1;
	badem::keypair keypair2;
...
TEST (node, rep_remove)
{
	...
	auto node1 = system.add_node (badem::node_config (24002, system.logging));
	...
	node.rep_crawler.response (channel1, badem::test_genesis_key.pub, badem::genesis_amount);
	ASSERT_EQ (2, node.rep_crawler.representative_count ());
	// Add inactive TCP representative channel
	badem::node_init init;
	auto node2 (std::make_shared<badem::node> (init, system.io_ctx, badem::unique_path (), system.alarm, badem::node_config (24003, system.logging), system.work));
	std::atomic<bool> done{ false };
	std::weak_ptr<badem::node> node_w (node.shared ());
	node.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w, &done](std::shared_ptr<badem::transport::channel> channel2) {
		if (auto node_l = node_w.lock ())
		{
			badem::keypair keypair2;
			node_l->rep_crawler.response (channel2, keypair2.pub, badem::kBDM_ratio);
	...
	ASSERT_EQ (badem::test_genesis_key.pub, reps[0].account);
...
TEST (node, no_voting)
{
	...
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (badem::test_genesis_key.pub, key1.pub, badem::BDM_ratio);
	system.deadline_set (10s);
	while (!node0.active.empty ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (0, node0.stats.count (badem::stat::type::message, badem::stat::detail::confirm_ack, badem::stat::dir::in));
}
...
TEST (node, vote_replay)
{
	badem::system system (24000, 2);
	badem::keypair key;
	auto open (std::make_shared<badem::open_block> (0, 1, key.pub, key.prv, key.pub, 0));
	system.nodes[0]->work_generate_blocking (*open);
	for (auto i (0); i < 11000; ++i)
	{
		auto transaction (system.nodes[1]->store.tx_begin_read ());
		auto vote (system.nodes[1]->store.vote_generate (transaction, badem::test_genesis_key.pub, badem::test_genesis_key.prv, open));
	}
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		std::lock_guard<std::mutex> lock (system.nodes[0]->store.get_cache_mutex ());
		auto vote (system.nodes[0]->store.vote_current (transaction, badem::test_genesis_key.pub));
...
TEST (node, balance_observer)
{
	...
	badem::keypair key;
	node1.observers.account_balance.add ([&key, &balances](badem::account const & account_a, bool is_pending) {
		if (key.pub == account_a && is_pending)
		{
			balances++;
		}
		else if (badem::test_genesis_key.pub == account_a && !is_pending)
		{
			balances++;
		}
	});
...
TEST (node, stat_counting)
{
	badem::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	node1.stats.add (badem::stat::type::ledger, badem::stat::dir::in, 1);
	node1.stats.add (badem::stat::type::ledger, badem::stat::dir::in, 5);
	node1.stats.inc (badem::stat::type::ledger, badem::stat::dir::in);
	node1.stats.inc (badem::stat::type::ledger, badem::stat::detail::send, badem::stat::dir::in);
	node1.stats.inc (badem::stat::type::ledger, badem::stat::detail::send, badem::stat::dir::in);
	node1.stats.inc (badem::stat::type::ledger, badem::stat::detail::receive, badem::stat::dir::in);
	ASSERT_EQ (10, node1.stats.count (badem::stat::type::ledger, badem::stat::dir::in));
	ASSERT_EQ (2, node1.stats.count (badem::stat::type::ledger, badem::stat::detail::send, badem::stat::dir::in));
	ASSERT_EQ (1, node1.stats.count (badem::stat::type::ledger, badem::stat::detail::receive, badem::stat::dir::in));
}
...
TEST (node, online_reps)
{
	...
	auto vote (std::make_shared<badem::vote> ());
	system.nodes[0]->online_reps.observe (badem::test_genesis_key.pub);
	// 1 minimum, 1 maximum
	system.nodes[0]->online_reps.sample ();
	ASSERT_EQ (badem::genesis_amount, system.nodes[0]->online_reps.online_stake ());
	// 2 minimum, 1 maximum
	system.nodes[0]->online_reps.sample ();
	ASSERT_EQ (system.nodes[0]->config.online_weight_minimum, system.nodes[0]->online_reps.online_stake ());
}
...
TEST (node, block_confirm)
{
	...
	for (auto & type : types)
	{
		...
		badem::keypair key;
		system.wallet (1)->insert_adhoc (badem::test_genesis_key.prv);
		auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
		system.nodes[0]->block_processor.add (send1, badem::seconds_since_epoch ());
		system.nodes[1]->block_processor.add (send1, badem::seconds_since_epoch ());
		...
		auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio * 2, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (send1->hash ())));
		{
			auto transaction (system.nodes[0]->store.tx_begin_write ());
			ASSERT_EQ (badem::process_result::progress, system.nodes[0]->ledger.process (transaction, *send2).code);
		}
		{
			auto transaction (system.nodes[1]->store.tx_begin_write ());
			ASSERT_EQ (badem::process_result::progress, system.nodes[1]->ledger.process (transaction, *send2).code);
		}
...
TEST (node, block_arrival_size)
{
	badem::system system (24000, 1);
	auto & node (*system.nodes[0]);
	auto time (std::chrono::steady_clock::now () - badem::block_arrival::arrival_time_min - std::chrono::seconds (5));
	badem::block_hash hash (0);
	for (auto i (0); i < badem::block_arrival::arrival_size_min * 2; ++i)
	{
		node.block_arrival.arrival.insert (badem::block_arrival_info{ time, hash });
		++hash.qwords[0];
	}
	ASSERT_EQ (badem::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
	node.block_arrival.recent (0);
	ASSERT_EQ (badem::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
	...
	badem::uint128_union new_balance (system.nodes[0]->config.online_weight_minimum.number () - badem::kBDM_ratio);
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, new_balance, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, system.nodes[0]->ledger.process (transaction, *send1).code);
	}
	system.wallet (0)->send_action (badem::test_genesis_key.pub, badem::test_genesis_key.pub, new_balance.number ());
	system.deadline_set (10s);
	while (system.nodes[0]->active.empty ())
	{
		ASSERT_NO_ERROR (system.poll ());
	...
	while (!done)
	{
		...
		{
			std::lock_guard<std::mutex> guard (system.nodes[0]->active.mutex);
			auto info (system.nodes[0]->active.roots.find (badem::qualified_root (send1->hash (), send1->hash ())));
			ASSERT_NE (system.nodes[0]->active.roots.end (), info);
			done = info->election->confirmation_request_count > badem::active_transactions::minimum_confirmation_request_count;
		}
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (0, system.nodes[0]->balance (badem::test_genesis_key.pub));
}
...
TEST (node, local_votes_cache)
{
	...
	badem::genesis genesis;
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, node.work_generate_blocking (genesis.hash ())));
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::kBDM_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, node.work_generate_blocking (send1->hash ())));
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send2->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 3 * badem::kBDM_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, node.work_generate_blocking (send2->hash ())));
	{
		auto transaction (node.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node.ledger.process (transaction, *send1).code);
		ASSERT_EQ (badem::process_result::progress, node.ledger.process (transaction, *send2).code);
	}
	badem::confirm_req message1 (send1);
	badem::confirm_req message2 (send2);
	...
	}
	{
		...
		auto transaction (node.store.tx_begin_read ());
		auto current_vote (node.store.vote_current (transaction, badem::test_genesis_key.pub));
		ASSERT_EQ (current_vote->sequence, 2);
	}
	// Max cache
	{
		auto transaction (node.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node.ledger.process (transaction, *send3).code);
	}
	badem::confirm_req message3 (send3);
	for (auto i (0); i < 100; ++i)
	{
...
TEST (node, local_votes_cache_generate_new_vote)
{
	...
	ASSERT_EQ (send1->hash (), boost::get<badem::block_hash> (votes1[0]->blocks[0]));
	{
		std::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		auto transaction (node.store.tx_begin_read ());
		auto current_vote (node.store.vote_current (transaction, badem::test_genesis_key.pub));
	...
	std::vector<std::pair<badem::block_hash, badem::block_hash>> roots_hashes{ std::make_pair (send1->hash (), send1->root ()), std::make_pair (send2->hash (), send2->root ()) };
	badem::confirm_req message2 (roots_hashes);
	for (auto i (0); i < 100; ++i)
	{
		node.network.process_message (message2, channel);
	}
...
TEST (node, vote_republish)
{
	...
	system.wallet (1)->insert_adhoc (key2.prv);
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	...
	auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, send2));
	ASSERT_TRUE (system.nodes[0]->active.active (*send1));
	ASSERT_TRUE (system.nodes[1]->active.active (*send1));
	system.nodes[0]->vote_processor.vote (vote, std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system.nodes[0]->network.endpoint ()));
...
TEST (node, vote_by_hash_bundle)
{
	...
	system.nodes[0]->observers.vote.add ([&max_hashes](badem::transaction const & transaction, std::shared_ptr<badem::vote> vote_a, std::shared_ptr<badem::transport::channel> channel_a) {
	...
	for (int i = 1; i <= 200; i++)
	{
		auto send (std::make_shared<badem::send_block> (genesis.hash (), key1.pub, std::numeric_limits<badem::uint128_t>::max () - (system.nodes[0]->config.receive_minimum.number () * i), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
...
TEST (node, vote_by_hash_republish)
{
	...
	for (auto & type : types)
	{
		...
		auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, vote_blocks));
		ASSERT_TRUE (system.nodes[0]->active.active (*send1));
		ASSERT_TRUE (system.nodes[1]->active.active (*send1));
		system.nodes[0]->vote_processor.vote (vote, std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system.nodes[0]->network.endpoint ()));
		while (!system.nodes[0]->block (send2->hash ()))
...
TEST (node, vote_by_hash_epoch_block_republish)
{
	...
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto epoch1 (std::make_shared<badem::state_block> (badem::genesis_account, genesis.hash (), badem::genesis_account, badem::genesis_amount, system.nodes[0]->ledger.epoch_link, epoch_signer.prv, epoch_signer.pub, system.work.generate (genesis.hash ())));
	system.nodes[0]->process_active (send1);
	system.deadline_set (5s);
	while (!system.nodes[1]->block (send1->hash ()))
	{
...
TEST (node, epoch_conflict_confirm)
{
	...
	badem::node_flags node_flags;
	node_flags.delay_frontier_confirmation_height_updating = true;
	auto node0 = system.add_node (badem::node_config (24000, system.logging), node_flags);
	auto node1 = system.add_node (badem::node_config (24001, system.logging), node_flags);
	badem::keypair key;
	badem::genesis genesis;
	badem::keypair epoch_signer (badem::test_genesis_key);
	auto send (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - 1, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto open (std::make_shared<badem::state_block> (key.pub, 0, key.pub, 1, send->hash (), key.prv, key.pub, system.work.generate (key.pub)));
	auto change (std::make_shared<badem::state_block> (key.pub, open->hash (), key.pub, 1, 0, key.prv, key.pub, system.work.generate (open->hash ())));
	auto epoch (std::make_shared<badem::state_block> (change->root (), 0, 0, 0, node0->ledger.epoch_link, epoch_signer.prv, epoch_signer.pub, system.work.generate (open->hash ())));
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node0->block_processor.process_one (transaction, send).code);
		ASSERT_EQ (badem::process_result::progress, node0->block_processor.process_one (transaction, open).code);
	}
	{
		auto transaction (node1->store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node1->block_processor.process_one (transaction, send).code);
		ASSERT_EQ (badem::process_result::progress, node1->block_processor.process_one (transaction, open).code);
	}
...
TEST (node, fork_invalid_block_signature)
{
	...
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto send2_corrupt (std::make_shared<badem::send_block> (*send2));
	send2_corrupt->signature = badem::signature (123);
	system.nodes[0]->process_active (send1);
	system.deadline_set (5s);
	...
	auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, send2));
	auto vote_corrupt (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, send2_corrupt));
...
TEST (node, fork_invalid_block_signature_vote_by_hash)
{
	...
	auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, vote_blocks));
	{
		...
		system.nodes[0]->vote_processor.vote_blocking (transaction, vote, std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system.nodes[0]->network.endpoint ()));
	}
	while (system.nodes[0]->block (send1->hash ()))
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (node, block_processor_signatures)
{
	...
	badem::block_hash latest (system0.nodes[0]->latest (badem::test_genesis_key.pub));
	badem::keypair key1;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, latest, badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	badem::keypair key2;
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::kBDM_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	badem::keypair key3;
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send2->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 3 * badem::kBDM_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send3);
	// Invalid signature bit
	auto send4 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 4 * badem::kBDM_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send4);
	send4->signature.bytes[32] ^= 0x1;
	// Invalid signature bit (force)
	auto send5 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 5 * badem::kBDM_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	auto receive1 (std::make_shared<badem::state_block> (key1.pub, 0, badem::test_genesis_key.pub, badem::kBDM_ratio, send1->hash (), key1.prv, key1.pub, 0));
	node1.work_generate_blocking (*receive1);
	auto receive2 (std::make_shared<badem::state_block> (key2.pub, 0, badem::test_genesis_key.pub, badem::kBDM_ratio, send2->hash (), key2.prv, key2.pub, 0));
	node1.work_generate_blocking (*receive2);
	// Invalid private key
	auto receive3 (std::make_shared<badem::state_block> (key3.pub, 0, badem::test_genesis_key.pub, badem::kBDM_ratio, send3->hash (), key2.prv, key3.pub, 0));
...
TEST (node, block_processor_reject_state)
{
	...
	auto & node (*system.nodes[0]);
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	ASSERT_FALSE (node.ledger.block_exists (send1->hash ()));
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::kBDM_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
...
TEST (node, confirm_back)
{
	...
	auto & node (*system.nodes[0]);
	badem::genesis genesis;
	auto genesis_start_balance (node.balance (badem::test_genesis_key.pub));
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key.pub, genesis_start_balance - 1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (genesis.hash ())));
	auto open (std::make_shared<badem::state_block> (key.pub, 0, key.pub, 1, send1->hash (), key.prv, key.pub, system.work.generate (key.pub)));
	auto send2 (std::make_shared<badem::state_block> (key.pub, open->hash (), key.pub, 0, badem::test_genesis_key.pub, key.prv, key.pub, system.work.generate (open->hash ())));
	node.process_active (send1);
	...
	auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, vote_blocks));
	{
		...
		node.vote_processor.vote_blocking (transaction, vote, std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, node.network.endpoint ()));
	}
	system.deadline_set (10s);
	while (!node.active.empty ())
	{
...
TEST (node, peers)
{
	...
	badem::node_init init;
	auto node (std::make_shared<badem::node> (init, system.io_ctx, 24001, badem::unique_path (), system.alarm, system.logging, system.work));
	...
	auto & store = system.nodes.back ()->store;
	{
		...
		store.peer_put (transaction, badem::endpoint_key{ boost::asio::ip::address_v6::any ().to_bytes (), 55555 });
	}
...
TEST (node, peer_cache_restart)
{
	...
	{
		badem::node_init init;
		auto node (std::make_shared<badem::node> (init, system.io_ctx, 24001, path, system.alarm, system.logging, system.work));
		system.nodes.push_back (node);
	...
	// Restart node
	{
		...
		auto node (std::make_shared<badem::node> (init, system.io_ctx, 24002, path, system.alarm, system.logging, system.work, node_flags));
		system.nodes.push_back (node);
...
TEST (node, dont_write_lock_node)
{
	...
	std::thread ([&path, &write_lock_held_promise, &finished_promise]() {
		...
		bool init (false);
		badem::mdb_store store (init, logger, path / "data.ldb");
		badem::genesis genesis;
		{
			auto transaction (store.tx_begin_write ());
...
TEST (node, bidirectional_tcp)
{
	...
	badem::genesis genesis;
	badem::keypair key;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::kBDM_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, node1->work_generate_blocking (genesis.hash ())));
	node1->process_active (send1);
	node1->block_processor.flush ();
	system.deadline_set (5s);
	...
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::kBDM_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, node1->work_generate_blocking (send1->hash ())));
	node2->process_active (send2);
	node2->block_processor.flush ();
	system.deadline_set (5s);
...
TEST (confirmation_height, prioritize_frontiers)
{
	...
	badem::node_flags node_flags;
	node_flags.delay_frontier_confirmation_height_updating = true;
	auto node = system.add_node (badem::node_config (24001, system.logging), node_flags);
	...
	badem::send_block send1 (latest1, key1.pub, node->config.online_weight_minimum.number () + 10000, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (latest1));
	badem::send_block send2 (send1.hash (), key1.pub, node->config.online_weight_minimum.number () + 8500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send1.hash ()));
	badem::send_block send3 (send2.hash (), key1.pub, node->config.online_weight_minimum.number () + 8000, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send2.hash ()));
	badem::send_block send4 (send3.hash (), key2.pub, node->config.online_weight_minimum.number () + 7500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send3.hash ()));
	badem::send_block send5 (send4.hash (), key3.pub, node->config.online_weight_minimum.number () + 6500, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send4.hash ()));
	badem::send_block send6 (send5.hash (), key4.pub, node->config.online_weight_minimum.number () + 6000, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send5.hash ()));

	// Open all accounts and add other sends to get different uncemented counts (as well as some which are the same)
	badem::open_block open1 (send1.hash (), badem::genesis_account, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub));
	badem::send_block send7 (open1.hash (), badem::test_genesis_key.pub, 500, key1.prv, key1.pub, system.work.generate (open1.hash ()));

	badem::open_block open2 (send4.hash (), badem::genesis_account, key2.pub, key2.prv, key2.pub, system.work.generate (key2.pub));

	badem::open_block open3 (send5.hash (), badem::genesis_account, key3.pub, key3.prv, key3.pub, system.work.generate (key3.pub));
	badem::send_block send8 (open3.hash (), badem::test_genesis_key.pub, 500, key3.prv, key3.pub, system.work.generate (open3.hash ()));
	badem::send_block send9 (send8.hash (), badem::test_genesis_key.pub, 200, key3.prv, key3.pub, system.work.generate (send8.hash ()));

	badem::open_block open4 (send6.hash (), badem::genesis_account, key4.pub, key4.prv, key4.pub, system.work.generate (key4.pub));
	badem::send_block send10 (open4.hash (), badem::test_genesis_key.pub, 500, key4.prv, key4.pub, system.work.generate (open4.hash ()));
	badem::send_block send11 (send10.hash (), badem::test_genesis_key.pub, 200, key4.prv, key4.pub, system.work.generate (send10.hash ()));

	{
		...
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send2).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send3).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send4).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send5).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send6).code);

		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, open1).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send7).code);

		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, open2).code);

		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, open3).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send8).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send9).code);

		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, open4).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send10).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send11).code);
	}
	...
	auto priority_orders_match = [](auto const & cementable_frontiers, auto const & desired_order) {
		return std::equal (desired_order.begin (), desired_order.end (), cementable_frontiers.template get<1> ().begin (), cementable_frontiers.template get<1> ().end (), [](badem::account const & account, badem::cementable_account const & cementable_account) {
	...
	// clang-format on
	{
		...
		std::array<badem::account, num_accounts> desired_order_1{ badem::genesis_account, key3.pub, key4.pub, key1.pub, key2.pub };
		std::array<badem::account, num_accounts> desired_order_2{ badem::genesis_account, key4.pub, key3.pub, key1.pub, key2.pub };
...

	{
		...
		std::array<badem::account, 3> local_desired_order{ badem::genesis_account, key1.pub, key2.pub };
		ASSERT_TRUE (priority_orders_match (node->active.priority_wallet_cementable_frontiers, local_desired_order));
		std::array<badem::account, 2> desired_order_1{ key3.pub, key4.pub };
		std::array<badem::account, 2> desired_order_2{ key4.pub, key3.pub };
		ASSERT_TRUE (priority_orders_match (node->active.priority_cementable_frontiers, desired_order_1) || priority_orders_match (node->active.priority_cementable_frontiers, desired_order_2));
	}
	...
	badem::send_block send12 (send9.hash (), badem::test_genesis_key.pub, 100, key3.prv, key3.pub, system.work.generate (send9.hash ()));
	badem::send_block send13 (send12.hash (), badem::test_genesis_key.pub, 90, key3.prv, key3.pub, system.work.generate (send12.hash ()));
	badem::send_block send14 (send13.hash (), badem::test_genesis_key.pub, 80, key3.prv, key3.pub, system.work.generate (send13.hash ()));
	badem::send_block send15 (send14.hash (), badem::test_genesis_key.pub, 70, key3.prv, key3.pub, system.work.generate (send14.hash ()));
	badem::send_block send16 (send15.hash (), badem::test_genesis_key.pub, 60, key3.prv, key3.pub, system.work.generate (send15.hash ()));
	badem::send_block send17 (send16.hash (), badem::test_genesis_key.pub, 50, key3.prv, key3.pub, system.work.generate (send16.hash ()));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send12).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send13).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send14).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send15).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send16).code);
		ASSERT_EQ (badem::process_result::progress, node->ledger.process (transaction, send17).code);
	}
	transaction.refresh ();
	node->active.prioritize_frontiers_for_confirmation (transaction, std::chrono::seconds (1), std::chrono::seconds (1));
	ASSERT_TRUE (priority_orders_match (node->active.priority_wallet_cementable_frontiers, std::array<badem::account, num_accounts>{ key3.pub, badem::genesis_account, key4.pub, key1.pub, key2.pub }));
	...
	std::array<badem::qualified_root, num_accounts> frontiers{ send17.qualified_root (), send6.qualified_root (), send7.qualified_root (), open2.qualified_root (), send11.qualified_root () };
...
TEST (active_difficulty, recalculate_work)
{
	...
	ASSERT_EQ (node1.active.active_difficulty (), badem::difficulty::from_multiplier (sum / node1.active.multipliers_cb.size (), node1.network_params.network.publish_threshold));
	std::unique_lock<std::mutex> lock (node1.active.mutex);
	...
	badem::work_validate (*send1, &difficulty2);
	node1.process_active (send1);
...
void add_required_children_node_config_tree (badem::jsonconfig & tree)
{
	...
	badem::jsonconfig preconfigured_representatives_l;
	tree.put_child ("preconfigured_representatives", preconfigured_representatives_l);
	badem::jsonconfig work_peers_l;
	tree.put_child ("work_peers", work_peers_l);
	tree.put ("version", std::to_string (badem::node_config::json_version ()));
}
}
