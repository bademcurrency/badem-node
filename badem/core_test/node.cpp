...
TEST (node, block_store_path_failure)
{
	...
	auto node (std::make_shared<badem::node> (*service, 24000, path, alarm, logging, work));
...
TEST (node, password_fanout)
{
	...
	auto node (std::make_shared<badem::node> (*service, path, alarm, config, work));
...
TEST (node, node_receive_quorum)
{
	...
	badem::block_hash previous (system.nodes[0]->latest (badem::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (std::make_shared<badem::send_block> (previous, key.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (previous)));
	system.nodes[0]->process_active (send);
	system.deadline_set (10s);
	...
	{
		{
			badem::lock_guard<std::mutex> guard (system.nodes[0]->active.mutex);
	...
	auto channel (std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system2.nodes[0]->network.endpoint (), system.nodes[0]->network_params.protocol.protocol_version));
...
TEST (node, auto_bootstrap)
{
	...
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
...
TEST (node, auto_bootstrap_reverse)
{
	...
	ASSERT_FALSE (node1->init_error ());
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
	auto channel (std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, node1->network.endpoint (), system.nodes[0]->network_params.protocol.protocol_version));
	system.nodes[0]->network.send_keepalive (channel);
	node1->start ();
	system.nodes.push_back (node1);
	system.deadline_set (10s);
...
TEST (node, unlock_search)
{
	...
	system.wallet (0)->insert_adhoc (key2.prv);
	{
		badem::lock_guard<std::recursive_mutex> lock (system.wallet (0)->store.mutex);
		system.wallet (0)->store.password.value_set (badem::keypair ().prv);
	}
	auto node (system.nodes[0]);
	{
...
TEST (node, connect_after_junk)
{
	...
	channel1->send_buffer (badem::shared_const_buffer (std::move (junk_buffer)), badem::stat::detail::bulk_pull, [](boost::system::error_code const &, size_t) {});
	system.deadline_set (10s);
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel2 (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, system.nodes[0]->network.endpoint (), node1->network_params.protocol.protocol_version));
...
{
	badem::system system (24000, 1);
	auto price1 (system.nodes[0]->price (badem::Gbdm_ratio, 1));
	ASSERT_EQ (badem::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (badem::Gbdm_ratio * int(badem::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (badem::Gbdm_ratio * int(badem::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (badem::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (badem::Gbdm_ratio * int(badem::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node_config, v15_v16_upgrade)
{
	...
	test_upgrade ("rai.raiblocks.net", "node.badem.io");
	test_upgrade ("rai-beta.raiblocks.net", "peering-beta.badem.io");
}
...
TEST (node_config, v16_values)
{
	...
	tree.put ("vote_minimum", badem::Gbdm_ratio.convert_to<std::string> ());
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_FALSE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 1);
	ASSERT_EQ (config.vote_minimum.number (), badem::Gbdm_ratio);
...
TEST (node_flags, disable_udp)
{
	...
	auto node2 (std::make_shared<badem::node> (system.io_ctx, badem::unique_path (), system.alarm, badem::node_config (24001, system.logging), system.work, node_flags));
	system.nodes.push_back (node2);
	node2->start ();
	// Send UDP message
	auto channel (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, node2->network.endpoint (), node2->network_params.protocol.protocol_version));
	node1->network.send_keepalive (channel);
...
TEST (node, fork_keep)
{
	...
	node2.block_processor.flush ();
	badem::unique_lock<std::mutex> lock (node2.active.mutex);
...
TEST (node, fork_bootstrap_flip)
{
	...
	badem::keypair key1;
	auto send1 (std::make_shared<badem::send_block> (latest, key1.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system0.work.generate (latest)));
	badem::keypair key2;
	auto send2 (std::make_shared<badem::send_block> (latest, key2.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system0.work.generate (latest)));
...
TEST (node, fork_no_vote_quorum)
{
	...
	badem::transport::channel_udp channel (node2.network.udp_channels, node3.network.endpoint (), node1.network_params.protocol.protocol_version);
	channel.send_buffer (badem::shared_const_buffer (std::move (buffer)), badem::stat::detail::confirm_ack);
...
TEST (node, DISABLED_fork_stale)
{
	...
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	auto channel (std::make_shared<badem::transport::channel_udp> (node2.network.udp_channels, node1.network.endpoint (), node2.network_params.protocol.protocol_version));
	node2.rep_crawler.response (channel, badem::test_genesis_key.pub, badem::genesis_amount);
	badem::genesis genesis;
	badem::keypair key1;
	badem::keypair key2;
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Mbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Mbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Mbdm_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	{
...
TEST (node, broadcast_elected)
{
	...
	for (auto & type : types)
	{
		...
		//std::cerr << "Other: " << rep_other.pub.to_account () << std::endl;
		{
			...
			auto transaction1 (node1->store.tx_begin_write ());
			auto transaction2 (node2->store.tx_begin_write ());
			badem::send_block fund_big (node0->ledger.latest (transaction0, badem::test_genesis_key.pub), rep_big.pub, badem::Gbdm_ratio * 5, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
			badem::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
			badem::send_block fund_small (fund_big.hash (), rep_small.pub, badem::Gbdm_ratio * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
			badem::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
			badem::send_block fund_other (fund_small.hash (), rep_other.pub, badem::Gbdm_ratio * 1, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
...
TEST (node, rep_self_vote)
{
	...
	active.start (block0);
	badem::unique_lock<std::mutex> lock (active.mutex);
...
TEST (node, bootstrap_fork_open)
{
	...
	badem::node_config node_config (24000, system0.logging);
	node_config.frontiers_confirmation = badem::frontiers_confirmation_mode::disabled;
...
TEST (node, bootstrap_confirm_frontiers)
{
	...
	{
		badem::lock_guard<std::mutex> guard (node1->active.mutex);
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	auto send1 (wallet0->send_action (badem::genesis_account, key0.pub, 2 * badem::Mbdm_ratio));
	system.deadline_set (10s);
	while (node1.balance (key0.pub) != 2 * badem::Mbdm_ratio || node1.bootstrap_initiator.in_progress ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto latest (node1.latest (key0.pub));
	badem::state_block send2 (key0.pub, latest, badem::genesis_account, badem::Mbdm_ratio, badem::genesis_account, key0.prv, key0.pub, *node0.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (badem::process_result::progress, node1.ledger.process (transaction, send2).code);
	}
	auto send3 (wallet1->send_action (key0.pub, badem::genesis_account, badem::Mbdm_ratio));
	system.deadline_set (10s);
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (badem::test_genesis_key.pub, key1.pub, badem::Mbdm_ratio);
...
TEST (node, rep_weight)
{
	...
	auto channel0 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint0, node.network_params.protocol.protocol_version));
	auto channel1 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint1, node.network_params.protocol.protocol_version));
	auto channel2 (std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, endpoint2, node.network_params.protocol.protocol_version));
...
TEST (node, rep_remove)
{
	...
	auto node2 (std::make_shared<badem::node> (system.io_ctx, badem::unique_path (), system.alarm, badem::node_config (24003, system.logging), system.work));
	...
	node.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w, &done](std::shared_ptr<badem::transport::channel> channel2) {
		...
		{
			badem::keypair keypair2;
			node_l->rep_crawler.response (channel2, keypair2.pub, badem::Gbdm_ratio);
...
TEST (node, no_voting)
{
	...
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (badem::test_genesis_key.pub, key1.pub, badem::Mbdm_ratio);
	system.deadline_set (10s);
...
TEST (node, vote_replay)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		badem::lock_guard<std::mutex> lock (system.nodes[0]->store.get_cache_mutex ());
...
TEST (node, block_confirm)
{
	...
	for (auto & type : types)
	{
		...
		badem::keypair key;
		system.wallet (1)->insert_adhoc (badem::test_genesis_key.prv);
		auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (genesis.hash ())));
		...
		auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio * 2, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (send1->hash ())));
		{
			auto transaction (system.nodes[0]->store.tx_begin_write ());
...
TEST (node, confirm_quorum)
{
	...
	badem::amount new_balance (system.nodes[0]->config.online_weight_minimum.number () - badem::Gbdm_ratio);
...
TEST (node, local_votes_cache)
{
	...
	badem::genesis genesis;
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *node.work_generate_blocking (genesis.hash ())));
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *node.work_generate_blocking (send1->hash ())));
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send2->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 3 * badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *node.work_generate_blocking (send2->hash ())));
	{
	...
	{
		badem::lock_guard<std::mutex> lock (node.store.get_cache_mutex ());
		auto transaction (node.store.tx_begin_read ());
...
TEST (node, local_votes_cache_generate_new_vote)
{
	...
	std::vector<std::pair<badem::block_hash, badem::root>> roots_hashes{ std::make_pair (send1->hash (), send1->root ()), std::make_pair (send2->hash (), send2->root ()) };
...
TEST (node, vote_by_hash_bundle)
{
	...
	system.nodes[0]->observers.vote.add ([&max_hashes](std::shared_ptr<badem::vote> vote_a, std::shared_ptr<badem::transport::channel> channel_a) {
...
TEST (node, vote_by_hash_epoch_block_republish)
{
	...
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::send_block> (genesis.hash (), key2.pub, std::numeric_limits<badem::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto epoch1 (std::make_shared<badem::state_block> (badem::genesis_account, genesis.hash (), badem::genesis_account, badem::genesis_amount, system.nodes[0]->ledger.epoch_link (badem::epoch::epoch_1), badem::test_genesis_key.prv, badem::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	system.nodes[0]->process_active (send1);
	system.deadline_set (5s);
	while (!system.nodes[1]->block (send1->hash ()))
	{
...
TEST (node, epoch_conflict_confirm)
{
	...
	auto epoch (std::make_shared<badem::state_block> (change->root (), 0, 0, 0, node0->ledger.epoch_link (badem::epoch::epoch_1), epoch_signer.prv, epoch_signer.pub, *system.work.generate (open->hash ())));
	{
		auto transaction (node0->store.tx_begin_write ());
	...
	{
		badem::lock_guard<std::mutex> lock (node0->active.mutex);
...
TEST (node, fork_invalid_block_signature_vote_by_hash)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		badem::unique_lock<std::mutex> lock (system.nodes[0]->active.mutex);
		system.nodes[0]->vote_processor.vote_blocking (transaction, vote, std::make_shared<badem::transport::channel_udp> (system.nodes[0]->network.udp_channels, system.nodes[0]->network.endpoint (), system.nodes[0]->network_params.protocol.protocol_version));
	}
	while (system.nodes[0]->block (send1->hash ()))
	{
		ASSERT_NO_ERROR (system.poll ());
...
TEST (node, block_processor_signatures)
{
	...
	badem::block_hash latest (system0.nodes[0]->latest (badem::test_genesis_key.pub));
	badem::keypair key1;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, latest, badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key1.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send1);
	badem::keypair key2;
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, key2.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send2);
	badem::keypair key3;
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send2->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 3 * badem::Gbdm_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send3);
	// Invalid signature bit
	auto send4 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 4 * badem::Gbdm_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*send4);
	send4->signature.bytes[32] ^= 0x1;
	// Invalid signature bit (force)
	auto send5 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send3->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 5 * badem::Gbdm_ratio, key3.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	auto receive1 (std::make_shared<badem::state_block> (key1.pub, 0, badem::test_genesis_key.pub, badem::Gbdm_ratio, send1->hash (), key1.prv, key1.pub, 0));
	node1.work_generate_blocking (*receive1);
	auto receive2 (std::make_shared<badem::state_block> (key2.pub, 0, badem::test_genesis_key.pub, badem::Gbdm_ratio, send2->hash (), key2.prv, key2.pub, 0));
	node1.work_generate_blocking (*receive2);
	// Invalid private key
	auto receive3 (std::make_shared<badem::state_block> (key3.pub, 0, badem::test_genesis_key.pub, badem::Gbdm_ratio, send3->hash (), key2.prv, key3.pub, 0));
...
TEST (node, block_processor_reject_state)
{
	...
	auto & node (*system.nodes[0]);
	badem::genesis genesis;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	...
	ASSERT_FALSE (node.ledger.block_exists (send1->hash ()));
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
...
TEST (node, block_processor_full)
{
	...
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node.work_generate_blocking (*send1);
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node.work_generate_blocking (*send2);
	auto send3 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send2->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 3 * badem::Gbdm_ratio, badem::test_genesis_key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
	node.work_generate_blocking (*send3);
	// The write guard prevents block processor doing any writes
	auto write_guard = node.write_database_queue.wait (badem::writer::confirmation_height);
	node.block_processor.add (send1);
...
TEST (node, confirm_back)
{
	...
	auto vote (std::make_shared<badem::vote> (badem::test_genesis_key.pub, badem::test_genesis_key.prv, 0, vote_blocks));
	{
		...
		node.vote_processor.vote_blocking (transaction, vote, std::make_shared<badem::transport::channel_udp> (node.network.udp_channels, node.network.endpoint (), node.network_params.protocol.protocol_version));
	}
	system.deadline_set (10s);
	while (!node.active.empty ())
	{
...
TEST (node, peers)
{
	...
	auto node (std::make_shared<badem::node> (system.io_ctx, 24001, badem::unique_path (), system.alarm, system.logging, system.work));
...
TEST (node, peer_cache_restart)
{
	...
	auto path (badem::unique_path ());
	{
		auto node (std::make_shared<badem::node> (system.io_ctx, 24001, path, system.alarm, system.logging, system.work));
		system.nodes.push_back (node);
	...
	// Restart node
	{
		...
		auto node (std::make_shared<badem::node> (system.io_ctx, 24002, path, system.alarm, system.logging, system.work, node_flags));
		system.nodes.push_back (node);
...
TEST (node, dont_write_lock_node)
{
	...
	std::thread ([&path, &write_lock_held_promise, &finished_promise]() {
		...
		auto store = badem::make_store (logger, path, false, true);
		{
			badem::genesis genesis;
...
TEST (node, bidirectional_tcp)
{
	...
	badem::genesis genesis;
	badem::keypair key;
	auto send1 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, genesis.hash (), badem::test_genesis_key.pub, badem::genesis_amount - badem::Gbdm_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *node1->work_generate_blocking (genesis.hash ())));
	node1->process_active (send1);
	node1->block_processor.flush ();
	system.deadline_set (5s);
	...
	auto send2 (std::make_shared<badem::state_block> (badem::test_genesis_key.pub, send1->hash (), badem::test_genesis_key.pub, badem::genesis_amount - 2 * badem::Gbdm_ratio, key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, *node1->work_generate_blocking (send1->hash ())));
	node2->process_active (send2);
	node2->block_processor.flush ();
	system.deadline_set (5s);
...
TEST (active_difficulty, recalculate_work)
{
	...
	ASSERT_EQ (node1.active.trended_active_difficulty, badem::difficulty::from_multiplier (sum / node1.active.multipliers_cb.size (), node1.network_params.network.publish_threshold));
	lock.unlock ();
}
