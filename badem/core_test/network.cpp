...
TEST (network, last_contacted)
{
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel1 (std::make_shared<badem::transport::channel_udp> (node1->network.udp_channels, badem::endpoint (boost::asio::ip::address_v6::loopback (), 24000)));
...
TEST (network, multi_keepalive)
{
	...
	auto channel2 (std::make_shared<badem::transport::channel_udp> (node2->network.udp_channels, system.nodes[0]->network.endpoint ()));
...
TEST (network, send_insufficient_work)
{
	...
	badem::transport::channel_udp channel (system.nodes[0]->network.udp_channels, system.nodes[1]->network.endpoint ());
...
TEST (bootstrap_processor, lazy_hash)
{
	...
	auto node1 (std::make_shared<badem::node> (init1, system.io_ctx, 24001, badem::unique_path (), system.alarm, system.logging, system.work));
	node1->network.udp_channels.insert (system.nodes[0]->network.endpoint (), badem::protocol_version);
...
TEST (confirmation_height, multiple_accounts)
{
	...
	system.add_node (badem::node_config (24001, system.logging), node_flags);
	system.add_node (badem::node_config (24002, system.logging), node_flags);
	badem::keypair key1;
	badem::keypair key2;
	badem::keypair key3;
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
...
TEST (confirmation_height, gap_bootstrap)
{
	...
	auto send3 (std::make_shared<badem::state_block> (badem::genesis_account, send2->hash (), badem::genesis_account, badem::genesis_amount - 3 * badem::kBDM_ratio, destination.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0));
...
TEST (confirmation_height, all_block_types)
{
	...
	badem::keypair key2;
	auto & store = node->store;
	badem::send_block send (latest, key1.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (latest));
	badem::send_block send1 (send.hash (), key2.pub, badem::genesis_amount - badem::kBDM_ratio * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send.hash ()));

	badem::open_block open (send.hash (), badem::test_genesis_key.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub));
	badem::state_block state_open (key2.pub, 0, 0, badem::kBDM_ratio, send1.hash (), key2.prv, key2.pub, system.work.generate (key2.pub));

	badem::send_block send2 (open.hash (), key2.pub, 0, key1.prv, key1.pub, system.work.generate (open.hash ()));
	badem::state_block state_receive (key2.pub, state_open.hash (), 0, badem::kBDM_ratio * 2, send2.hash (), key2.prv, key2.pub, system.work.generate (state_open.hash ()));

	badem::state_block state_send (key2.pub, state_receive.hash (), 0, badem::kBDM_ratio, key1.pub, key2.prv, key2.pub, system.work.generate (state_receive.hash ()));
	...
	badem::state_block state_change (key2.pub, state_send.hash (), badem::test_genesis_key.pub, badem::kBDM_ratio, 0, key2.prv, key2.pub, system.work.generate (state_send.hash ()));

	badem::keypair epoch_key;
	node->ledger.epoch_signer = epoch_key.pub;

	badem::state_block epoch (key2.pub, state_change.hash (), badem::test_genesis_key.pub, badem::kBDM_ratio, node->ledger.epoch_link, epoch_key.prv, epoch_key.pub, system.work.generate (state_change.hash ()));

	badem::state_block epoch1 (key1.pub, change.hash (), key2.pub, badem::kBDM_ratio, node->ledger.epoch_link, epoch_key.prv, epoch_key.pub, system.work.generate (change.hash ()));
	badem::state_block state_send1 (key1.pub, epoch1.hash (), 0, badem::kBDM_ratio - 1, key2.pub, key1.prv, key1.pub, system.work.generate (epoch1.hash ()));
	badem::state_block state_receive2 (key2.pub, epoch.hash (), 0, badem::kBDM_ratio + 1, state_send1.hash (), key2.prv, key2.pub, system.work.generate (epoch.hash ()));

	auto state_send2 = std::make_shared<badem::state_block> (key2.pub, state_receive2.hash (), 0, badem::kBDM_ratio, key1.pub, key2.prv, key2.pub, system.work.generate (state_receive2.hash ()));
	badem::state_block state_send3 (key2.pub, state_send2->hash (), 0, badem::kBDM_ratio - 1, key1.pub, key2.prv, key2.pub, system.work.generate (state_send2->hash ()));

	badem::state_block state_send4 (key1.pub, state_send1.hash (), 0, badem::kBDM_ratio - 2, badem::test_genesis_key.pub, key1.prv, key1.pub, system.work.generate (state_send1.hash ()));
	badem::state_block state_receive3 (badem::genesis_account, send1.hash (), badem::genesis_account, badem::genesis_amount - badem::kBDM_ratio * 2 + 1, state_send4.hash (), badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send1.hash ()));
...
TEST (confirmation_height, modified_chain)
{
	...
	badem::keypair key1;
	auto & store = node->store;
	auto send = std::make_shared<badem::send_block> (latest, key1.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (latest));
...

	{
		...
		auto write_guard = node->write_database_queue.wait (badem::writer::process_batch);
...
TEST (confirmation_height, pending_observer_callbacks)
{
	...
	badem::keypair key1;
	badem::send_block send (latest, key1.pub, badem::genesis_amount - badem::kBDM_ratio, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (latest));
	auto send1 = std::make_shared<badem::send_block> (send.hash (), key1.pub, badem::genesis_amount - badem::kBDM_ratio * 2, badem::test_genesis_key.prv, badem::test_genesis_key.pub, system.work.generate (send.hash ()));
...
TEST (network, replace_port)
{
	...
	system.nodes.push_back (node1);
	{
		auto channel (system.nodes[0]->network.udp_channels.insert (badem::endpoint (node1->network.endpoint ().address (), 23000), badem::protocol_version));
		if (channel)
		{
