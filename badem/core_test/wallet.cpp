...
TEST (wallet, one_item_iteration)
{
	...
	{
		ASSERT_EQ (key1.pub, badem::uint256_union (i->first));
...
TEST (wallet, two_item_iteration)
{
	...
	std::unordered_set<badem::public_key> pubs;
	std::unordered_set<badem::private_key> prvs;
	badem::kdf kdf;
	{
		auto transaction (env.tx_begin_write ());
		badem::wallet_store wallet (init, kdf, transaction, badem::genesis_account, 1, "0");
...
TEST (wallet, insufficient_spend_one)
{
	...
	auto block (system.wallet (0)->send_action (badem::test_genesis_key.pub, key1.pub, 500));
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key1.pub, badem::genesis_amount));
}
...
TEST (wallet, spend_all_one)
{
	...
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, std::numeric_limits<badem::uint128_t>::max ()));
	badem::account_info info2;
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		system.nodes[0]->store.account_get (transaction, badem::test_genesis_key.pub, info2);
...
TEST (wallet, send_async)
{
	...
	boost::thread thread ([&system]() {
		...
		while (!system.nodes[0]->balance (badem::test_genesis_key.pub).is_zero ())
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	});
	std::atomic<bool> success (false);
	system.wallet (0)->send_async (badem::test_genesis_key.pub, key2.pub, std::numeric_limits<badem::uint128_t>::max (), [&success](std::shared_ptr<badem::block> block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
	ASSERT_TRUE (success);
}
...
TEST (wallet, partial_spend)
{
	...
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv);
	badem::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<badem::uint128_t>::max () - 500, system.nodes[0]->balance (badem::test_genesis_key.pub));
}
...
{
	badem::uint256_union value0 (0);
	badem::fan fan (value0, 1024);
	for (auto & i : fan.values)
	{
		ASSERT_NE (value0, *i);
	}
	badem::raw_key value1;
	fan.value (value1);
...
TEST (fan, change)
{
	badem::raw_key value0;
	value0.data = 0;
	badem::raw_key value1;
	value1.data = 1;
	ASSERT_NE (value0, value1);
	badem::fan fan (value0.data, 1024);
	ASSERT_EQ (1024, fan.values.size ());
	badem::raw_key value2;
	fan.value (value2);
...
{
	auto error (false);
	badem::mdb_env env (error, badem::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	badem::kdf kdf;
	badem::wallet_store wallet (error, kdf, transaction, badem::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (badem::genesis_account, wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	badem::keypair key;
...
TEST (wallet, serialize_json_empty)
{
	...
	auto transaction (env.tx_begin_write ());
	badem::kdf kdf;
	badem::wallet_store wallet1 (error, kdf, transaction, badem::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	badem::wallet_store wallet2 (error, kdf, transaction, badem::genesis_account, 1, "1", serialized);
	ASSERT_FALSE (error);
	badem::raw_key password1;
	badem::raw_key password2;
...
TEST (wallet_store, move)
{
	...
	badem::wallet_store wallet2 (error, kdf, transaction, badem::genesis_account, 1, "1");
	ASSERT_FALSE (error);
	badem::keypair key2;
	...
	std::vector<badem::public_key> keys;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		...
		uint64_t work (0);
		if (!wallet->store.work_get (transaction, badem::test_genesis_key.pub, work))
		{
			done = !badem::work_validate (genesis.hash (), work);
		}
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, work_generate)
{
	badem::system system (24000, 1);
	auto wallet (system.wallet (0));
	badem::uint128_t amount1 (system.nodes[0]->balance (badem::test_genesis_key.pub));
	uint64_t work1;
	wallet->insert_adhoc (badem::test_genesis_key.prv);
	...
	badem::keypair key;
	wallet->send_action (badem::test_genesis_key.pub, key.pub, 100);
	system.deadline_set (10s);
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	while (system.nodes[0]->ledger.account_balance (transaction, badem::test_genesis_key.pub) == amount1)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.deadline_set (10s);
	...
	while (again)
	{
		...
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		again = wallet->store.work_get (transaction, account1, work1) || badem::work_validate (system.nodes[0]->ledger.latest_root (block_transaction, account1), work1);
	}
}
...
TEST (wallet, insert_locked)
{
	...
	ASSERT_TRUE (wallet->insert_adhoc (badem::keypair ().prv).is_zero ());
}
...
TEST (wallet, version_1_upgrade)
{
	...
	badem::wallet_value value (wallet->store.entry_get_raw (transaction, badem::wallet_store::wallet_key_special));
	badem::raw_key kdf;
	kdf.data.clear ();
	...
	wallet->store.entry_put_raw (transaction, key.pub, badem::wallet_value (ciphertext, 0));
	...
	ASSERT_EQ (key.prv, prv);
	value = wallet->store.entry_get_raw (transaction, badem::wallet_store::wallet_key_special);
...
TEST (wallet, version_2_upgrade)
{
	...
	wallet->store.erase (transaction, badem::wallet_store::deterministic_index_special);
...
TEST (wallet, version_3_upgrade)
{
	...
	wallet->store.entry_put_raw (transaction, badem::wallet_store::seed_special, badem::wallet_value (seed_ciphertext, 0));
...
{
	badem::system system (24000, 1);
	system.wallet (0)->insert_adhoc (badem::test_genesis_key.prv, false);
	badem::keypair key2;
	auto block (system.wallet (0)->send_action (badem::test_genesis_key.pub, key2.pub, std::numeric_limits<badem::uint128_t>::max (), false));
	ASSERT_NE (nullptr, block);
	ASSERT_NE (0, block->block_work ());
	ASSERT_FALSE (badem::work_validate (block->root (), block->block_work ()));
	auto transaction (system.wallet (0)->wallets.tx_begin_read ());
	uint64_t cached_work (0);
	system.wallet (0)->store.work_get (transaction, badem::test_genesis_key.pub, cached_work);
