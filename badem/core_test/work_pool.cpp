...
TEST (work, opencl)
{
	...
	{
		auto opencl (badem::opencl_work::create (true, { 0, 0, 16 * 1024 }, logger));
		if (opencl != nullptr)
		{
			badem::work_pool pool (std::numeric_limits<unsigned>::max (), std::chrono::nanoseconds (0), opencl ? [&opencl](badem::uint256_union const & root_a, uint64_t difficulty_a) {
