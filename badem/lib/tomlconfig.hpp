...
#include <badem/lib/configbase.hpp>
...
class tomlconfig : public badem::configbase
{
public:
	tomlconfig () :
	tree (cpptoml::make_table ())
	{
		error = std::make_shared<badem::error> ();
	}

	tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<badem::error> const & error_a = nullptr) :
	badem::configbase (error_a), tree (tree_a)
	{
		if (!error)
		{
			error = std::make_shared<badem::error> ();
	 ...
	 * @return badem::error&, including a descriptive error message if the config file is malformed.
	 */
	badem::error & read (boost::filesystem::path const & path_a)
	{
	...
	badem::error & read (std::istream & stream_overrides, boost::filesystem::path const & path_a)
	{
		std::fstream stream;
	...
	void open_or_create (std::fstream & stream_a, std::string const & path_a)
	{
		if (!boost::filesystem::exists (path_a))
		{
			...
			badem::set_secure_perm_file (path_a);
		}
	...
	tomlconfig get_required_child (std::string const & key_a)
	{
		if (!tree->contains (key_a))
		{
			*error = badem::error_config::missing_value;
	...
	tomlconfig & put_child (std::string const & key_a, badem::tomlconfig & conf_a)
	{
		tree->insert (key_a, conf_a.get_tree ());
		return *this;
	}

	tomlconfig & replace_child (std::string const & key_a, badem::tomlconfig & conf_a)
	{
		tree->erase (key_a);
	...
	tomlconfig & array_entries_required (std::string const & key, std::function<void(T)> callback)
	{
		...
		{
			conditionally_set_error<T> (badem::error_config::missing_value, false, key);
		}
		return *this;
	}
	 ...
	 * @return May return badem::error_config::invalid_value
	 */
	template <typename T>
	...
	/**
	 * Get required value.
	 * @note May set badem::error_config::missing_value if \p key is missing, badem::error_config::invalid_value if value is invalid.
	 */
	template <typename T>
...
protected:
	template <typename T, typename = std::enable_if_t<badem::is_lexical_castable<T>::value>>
	tomlconfig & get_config (bool optional, std::string const & key, T & target, T default_value = T ())
	{
		try
		{
			if (tree->contains_qualified (key))
			{
				...
				{
					conditionally_set_error<T> (badem::error_config::invalid_value, optional, key);
				}
			}
			else if (!optional)
			{
				conditionally_set_error<T> (badem::error_config::missing_value, optional, key);
			}
			else
			{
