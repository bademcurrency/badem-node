...
inline badem::build_flags operator| (badem::build_flags a, badem::build_flags b)
{
	return static_cast<badem::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, badem::build_flags b)
{
...
public:
	...
	inline abstract_builder & sign (badem::raw_key const & private_key, badem::public_key const & public_key)
	{
		block->signature = badem::sign_message (private_key, public_key, block->hash ());
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (badem::build_flags::work_present | badem::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<badem::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (badem::state_block const & block);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (badem::build_flags::account_present | badem::build_flags::balance_present | badem::build_flags::link_present | badem::build_flags::previous_present | badem::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<badem::open_block, open_block_builder>
{
public:
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (badem::build_flags::account_present | badem::build_flags::representative_present | badem::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<badem::change_block, change_block_builder>
{
public:
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (badem::build_flags::previous_present | badem::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<badem::send_block, send_block_builder>
{
public:
	...
	/** Set balance */
	send_block_builder & balance (badem::amount balance);
	/** Set balance from decimal string */
...
class receive_block_builder : public abstract_builder<badem::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (badem::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (badem::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	inline badem::state_block_builder & state ()
	{
	...
	inline badem::open_block_builder & open ()
	{
	...
	inline badem::change_block_builder & change ()
	{
	...
	inline badem::send_block_builder & send ()
	{
	...
	inline badem::receive_block_builder & receive ()
	{
...
private:
	badem::state_block_builder state_builder;
	badem::open_block_builder open_builder;
	badem::change_block_builder change_builder;
	badem::send_block_builder send_builder;
	badem::receive_block_builder receive_builder;
};
}
