...
void previous_hex_impl (std::string const & previous_hex, std::error_code & ec, BLOCKTYPE & block)
{
	...
	{
		ec = badem::error_common::bad_previous;
	}
}
...
void account_hex_impl (std::string const & account_hex, std::error_code & ec, BLOCKTYPE & block)
{
	...
	{
		ec = badem::error_common::bad_account_number;
	}
}
...
void representative_hex_impl (std::string const & account_hex, std::error_code & ec, BLOCKTYPE & block)
{
	...
	{
		ec = badem::error_common::bad_representative_number;
	}
}
...
void source_hex_impl (std::string const & source_hex, std::error_code & ec, BLOCKTYPE & block)
{
	...
	{
		ec = badem::error_common::bad_source;
	}
}
...
void balance_dec_impl (std::string const & balance_decimal, std::error_code & ec, BLOCKTYPE & block)
{
	...
	{
		ec = badem::error_common::bad_balance;
	}
}
...
std::unordered_map<uint8_t, std::error_code> ec_map = {
	{ static_cast<uint8_t> (badem::build_flags::account_present), badem::error_common::missing_account },
	{ static_cast<uint8_t> (badem::build_flags::balance_present), badem::error_common::missing_balance },
	{ static_cast<uint8_t> (badem::build_flags::link_present), badem::error_common::missing_link },
	{ static_cast<uint8_t> (badem::build_flags::previous_present), badem::error_common::missing_previous },
	{ static_cast<uint8_t> (badem::build_flags::representative_present), badem::error_common::missing_representative },
	{ static_cast<uint8_t> (badem::build_flags::signature_present), badem::error_common::missing_signature },
	{ static_cast<uint8_t> (badem::build_flags::work_present), badem::error_common::missing_work }
};
 ...
 * If not, return the corresponding badem::error_common::missing_<...> value.
 */
...
badem::state_block_builder::state_block_builder ()
{
	make_block ();
}

badem::state_block_builder & badem::state_block_builder::make_block ()
{
	construct_block ();
	return *this;
}

badem::state_block_builder & badem::state_block_builder::from (badem::state_block const & other_block)
{
...
void badem::state_block_builder::validate ()
{
	if (!ec)
	{
...
badem::state_block_builder & badem::state_block_builder::zero ()
{
...
badem::state_block_builder & badem::state_block_builder::link_hex (std::string const & link_hex)
{
	...
	{
		ec = badem::error_common::bad_link;
	}
	return *this;
}
...
badem::open_block_builder::open_block_builder ()
{
	make_block ();
}

badem::open_block_builder & badem::open_block_builder::make_block ()
{
	construct_block ();
	return *this;
}

void badem::open_block_builder::validate ()
{
	if (!ec)
	{
		ec = check_fields_set (required_fields, build_state);
...
badem::open_block_builder & badem::open_block_builder::zero ()
{
	block->work = uint64_t (0);
	block->signature.clear ();
	block->hashables.account.clear ();
...
badem::open_block_builder & badem::open_block_builder::account (badem::account account)
{
	block->hashables.account = account;
	build_state |= build_flags::account_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::account_hex (std::string account_hex)
{
	account_hex_impl (account_hex, ec, block);
	build_state |= build_flags::account_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::account_address (std::string address)
{
	account_address_impl (address, ec, block);
	build_state |= build_flags::account_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::representative (badem::account account)
{
	block->hashables.representative = account;
	build_state |= build_flags::representative_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::representative_hex (std::string account_hex)
{
	representative_hex_impl (account_hex, ec, block);
	build_state |= build_flags::representative_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::representative_address (std::string address)
{
	representative_address_impl (address, ec, block);
	build_state |= build_flags::representative_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::source (badem::block_hash source)
{
	block->hashables.source = source;
	build_state |= build_flags::link_present;
	return *this;
...
badem::open_block_builder & badem::open_block_builder::source_hex (std::string source_hex)
{
...
badem::change_block_builder::change_block_builder ()
{
	make_block ();
}

badem::change_block_builder & badem::change_block_builder::make_block ()
{
	construct_block ();
	return *this;
}

void badem::change_block_builder::validate ()
{
	if (!ec)
	{
		ec = check_fields_set (required_fields, build_state);
...
badem::change_block_builder & badem::change_block_builder::zero ()
{
	block->work = uint64_t (0);
	block->signature.clear ();
	block->hashables.previous.clear ();
...
badem::change_block_builder & badem::change_block_builder::representative (badem::account account)
{
	block->hashables.representative = account;
	build_state |= build_flags::representative_present;
	return *this;
...
badem::change_block_builder & badem::change_block_builder::representative_hex (std::string account_hex)
{
	representative_hex_impl (account_hex, ec, block);
	build_state |= build_flags::representative_present;
	return *this;
...
badem::change_block_builder & badem::change_block_builder::representative_address (std::string address)
{
	representative_address_impl (address, ec, block);
	build_state |= build_flags::representative_present;
	return *this;
...
badem::change_block_builder & badem::change_block_builder::previous (badem::block_hash previous)
{
	block->hashables.previous = previous;
	build_state |= build_flags::previous_present;
	return *this;
...
badem::change_block_builder & badem::change_block_builder::previous_hex (std::string previous_hex)
{
	previous_hex_impl (previous_hex, ec, block);
	build_state |= build_flags::previous_present;
	return *this;
...
badem::send_block_builder::send_block_builder ()
{
	make_block ();
}

badem::send_block_builder & badem::send_block_builder::make_block ()
{
	construct_block ();
	return *this;
}

void badem::send_block_builder::validate ()
{
	if (!ec)
	{
		ec = check_fields_set (required_fields, build_state);
...
badem::send_block_builder & badem::send_block_builder::zero ()
{
	block->work = uint64_t (0);
	block->signature.clear ();
	block->hashables.previous.clear ();
...
badem::send_block_builder & badem::send_block_builder::destination (badem::account account)
{
	block->hashables.destination = account;
	build_state |= build_flags::link_present;
	return *this;
...
badem::send_block_builder & badem::send_block_builder::destination_hex (std::string account_hex)
{
...
badem::send_block_builder & badem::send_block_builder::destination_address (std::string address)
{
...
badem::send_block_builder & badem::send_block_builder::previous (badem::block_hash previous)
{
	block->hashables.previous = previous;
	build_state |= build_flags::previous_present;
	return *this;
...
badem::send_block_builder & badem::send_block_builder::previous_hex (std::string previous_hex)
{
	previous_hex_impl (previous_hex, ec, block);
	build_state |= build_flags::previous_present;
	return *this;
...
badem::send_block_builder & badem::send_block_builder::balance (badem::amount balance)
{
	block->hashables.balance = balance;
	build_state |= build_flags::balance_present;
	return *this;
...
badem::send_block_builder & badem::send_block_builder::balance_dec (std::string balance_decimal)
{
	balance_dec_impl (balance_decimal, ec, block);
	build_state |= build_flags::balance_present;
	return *this;
...
badem::send_block_builder & badem::send_block_builder::balance_hex (std::string balance_hex)
{
	balance_hex_impl (balance_hex, ec, block);
	build_state |= build_flags::balance_present;
	return *this;
...
badem::receive_block_builder::receive_block_builder ()
{
	make_block ();
}

badem::receive_block_builder & badem::receive_block_builder::make_block ()
{
	construct_block ();
	return *this;
}

void badem::receive_block_builder::validate ()
{
	if (!ec)
	{
		ec = check_fields_set (required_fields, build_state);
...
badem::receive_block_builder & badem::receive_block_builder::zero ()
{
	block->work = uint64_t (0);
	block->signature.clear ();
	block->hashables.previous.clear ();
...
badem::receive_block_builder & badem::receive_block_builder::previous (badem::block_hash previous)
{
	block->hashables.previous = previous;
	build_state |= build_flags::previous_present;
	return *this;
...
badem::receive_block_builder & badem::receive_block_builder::previous_hex (std::string previous_hex)
{
	previous_hex_impl (previous_hex, ec, block);
	build_state |= build_flags::previous_present;
	return *this;
...
badem::receive_block_builder & badem::receive_block_builder::source (badem::block_hash source)
{
	block->hashables.source = source;
	build_state |= build_flags::link_present;
	return *this;
...
badem::receive_block_builder & badem::receive_block_builder::source_hex (std::string source_hex)
{
	source_hex_impl (source_hex, ec, block);
	build_state |= build_flags::link_present;
	return *this;
