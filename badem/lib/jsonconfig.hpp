...
class jsonconfig : public badem::error_aware<>
{
public:
	jsonconfig () :
	tree (tree_default)
	...
	jsonconfig (boost::property_tree::ptree & tree_a, std::shared_ptr<badem::error> error_a = nullptr) :
	 ...
	 */
	template <typename T>
	badem::error & read_and_update (T & object, boost::filesystem::path const & path_a)
	{
	...
	jsonconfig & put_child (std::string const & key_a, badem::jsonconfig & conf_a)
	{
		tree.add_child (key_a, conf_a.get_tree ());
		return *this;
	}

	jsonconfig & replace_child (std::string const & key_a, badem::jsonconfig & conf_a)
	{
		tree.erase (key_a);
	...
	badem::error & get_error () override
	{
		return *error;
	}
...
private:
	...
	/** We're a badem::error_aware type. Child nodes share the error state. */
	std::shared_ptr<badem::error> error;
};
}
