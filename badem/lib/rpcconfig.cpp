...
badem::error badem::rpc_secure_config::serialize_json (badem::jsonconfig & json) const
{
...
badem::error badem::rpc_secure_config::deserialize_json (badem::jsonconfig & json)
{
...
badem::error badem::rpc_secure_config::serialize_toml (badem::tomlconfig & toml) const
{
...
badem::error badem::rpc_secure_config::deserialize_toml (badem::tomlconfig & toml)
{
...
badem::rpc_config::rpc_config (bool enable_control_a) :
enable_control (enable_control_a)
{
}

badem::error badem::rpc_config::serialize_json (badem::jsonconfig & json) const
{
	...
	badem::jsonconfig rpc_process_l;
...
badem::error badem::rpc_config::deserialize_json (bool & upgraded_a, badem::jsonconfig & json)
{
	if (!json.empty ())
	{
...
badem::error badem::rpc_config::serialize_toml (badem::tomlconfig & toml) const
{
	...
	badem::tomlconfig rpc_process_l;
...
badem::error badem::rpc_config::deserialize_toml (badem::tomlconfig & toml)
{
	if (!toml.empty ())
	{
...
namespace badem
{
badem::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, badem::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	badem::error error;
	auto json_config_path = badem::get_rpc_config_path (data_path_a);
	auto toml_config_path = badem::get_rpc_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		{
			// Migrate
			badem::rpc_config config_json_l;
			...
			if (!error)
			{
				...
				badem::rpc_config config_defaults_l;
...
badem::error read_and_update_rpc_config (boost::filesystem::path const & data_path, badem::rpc_config & config_a)
{
	...
	auto config_path = badem::get_rpc_config_path (data_path);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the badem_rpc executable file path based on where the currently running executable is found.
