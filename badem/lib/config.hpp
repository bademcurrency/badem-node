...
*/
static const char * BADEM_VERSION_STRING = xstr (TAG_VERSION_STRING);
...
public:
	...
	network_constants (badem_networks network_a) :
	...
	badem_networks current_network;
	 ...
	 */
	static void set_active_network (badem_networks network_a)
	{
	 ...
	 */
	static badem::error set_active_network (std::string network_a)
	{
		badem::error err;
		if (network_a == "live")
		{
			active_network = badem::badem_networks::badem_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = badem::badem_networks::badem_beta_network;
		}
		else if (network_a == "test")
		{
			active_network = badem::badem_networks::badem_test_network;
		}
		else
		{
	...
	{
		return current_network == badem_networks::badem_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == badem_networks::badem_beta_network;
	}
	bool is_test_network () const
	{
		return current_network == badem_networks::badem_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static badem::badem_networks active_network;
};
