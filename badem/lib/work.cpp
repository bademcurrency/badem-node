...
bool badem::work_validate (badem::block_hash const & root_a, uint64_t work_a, uint64_t * difficulty_a)
{
...
uint64_t badem::work_value (badem::block_hash const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
badem::work_pool::work_pool (unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (badem::uint256_union const &, uint64_t)> opencl_a) :
ticket (0),
done (false),
...
void badem::work_pool::cancel (badem::uint256_union const & root_a)
{
...
void badem::work_pool::generate (badem::uint256_union const & hash_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
	generate (hash_a, callback_a, network_constants.publish_threshold);
}

void badem::work_pool::generate (badem::uint256_union const & hash_a, std::function<void(boost::optional<uint64_t> const &)> callback_a, uint64_t difficulty_a)
{
...
uint64_t badem::work_pool::generate (badem::uint256_union const & hash_a)
{
	return generate (hash_a, network_constants.publish_threshold);
}

uint64_t badem::work_pool::generate (badem::uint256_union const & hash_a, uint64_t difficulty_a)
{
