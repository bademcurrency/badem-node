...
bool badem::work_validate (badem::root const & root_a, uint64_t work_a, uint64_t * difficulty_a)
{
	static badem::network_constants network_constants;
	auto value (badem::work_value (root_a, work_a));
...
bool badem::work_validate (badem::block const & block_a, uint64_t * difficulty_a)
{
	return work_validate (block_a.root (), block_a.block_work (), difficulty_a);
}

uint64_t badem::work_value (badem::root const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
badem::work_pool::work_pool (unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (badem::root const &, uint64_t, std::atomic<int> &)> opencl_a) :
ticket (0),
done (false),
...
opencl (opencl_a)
{
	...
	for (auto i (0u); i < count; ++i)
	{
		auto thread (boost::thread (attrs, [this, i]() {
			badem::thread_role::set (badem::thread_role::name::work);
			badem::work_thread_reprioritize ();
			loop (i);
		}));
...
void badem::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	badem::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
...
void badem::work_pool::cancel (badem::root const & root_a)
{
...
void badem::work_pool::generate (badem::root const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a)
{
	generate (root_a, callback_a, network_constants.publish_threshold);
}

void badem::work_pool::generate (badem::root const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a, uint64_t difficulty_a)
{
...
boost::optional<uint64_t> badem::work_pool::generate (badem::root const & root_a)
{
	return generate (root_a, network_constants.publish_threshold);
}

boost::optional<uint64_t> badem::work_pool::generate (badem::root const & root_a, uint64_t difficulty_a)
{
...
std::unique_ptr<seq_con_info_component> collect_seq_con_info (work_pool & work_pool, const std::string & name)
{
	...
	size_t count = 0;
	{
		badem::lock_guard<std::mutex> guard (work_pool.mutex);
