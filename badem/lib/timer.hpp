...
public:
	...
	timer (badem::timer_state state_a, std::string description_a = "timer") :
	desc (description_a)
	{
		if (state_a == badem::timer_state::started)
		{
			start ();
		}
	}
	...
	void start ()
	{
		assert (state == badem::timer_state::stopped);
		state = badem::timer_state::started;
		begin = CLOCK::now ();
	}
	...
	UNIT stop ()
	{
		assert (state == badem::timer_state::started);
		state = badem::timer_state::stopped;
	...
	badem::timer_state current_state () const
	{
		return state;
	}
...
private:
	...
	badem::timer_state state{ badem::timer_state::stopped };
	std::string desc;
...
class autotimer : public badem::timer<UNIT>
{
public:
	autotimer (std::string description_a, std::ostream & stream_a = std::cout) :
	badem::timer<UNIT> (description_a), stream (stream_a)
	{
		badem::timer<UNIT>::start ();
	}
	~autotimer ()
	{
		badem::timer<UNIT>::stop (stream);
	}
