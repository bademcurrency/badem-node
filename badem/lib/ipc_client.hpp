...
namespace ipc
{
	...
	public:
		...
		badem::error connect (std::string const & path);

		/** Connect to a tcp socket synchronously */
		badem::error connect (std::string const & host, uint16_t port);

		/** Connect to a tcp socket asynchronously */
		void async_connect (std::string const & host, uint16_t port, std::function<void(badem::error)> callback);

		/** Write buffer asynchronously */
		void async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(badem::error, size_t)> callback_a);

		/** Read \p size_a bytes asynchronously */
		void async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(badem::error, size_t)> callback_a);
	...
	std::string request (badem::ipc::ipc_client & ipc_client, std::string const & rpc_action_a);
	 ...
	 */
	std::shared_ptr<std::vector<uint8_t>> prepare_request (badem::ipc::payload_encoding encoding_a, std::string const & payload_a);
}
}
