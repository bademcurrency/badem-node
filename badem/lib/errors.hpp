...
REGISTER_ERROR_CODES (badem, error_common);
REGISTER_ERROR_CODES (badem, error_blocks);
REGISTER_ERROR_CODES (badem, error_rpc);
REGISTER_ERROR_CODES (badem, error_process);
REGISTER_ERROR_CODES (badem, error_config);
...
inline std::error_code make_error_code (boost::system::errc::errc_t e)
{
	...
	::badem::error_conversion::generic_category ());
}
}
namespace badem
{
namespace error_conversion
{
	...
	inline std::error_code convert (const boost::system::error_code & error)
	{
		...
		return badem::error_common::invalid_type_conversion;
	}
}
}
...
public:
	error () = default;
	error (badem::error const & error_a) = default;
	error (badem::error && error_a) = default;
	...
	{
		code = badem::error_common::generic;
		message = std::move (message_a);
	}

	error (std::exception const & exception_a)
	{
		code = badem::error_common::exception;
		message = exception_a.what ();
	}

	error & operator= (badem::error const & err_a)
	{
		code = err_a.code;
	...
	error & operator= (badem::error && err_a)
	{
		code = err_a.code;
	...
	{
		code = badem::error_conversion::convert (code_a);
		message.clear ();
		return *this;
	}
	...
	{
		code = badem::error_conversion::convert (boost::system::errc::make_error_code (code_a));
		message.clear ();
		return *this;
	}

	/** Set the error to badem::error_common::generic and the error message to \p message_a */
	...
	/** Sets the error to badem::error_common::exception and adopts the exception error message. */
	...
	error & then (std::function<badem::error &()> next)
	{
	...
	error & set (std::string message_a, std::error_code code_a = badem::error_common::generic)
	{
		message = message_a;
		code = code_a;
		return *this;
	...
	/** Set a custom error message. If the error code is not set, it will be set to badem::error_common::generic. */
...
/**
 * A type that manages a badem::error.
 * The default return type is badem::error&, though shared_ptr<badem::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = badem::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, badem::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<badem::error>>::value, "Must be badem::error& or shared_ptr<badem::error>");
