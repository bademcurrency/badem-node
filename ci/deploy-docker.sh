...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="ubuntu:18.04:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t bademcurrency/badem-beta:master --cache-from bademcurrency/badem-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push bademcurrency/badem-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=bademcurrency/badem-beta:master"
            docker pull bademcurrency/badem-beta:master
        fi
